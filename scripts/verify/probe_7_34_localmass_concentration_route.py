...
def main():
    ...
    if len(rows) >= 3:
        ...
        print("  bounded-influence functional => the reversible-chain (averaged Dirichlet-form)")
        print("  Markov-Poincare / Marton-coupling concentration (Paulin 2015; Samson 2000) gives")
        print("  Var(log2 q_t)=O(1)=o(n). Since Var(-log2 S_n)=Var(log2 q_t)+o(1) (Abel summation;")
        print("  the j_n additive part is a deterministic/smooth-additive shift, NOT a variance")
        print("  contributor -- see /tmp check_id), this gives Var(log2 S_n)/n->0, i.e. the DISPERSION")
        print("  achievability V_op<=V_lossless. The annealed near-mean lattice LLT (Aaronson-Denker,")
        print("  aperiodic, density-floor-free) is needed ONLY for the 3rd-order (1/2)log n PREFACTOR")
        print("  (the MEAN of -log S_n), NOT the dispersion.")
        print("  CONTRAST: the paper's 'Theta(n) energy, no concentration reaches o(n)' (Rmk 7.34b)")
        print("  was measured only in the FROZEN-RADIUS regime (t in {0,1,2}, n<=20); in the near-mean")
        print("  dispersion regime BOTH log2 S_n and log2 q_t have O(1) energy. The obstruction as")
        print("  stated is a regime artifact. THE ONE REMAINING LEMMA: a rigorous averaged-Dirichlet")
        print("  Markov-Poincare bound Var(log2 q_t)=O(1) for the filter functional on the BSMS path")
        print("  measure (resample-vs-flip energy comparison; O(1) single-site Glauber gap).")
