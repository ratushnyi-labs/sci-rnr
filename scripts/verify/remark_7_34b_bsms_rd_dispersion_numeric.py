...
if __name__ == "__main__":
    ...
    print( "  MECHANISM: Hamming distortion is additive, so the tilted backward kernel exp(-lambda*d_H)")
    print( "  FACTORIZES into single-letter BSC kernels => the SLB-achieving backward channel is FORCED")
    print( "  memoryless => deconvolving the Markov law by iid BSC(D)^n is valid only for n<=n0(D) and")
    print( "  FAILS for all larger n (window shrinks to 0 as n->inf; valid at n=2, fails n>=3 at D=0.05).")
    print( "  Since the dispersion is the n->inf limit at fixed D, the SLB-exact identity j_n=i_n-const")
    print( "  fails ASYMPTOTICALLY => for large n, j_n != i_n-const and (n-stably) Vj/Vi<1 => V_conv<V_lossless.")
    print(f"  B: SLB ever tight at D>0 for BSMS: {slb_tight_any};  V_n ever ~ V_lossless at D>0: {vn_at_Vloss_any}")
    if plateauA and not vn_at_Vloss_any:
        print("  => CONJECTURE B NOT SUPPORTED by the converse (d-tilted) dispersion: the exact n-letter")
        print("     Var(j_n)/n sits strictly BELOW the lossless varentropy at every tested D>0, with no")
        print("     plateau at V_lossless. Either V_op<V_lossless (B false) OR achievability covering")
        print("     excess (run-flipping) supplies the gap -- i.e. the converse is loose and the true")
        print("     dispersion is achievability-dominated. The d-tilted converse, tight for iid/Gaussian,")
        print("     does NOT certify V_lossless here -- a genuine discrete-Markov-specific phenomenon.")
    elif plateauA and vn_at_Vloss_any:
        print("  => conjecture B SUPPORTED: V_n plateaus at V_lossless in the Gray region.")
    else:
        print("  => INCONCLUSIVE (method validation failed or borderline).")
