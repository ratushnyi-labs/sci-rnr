...
if __name__ == "__main__":
    ...
    print( "  MECHANISM (now provable, per adversarial review): Hamming distortion is additive, so the")
    print( "  tilted backward kernel exp(-lambda*d_H) FACTORIZES into single-letter BSC kernels => the")
    print( "  SLB-achieving (Kostina condition-66) backward channel is FORCED memoryless => it cannot")
    print( "  reproduce the Markov law at D>0,n>=2 => condition (66) FAILS => j_n is NOT i_n - const")
    print( "  (numerically a strict variance contraction Vj/Vi<1, n-stable). So Var(j_n)/n < V_lossless.")
