...
if __name__ == "__main__":
    ...
    print( "  => On the WHOLE Gray region (0,D_c]: j_n=i_n-n h(D) for all n (i_n=iid switch-count) => BOTH")
    print( "     converse and achievability give V_op = V_lossless. CONJECTURE B is CONFIRMED & SOLVED there.")
    print( "  => For D>D_c (OUTSIDE the Gray region) the deconvolution fails, V_conv<V_lossless; separate open Q.")
    ...
    print("="*96)
    print("RESULT: discrete-Markov (BSMS,Hamming) RD dispersion SOLVED on the Gray region (0,D_c(p)]:")
    print("        V_op(D) = V_lossless = p(1-p)log2^2((1-p)/p) (conjecture B confirmed, both bounds).")
    print("        Open: the D>D_c regime (outside Gray) -- needs a quenched inhomogeneous Bahadur-Rao.")
