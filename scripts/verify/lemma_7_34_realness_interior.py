...
============================================================================
BUG-009-D / Route B (A=3): REALNESS OF THE REPLICA SPECTRUM AT ALL
INTERIOR ANGLES 0 < s < pi, proven on the whole open superdomain.

THEOREM (certified here, exact integer/rational arithmetic end to end).
  For A = 3, all p in (0, 2/3), all D in (0, Dbar(p)], and ALL spectral
  angles s in (0, pi), the 5x5 pattern-quotient replica matrix
  Q(p, D, w = e^{is}) has 5 REAL, DISTINCT eigenvalues.
  (Dbar(p) = (2/3)(1-sg)^2/(1+sg^2), sg = sqrt(1-(3/2)p): the proven upper
  bound of the Gray threshold, lemma_7_34_dbar_upper_bound.py.  Together
  with the committed s = pi lemma, lemma_7_34_realness_pi_w_closure.py,
  realness holds for ALL angles s in (0, pi] on the superdomain -- item
  (c) of the mid-band program.)

REDUCTION TO A POLYNOMIAL DISCRIMINANT (each step gated in-script):
  (1) M := c Q, c = p(1-p) G Gt, G = D^2 w + (D-2)(1-D), Gt = D^2 +
      (D-2)(1-D) w, is a POLYNOMIAL matrix (R1a); chiM = charpoly(M) =
      ...
      Laurent identity per k (R1c).  On |w| = 1 (t = 1 - cos s):
        psi(Y) := Y^5 + sum_k 2^k Etld_k(p,D,t) Y^{5-k}
      has INTEGER polynomial coefficients (R3) and satisfies
      psi(Y) = 2^5 w^{-5} chiM(wY/2): its roots are exactly
        2 p(1-p) B(D,t) * lambda_i(Q),
      B(D,t) = (3D-2)^2 + 2tD^2(1-D)(2-D) = G Gt / w  (R2), and B > 0 for
      0 < D < 2/3, t >= 0 (sum of nonnegatives, first term strictly
      positive).  So realness/simplicity of Q's spectrum on the domain
      is EQUIVALENT to realness/simplicity of psi's roots.
  (2) DISC(p,D,t) := disc_Y(psi): ONE integer polynomial (34171 terms,
      deg (54,78,20)), assembled through the universal quintic
      discriminant (59 terms, isobaric weight 20, R4/R5) and validated
      twice BEFORE any box run: exact rational equality with sympy's
      univariate discriminant at 24 random points (R6), and an mpmath
      eigenvalue-based sign+ratio cross-check at >= 50 points (R7).
  (3) t=2 face structure (R8): DISC(p,D,2) == 2^20 disc_X(f4) *
      f4(lam1M)^2, with chiM|_{w=-1} = f1 f4 the committed w=-1 split --
      the (evens * W * R^2) shape of the s=pi lemma, rederived in-script.
  (4) Rationalized domain (the committed move):  p = (2/3)(1-sg^2),
      D = th*Dbar, Dbar = (2/3)(1-sg)^2/(1+sg^2), (sg,th,t) in
      [0,1]x[0,1]x[0,2].  target := 3^132 (1+sg^2)^78 DISC(subst) is an
      integer polynomial (R9a, exact Fraction gates), and by EXACT
      division (R9b/R9c)
        target = content * th^14 sg^48 t^7 (1-sg)^34 (1+sg^2)^4 * core,
      every stripped factor > 0 on
        Omega := {0<sg<1} x {0<th<=1} x {0<t<2}
      (sg=0 <=> p=2/3, sg=1 <=> p=0, th=0 <=> D=0, t=0 <=> s=0 are all
      excluded limits).  Hence sign(disc psi) = sign(core) on Omega and
      the theorem reduces to:  core > 0 on Omega.
  (5) core > 0 ON Omega -- Bernstein decomposition (all certificates in
      exact integer arithmetic; "no-negatives" = all scaled Bernstein
      coefficients >= 0, not all zero => f >= 0 on the closed box AND
      f > 0 on the OPEN box; "closed-strict" = min > 0 AND no exact-zero
      coefficients => f > 0 on the CLOSED box; closed-strict pieces may
      be bisected, that gluing is sound):
        C1   [0,1] x [0,1]      x [0,15/8]      no-negatives + th=1 face
        C2   [0,1] x [0,23/32]  x [7/4,2]       no-negatives
        C3   [0,1] x [13/16,1]  x [7/4,2]       no-negatives + th=1 face
        C4   [1/64,1] x [11/16,13/16] x [7/4,2] CLOSED-STRICT
        W3a  [0,1/64] x [11/16,13/16] x [7/4,127/64]   CLOSED-STRICT
        W3b  [0,1/64] x [11/16,23/32] x [127/64,2]     CLOSED-STRICT
        W3c  [0,1/64] x [25/32,13/16] x [127/64,2]     CLOSED-STRICT
        W3d  the blow-up window [0,1/64]x[23/32,25/32]x[127/64,2]
             around the unique boundary pinch Z3 = (0, 3/4, 2).
      WHY Z3 NEEDS A BLOW-UP: the closed-box zero set of core consists of
      the identically-zero edge (sg=0, th=1), the corners on it, and the
      single interior-of-edge zero Z3 (T3 := 9 + 4t th(2th-3) satisfies
      T3 = (9/2)(2-t) + 8t(th-3/4)^2 >= 0 -- an in-script identity --
      and the t=2 face of core carries T3^2).  A zero at an interior
      point of a face/edge restriction that is not identically zero
      forces a negative Bernstein coefficient at EVERY box whose closure
      contains it, so no subdivision can certify a neighborhood of Z3;
      the standard cure (committed W-closure lemma) is a projective
      blow-up of finite order.
  (6) W3d, quasi-homogeneous order-4 blow-up (R13).  Local coordinates
      s = sg, m = th - 3/4, u = 2 - t (window |m| <= 1/32, s <= 1/64,
      u <= 1/64); core_loc := 4^64 * core(s, 3/4+m, 2-u) in Z[s,m,u].
      VERIFIED: minimal (1,1,2)-weighted degree of core_loc is EXACTLY 4
      (R13b), and the weighted-leading form is
        L4 = c002 u^2 + c021 (m - (3/2)s)^2 u
             + lam [ (1/2)s^4 + (1/81)(s - (2/3)m)^4 ],
      c002, c021, lam > 0 -- EXACT identities (R13c; the (u=0) quartic is
      the SAME K1-corner form as the committed W-closure lemma).  L4 > 0
      for u >= 0, (s,m,u) != 0.  Charts (monomial maps, bijective, exact
      inverse reconstruction gates R13d, Fraction spot identities R13e):
        HS :  core_loc(s, s mh, s^2 uh)     = s^4  HS(s, mh, uh)
        HM+/-: core_loc(mu sh, +-mu, mu^2 uh) = mu^4 HM+-(mu, sh, uh)
        HU :  core_loc(nu sh, nu mh, nu^2)  = nu^4 HU(nu, sh, mh)
      certified CLOSED-STRICT on  [0,1/64]x[-1,1]x[0,1],
      [0,1/32]x[0,1]x[0,1] (both signs), [0,1/8]x[0,1]x[-1,1]  (R13f).
      SOUNDNESS: for (s,m,u) != 0 in the window let rho = max(s, |m|,
      sqrt(u)); the corresponding chart point lies in its CLOSED box and
      the identity gives core = rho^4 * H(...) > 0.  Only Z3 itself
      (t = 2, outside Omega) is excluded.
  (7) COVERAGE (R14, exact rational comparisons).  For t in (0,15/8):
      C1 + its th=1 face.  For t in [15/8,2): th-union
      (0,23/32) u [11/16,13/16] u (13/16,1) u {1} = (0,1], where
      [11/16,13/16] is covered for sg >= 1/64 by C4 (closed) and for
      sg <= 1/64 by W3a (t <= 127/64), W3b/W3c (th outside the
      [23/32,25/32] window), and W3d-charts (the window, t >= 127/64).
      t-overlap: 7/4 < 15/8 < 127/64.   =>  core > 0 on all of Omega.
  (8) ANCHOR + CONNECTEDNESS (R15): at (sg,th,t) = (1/2,3/4,1), i.e.
      (p,D) = (1/2,1/10), s = pi/2: psi has 5 DISTINCT REAL roots (exact
      Sturm) and DISC > 0 (exact).  Omega is path-connected; disc psi > 0
      on Omega; along any path the real-root count of a monic real
      quintic can change only through a collision (disc = 0), which never
      happens on Omega.  Hence 5 distinct real roots everywhere on Omega,
      i.e. the replica spectrum is REAL and SIMPLE.               QED

COMBINED SPECTRAL STATE for A=3 after this certificate: realness holds
on the whole superdomain for all angles s in (0, pi] (this script + the
committed s=pi lemma); with the committed positive-side certificates
(chi', ..., chi'''' full-domain, cert5-on-Gray + Schur small-p) and the
committed negative-side certificates (psi^(k)(Lambda) > 0 full-domain),
the spectral-radius chain rho(Q) < Lambda on Gray -- i.e. R_3(s) >= 3/2
on the whole Gray family, all angles -- is CLOSED for A = 3.

ENGINE (packed-key integer tensors, key = a_sg<<20 | b_th<<10 | c_t):
...
HONEST NOTES / SCOPE:
 *  The claim is the OPEN angle interval (0, pi): s = pi realness is the
    separate committed lemma (the t=2 face of core genuinely vanishes at
    the excluded pinch Z3 and along the excluded edge (sg=0, th=1);
    both are OUTSIDE Omega).
 *  th = 1 (D = Dbar) IS included; th = 0 (D = 0), t = 0 (s = 0),
    sg in {0,1} (p in {2/3, 0}) are excluded limits where the quotient
    construction itself degenerates (eta(w=1) = 0 forces rank 1: the t^7
    factor; D = 0 likewise: th^14; p = 2/3 rank 1: sg^48; p = 0: the
    channel is the identity: (1-sg)^34).
 *  Simplicity (distinctness) is part of the certificate: disc > 0.
 *  R7 is a floating-point cross-check of an exact object (belt and
    suspenders on top of the exact R6); every load-bearing certificate
    is exact integer arithmetic.
 *  Runtime ~20-35 min single-threaded, ~2.5 GB peak RSS.

Deps: sympy, mpmath; imports the committed pipeline
lemma_7_34_midband_chi_certificates.py (same directory, read-only).
...
from lemma_7_34_midband_chi_certificates import (  # noqa: E402
    A, AVAL, D, p, w, t, sg, th, X, STATES, REPS, _pat,
    build_Q, sym_to_t_mid, PSUB, DSUB)

mp.mp.dps = 60
T0 = time.time()
PASS = True
CACHE = os.environ.get("RIC_CACHE", "")   # optional dev cache dir
...
def bern_axis(T, ax, dax):
    """scaled Bernstein transform along one axis (box [0,1])."""
    sh = SH3[ax]
    out = {}
    g = out.get
...
def closed_strict(T, degs, box, name, depth=0, maxdepth=7, budget=None):
...
def engine_selftest():
    ...
    rep("R0c engine: 2xy-1 shows negatives; x+y+1 closed-strict", okC1 and okC2)


# ===========================================================================
# R8 helper: w = -1 split and the t=2 face identity
# ===========================================================================
def t2_face_gate(DISC):
    ...
    return sp.expand(face2E - 2 ** 20 * disc4E * Rres ** 2) == 0


# ===========================================================================
# R9 helpers: substitution and strips
# ===========================================================================
def substitute_domain(DISC, Ap, Bd):
    """target = 3^(Ap+Bd) (1+sg^2)^Bd DISC(p->(2/3)(1-sg^2),
    D->th(2/3)(1-sg)^2/(1+sg^2), t) as a packed integer dict."""
    pow1m = [[1]]
    ...
    for (a, b), lst in groups.items():
        ...
        W = [0] * (2 * a + len(V))
        for i in range(a + 1):
            u = comb(a, i) * (-1 if i & 1 else 1)
            base = 2 * i
            for j, y in enumerate(V):
                if y:
                    W[base + j] += u * y
        mulc = P2[a + b] * P3[Ap + Bd - a - b]
        for (cc, cf) in lst:
            f = cf * mulc
            kb = (b << 10) | cc
...
def strip_target(target):
    ...
    def div_1psg2(T):
        ...
        for key, d in S.items():
            ...
            chk = {}
            for a2, v in r.items():
                chk[a2] = chk.get(a2, 0) + v
                chk[a2 + 2] = chk.get(a2 + 2, 0) + v
            if ({a2: v for a2, v in chk.items() if v}
                    != {a2: v for a2, v in d.items() if v}):
                return None
            out[key] = r
        return from_slices(out)
    ...
    core = dict(target)
    strips = {}
    for name, fn in (("th", lambda T: div_mono(T, 'th')),
...
# ===========================================================================
# R13 helpers: the Z3 blow-up
# ===========================================================================
def build_local(core, DTH):
    """core_loc = 4^DTH * core(s, 3/4 + m, 2 - u) as {(i,j,k): int}."""
    loc = {}
    P4 = [4 ** e for e in range(DTH + 1)]
    for k0, v in core.items():
        a, b, cc = k0 >> 20, (k0 >> 10) & 1023, k0 & 1023
        for j in range(b + 1):
            cb = comb(b, j) * (3 ** (b - j)) * P4[DTH - (b - j)]
            vb = v * cb
...
def main():
    print("=" * 78)
    print("Interior-angle realness, A = 3: disc(psi) > 0 on the rationalized")
    print("superdomain for all t in (0,2)  --  exact Bernstein certificates")
    print("=" * 78)
    engine_selftest()
    ...
    ok3 = True
    for k in range(1, 6):
        Pk = sp.Poly(sp.expand(2 ** k * Etld[k]), p, D, t)
        Tk = {}
        for mon, cc in zip(Pk.monoms(), Pk.coeffs()):
            ok3 &= bool(cc.is_Integer)
            Tk[(mon[0] << 16) | (mon[1] << 8) | mon[2]] = int(cc)
        Ehat.append(Tk)
    rep("R3 Ehat_k = 2^k Etld_k have integer coefficients", ok3)
    ...
    # ---------------- R5 ----------------
    cachef = os.path.join(CACHE, "disc_pdt.pkl") if CACHE else None
    ...
    rep(f"R5 DISC: {len(DISC)} terms, degs ({dP},{dD},{dT})",
        (dP, dD, dT) == (54, 78, 20) and len(DISC) == 34171)

    # ---------------- R6 ----------------
    log("R6 exact univariate-disc validation, 24 random rational points")
    random.seed(20260704)
    ok6 = True
    Y = sp.Symbol('Y')
    for _ in range(24):
        pv = sp.Rational(random.randint(1, 63), 96)
        Dv = sp.Rational(random.randint(1, 63), 100)
        tv = sp.Rational(random.randint(0, 32), 16)
        psi = Y ** 5
        for k in range(1, 6):
            psi += sp.Rational(2 ** k) * Etld[k].subs(
        ...
        ok6 &= (Fr(int(sp.fraction(dref)[0]), int(sp.fraction(dref)[1]))
                == dval)
    rep("R6 DISC == disc_Y(psi) exactly at 24 random rational points", ok6)
    ...
    for _ in range(90):
        ...
        Gv = mp.mpf(Dv) ** 2 * mp.e ** (1j * sv) \
            + (mp.mpf(Dv) - 2) * (1 - mp.mpf(Dv))
        rho = mp.mpf(pv) * (1 - mp.mpf(pv)) * abs(Gv) ** 2
        rts = [2 * rho * e for e in ev]
        dnum = mp.mpf(1)
    ...
    rep(f"R7 sign+ratio agree at {npts} pts (worst |ratio-1| ~ "
        f"{mp.nstr(worst, 3)})", ok7 and npts >= 40)

    # ---------------- R8 ----------------
    log("R8 t=2 face identity (w = -1 split rederived in-script) ...")
    rep("R8 DISC(p,D,2) == 2^20 disc(f4) f4(lam1M)^2", t2_face_gate(DISC))

    # ---------------- R9 ----------------
    log("R9 domain substitution -> target(sg,th,t)")
    target = substitute_domain(DISC, dP, dD)
    random.seed(31)
    ok9a = True
    for _ in range(8):
        ...
        pv = Fr(2, 3) * (1 - sgv ** 2)
        Dv = thv * Fr(2, 3) * (1 - sgv) ** 2 / (1 + sgv ** 2)
        dv = sum(Fr(cf) * pv ** (k >> 16) * Dv ** ((k >> 8) & 255)
                 * tv ** (k & 255) for k, cf in DISC.items())
        tv2 = sum(Fr(cf) * sgv ** (k >> 20) * thv ** ((k >> 10) & 1023)
                  * tv ** (k & 1023) for k, cf in target.items())
        ok9a &= (tv2 == Fr(3) ** (dP + dD) * (1 + sgv ** 2) ** dD * dv)
    rep("R9a target == 3^132 (1+sg^2)^78 DISC(subst): 8 exact points", ok9a)
    core, strips, content = strip_target(target)
    log(f"R9 strips: {strips}; content bits {content.bit_length()}")
    ok9b = (strips.get('th', 0) == 14 and strips.get('sg', 0) == 48
            and strips.get('t', 0) == 7 and strips.get('1-sg', 0) == 34
            and strips.get('1+sg^2', 0) == 4
            and set(strips) <= {'th', 'sg', 't', '1-sg', '1+sg', '1+sg^2'}
            and content > 0)
    rep("R9b target == content th^14 sg^48 t^7 (1-sg)^34 (1+sg^2)^4 core",
        ok9b)
    DS = max(k >> 20 for k in core)
    DTH = max((k >> 10) & 1023 for k in core)
    DT = max(k & 1023 for k in core)
    degs = [DS, DTH, DT]
    log(f"R9 core: {len(core)} terms, degs ({DS},{DTH},{DT})")
    random.seed(41)
    ok9c = True
    for _ in range(4):
        sgv = Fr(random.randint(1, 15), 16)
        ...
        cv = sum(Fr(cf) * sgv ** (k >> 20) * thv ** ((k >> 10) & 1023)
                 * tv ** (k & 1023) for k, cf in core.items())
        tv2 = sum(Fr(cf) * sgv ** (k >> 20) * thv ** ((k >> 10) & 1023)
                  * tv ** (k & 1023) for k, cf in target.items())
        recon = (content * thv ** 14 * sgv ** 48 * tv ** 7
                 * (1 - sgv) ** 34 * (1 + sgv ** 2) ** 4 * cv)
        ok9c &= (recon == tv2)
    ...
    fth1 = face_th1(core)
    ok10f = no_negatives(fth1, [DS, 0, DT], [(0, 1), (0, 1), (0, Fr(15, 8))],
                         "R10f C1 th=1 face", axes=(2, 0))
    rep("R10f th=1 face, t in [0,15/8]: no negatives (open-face > 0)",
        ok10f)

    # ---------------- R11: C2/C3/C4 ----------------
    ok11a = no_negatives(core, degs,
                         [(0, 1), (0, Fr(23, 32)), (Fr(7, 4), 2)],
                         "R11a C2 [0,1]x[0,23/32]x[7/4,2]")
    rep("R11a C2 no negatives", ok11a)
    ok11b = no_negatives(core, degs,
                         [(0, 1), (Fr(13, 16), 1), (Fr(7, 4), 2)],
                         "R11b C3 [0,1]x[13/16,1]x[7/4,2]")
    rep("R11b C3 no negatives", ok11b)
    ok11c = no_negatives(fth1, [DS, 0, DT], [(0, 1), (0, 1), (Fr(7, 4), 2)],
                         "R11c C3 th=1 face", axes=(2, 0))
    rep("R11c th=1 face, t in [7/4,2]: no negatives", ok11c)
    ok11d = closed_strict(core, degs,
                          [(Fr(1, 64), 1), (Fr(11, 16), Fr(13, 16)),
                           (Fr(7, 4), 2)],
                          "R11d C4 [1/64,1]x[11/16,13/16]x[7/4,2]")
    rep("R11d C4 CLOSED-STRICT", ok11d)

    # ---------------- R12: wedge boxes ----------------
    ok12a = closed_strict(core, degs,
                          [(0, Fr(1, 64)), (Fr(11, 16), Fr(13, 16)),
                           (Fr(7, 4), Fr(127, 64))], "R12a W3a")
    rep("R12a W3a [0,1/64]x[11/16,13/16]x[7/4,127/64] CLOSED-STRICT",
        ok12a)
    ok12b = closed_strict(core, degs,
                          [(0, Fr(1, 64)), (Fr(11, 16), Fr(23, 32)),
                           (Fr(127, 64), 2)], "R12b W3b")
    rep("R12b W3b [0,1/64]x[11/16,23/32]x[127/64,2] CLOSED-STRICT", ok12b)
    ok12c = closed_strict(core, degs,
                          [(0, Fr(1, 64)), (Fr(25, 32), Fr(13, 16)),
                           (Fr(127, 64), 2)], "R12c W3c")
    rep("R12c W3c [0,1/64]x[25/32,13/16]x[127/64,2] CLOSED-STRICT", ok12c)

    # ---------------- R13: the Z3 blow-up ----------------
    log("R13 local dict at Z3 = (0, 3/4, 2) ...")
    # T3 identity used in the docstring's zero-set analysis
    T3 = 8 * t * th ** 2 - 12 * t * th + 9
    okT3 = sp.expand(T3 - (sp.Rational(9, 2) * (2 - t)
                           + 8 * t * (th - sp.Rational(3, 4)) ** 2)) == 0
    rep("R13a T3 == (9/2)(2-t) + 8t(th-3/4)^2 (pinch location identity)",
        okT3)
    loc = build_local(core, DTH)
    del core, fth1          # memory: no longer needed (R14/R15 use DISC)
    ...
    lam = Fr(c040) * Fr(6561, 16)
    okL4 = (c002 > 0 and c021 > 0
            and c111 ** 2 == 4 * c021 * c201
            and Fr(c111, c021) == Fr(-3)
            and Fr(c130) == lam * Fr(-32, 27 * 81)
            and Fr(c220) == lam * Fr(8, 3 * 81)
            and Fr(c310) == lam * Fr(-8, 3 * 81)
            and Fr(c400) == lam * (Fr(1, 2) + Fr(1, 81))
            and lam > 0)
    rep("R13c L4 = c002 u^2 + c021 (m-(3/2)s)^2 u + lam[(1/2)s^4 + "
        "(1/81)(s-(2/3)m)^4]", okL4)
    charts = {}
    ok_coll = True
    for kind in ('S', 'M+', 'M-', 'U'):
    ...
    # polynomials have total degree <= ~470, so a nonzero difference
    # survives 4 uniform points with probability <= (470/q)^4 ~ 1e-70.
    ok_id = True
    for _ in range(4):
    ...
    # ---------------- R14: coverage ----------------
    okcov = (Fr(7, 4) < Fr(15, 8) < Fr(127, 64)
             and Fr(11, 16) < Fr(23, 32) < Fr(3, 4) < Fr(25, 32)
             < Fr(13, 16) < 1
             and Fr(23, 32) == Fr(3, 4) - Fr(1, 32)
             and Fr(25, 32) == Fr(3, 4) + Fr(1, 32)
             and Fr(127, 64) == 2 - Fr(1, 64)
             and Fr(1, 8) ** 2 == Fr(1, 64))
    rep("R14 coverage arithmetic (t and th unions, window bounds, "
        "sqrt(1/64) = 1/8)", okcov)
    log("R14 union: t<15/8: C1 (+th=1 face);  t>=15/8: th<23/32: C2;")
    log("    th in [11/16,13/16]: C4 (sg>=1/64) | W3a (t<=127/64) |")
    log("    W3b/W3c (|th-3/4|>=1/32) | W3d charts (window, s>0);")
    log("    th>13/16: C3 (+th=1 face)  ==> core > 0 on all of Omega")
    ...
    psi = Yv ** 5
    for k in range(1, 6):
        psi += (sp.Rational(2 ** k) * Etld[k].subs({p: pv, D: Dv, t: 1})
                * Yv ** (5 - k))
    nreal = sp.Poly(psi, Yv).count_roots()
    danch = sum(Fr(cc) * Fr(1, 2) ** (k >> 16) * Fr(1, 10) ** ((k >> 8) & 255)
                for k, cc in DISC.items())    # t = 1: t-powers all 1
    rep("R15 anchor (p,D,t)=(1/2,1/10,1): 5 distinct real roots (Sturm), "
        "DISC > 0", nreal == 5 and danch > 0)

    print("=" * 78)
    print(f"OVERALL -> {'PASS' if PASS else 'FAIL'}")
    print("disc(psi) > 0 on Omega = (0,1)x(0,1]x(0,2) in (sg,th,t); anchor")
    print("has 5 distinct real roots; Omega connected => the A=3 replica")
    print("spectrum is REAL and SIMPLE for all 0<p<2/3, 0<D<=Dbar(p),")
    print("0<s<pi.  With the committed s=pi lemma: all angles in (0,pi].")
    return PASS
