...
def predict_archive_size(N, H_M, delta_inf, p, K, delta_conf):
    ...
    ac_slack_term = m * p
    concentration = math.sqrt(2 * N * math.log(2 / delta_conf)) * p

    total_central = predictor_term + sync_term + ac_slack_term
    total_with_concentration = total_central + concentration
    ...
    return {
        ...
        'total_with_concentration': total_with_concentration,
        'm': m,
    }
...
def main() -> int:
    ...
    # 1-sigma concentration: P(|deviation| > 1*sigma) ≈ 0.317 for Gaussian
    # Use Gaussian probability (CLT limit) rather than Azuma tail bound
    ...
    sigma_1_kb = result_1sigma['concentration'] / 8 / 1024
    print(f"  1-sigma concentration: paper claims ~244 kB, computed "
          f"{bits_to_human(result_1sigma['concentration'])}")
    if abs(sigma_1_kb - 244.0) > 30.0:
        print(f"    FAIL: outside 214-274 kB range")
        all_ok = False

    # delta=10^-6 concentration
    sigma_6_kb = result['concentration'] / 8 / 1024
    print(f"  delta=10^-6 concentration: paper claims ~665 kB, computed "
          f"{bits_to_human(result['concentration'])}")
    if abs(sigma_6_kb - 665.0) > 30.0:
        print(f"    FAIL: outside 635-695 kB range")
        all_ok = False
