#!/usr/bin/env python3
r"""
thm_7_34_route_b_a4_assembly.py
============================================================================
BUG-009-D / Route B, A=4: ASSEMBLY of the mid-band certificate chain into
the sharp replica spectral inequality on the Gray region.

THEOREM (A = 4; continuum-certified on the WHOLE Gray family).  For all
    p in (0, 3/4),   0 < D <= D_c(p)   (the ternary Gray threshold),
    s in (0, pi],
the Route B replica ratio satisfies
    R_4(s; D) = (1 - g_4(s)) / (D(1-D)(1 - cos s))  >  3/2,
where g_4 = Cr * rho(Q) with Q the 5x5 pattern-quotient replica matrix.
...
+ O(p^2) = 2p/3 + O(p^2); every certificate margin necessarily vanishes
linearly there.)
...
this script re-verifies the LOGIC and the DOMAIN INCLUSIONS exactly, and
  ...
  [L1] lemma_7_34_a4_chi_certificates.py        (certs C1..C4)
       chi'(Lam), chi''(Lam), chi'''(Lam), chi''''(Lam) > 0 on the FULL
       superdomain {sg in (0,1), th in (0,1], t in (0,2]},
       Lam = L/Cr, L = 1 - (3/2) D(1-D) t.
  [L2] lemma_7_34_a4_cert5_on_gray.py           (checks A and B)
       chi(Lam) > 0 for sg in (0,1), th <= thetatilde4(sg) = 1 - sg^2/2
       (ONE curve, all p);  and  D_c(p) < thetatilde4(sg) * Dbar(p).
  [L3] lemma_7_34_a4_negside_certificates.py   (N0..N4)
       psi^(k)(Lam) > 0, k = 0..4, psi(x) = -chi(-x), FULL superdomain.
  [L4] lemma_7_34_a4_realness_pi.py
       spectrum real at s = pi on the CURVE domain th <= thetatilde4
       (full-strip realness is FALSE at A=4: exact island witness).
  [L5] lemma_7_34_a4_realness_interior.py
...
CHECKS:
  ...
  A7  END-TO-END numeric validation (mpmath dps 30; consistency guard,
      ...
      R_4(s; D) > 3/2 at every point; worst margin printed.  Plus 60
...
def A0():
    ...
    deps = ["lemma_7_34_a4_chi_certificates.py",
    ...
    for d in deps:
        ...
        if not present:
            print(f"     MISSING: {d}")
    return rep("A0 all A=4 lemma scripts present", ok)
...
def A5():
    ...
    spec = importlib.util.spec_from_file_location(
        "cert5gray", os.path.join(HERE, "lemma_7_34_a4_cert5_on_gray.py"))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    N4, Dn4 = mod.disc_pieces(4)
    p, D = mod.p, mod.D
    Nred = N4
    psub = sp.Rational(3, 4) * (1 - sg ** 2)
    Dbar = sp.Rational(3, 4) * (1 - sg) ** 2 / (1 + sg ** 2)
    curve = (1 - sg ** 2 / 2) * Dbar
    comp = sp.together(Nred.subs({p: psub, D: curve}))
    n_, d_ = sp.fraction(sp.cancel(comp))
    pref = sg ** 13 * (1 - sg) ** 13 * (1 + sg) ** 2
    quo = sp.cancel(n_ / pref)
    R28 = sp.Poly(quo, sg)
    ok = (sp.expand(n_ - pref * R28.as_expr()) == 0
          and R28.count_roots(0, 1) == 0
          and R28.eval(sp.Rational(1, 2)) < 0)
    return rep("A5 disc-num(curve) = sg^13 (1-sg)^13 (1+sg)^2 R28, "
               "R28 root-free & negative", ok)
...
def A7():
    ...
    pmin = 1e-4
    for i in range(50):
        pv = pmin + (3.0 / 4.0 - 1e-6 - pmin) * random.random()
        dc = Dc_of(pv)
        for fD in (0.3, 0.8, 0.999):
            ...
            for sv in (0.3, 1.2, 2.2, math.pi):
                ...
                ok &= (r > 1.5)
                npts += 1
    rep(f"A7a R_4 > 3/2 at {npts} random points in the claimed region", ok)
    print(f"     worst margin over region: R - 3/2 = {mp.nstr(worst - 1.5, 6)}")
    ok2 = True
    for i in range(20):
        pv = pmin + (3.0 / 4.0 - 1e-6 - pmin) * (i + 0.5) / 20
        dc = Dc_of(pv)
        r = R4(pv, float(dc), mp.pi)
        ok2 &= (r > 1.5)
    return rep("A7b binding corner D = D_c, s = pi: R_4 > 3/2 at 20 points",
               ok2)


if __name__ == "__main__":
    print("=" * 78)
    print("ASSEMBLY THEOREM (A=4): R_4(s;D) > 3/2 on the WHOLE Gray region,")
    print("p in (0, 3/4), all angles s in (0,pi] -- wiring of the six")
    ...
    print("g_4 < L, R_4 > 3/2.  Scope: CONTINUUM for all p in (0, 3/4);")
    print("realness consumed only below the ONE curve thetatilde4.")
