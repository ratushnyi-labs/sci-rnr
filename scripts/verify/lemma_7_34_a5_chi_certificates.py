#!/usr/bin/env python3
r"""
lemma_7_34_a5_chi_certificates.py
============================================================================
BUG-009-D / Route B mid-band program, A=5 REPLICATION: the four positive-side
charpoly-derivative sign certificates
  ...
  C4: chi'(Lambda)       = 5 Lambda^4 - 4 e1 Lambda^3 + 3 e2 Lambda^2
                           - 2 e3 Lambda + e4                         > 0
CERTIFIED for A=5 on the ENTIRE rationalized Gray superdomain -- exact
...
R_5(s) >= 3/2 are unchanged from A=3/A=4.  Realness of chi's coefficients
on |w| = 1 is mechanical here: the grading gate verifies E_k/(-w)^k is
w-symmetric, hence real on the circle.)

DOMAIN (A=5 rationalization; same key move as A=3/A=4 -- rationalize the
Gray region, don't box it; lemma_7_34_dbar_upper_bound conventions):
    q = pA/(A-1),  sigma = sqrt(1-q)   =>   p = (4/5)(1-sigma^2),
    Dbar = (4/5)(1-sigma)^2/(1+sigma^2)      -- EXACTLY RATIONAL,
    D = theta * Dbar,  theta in [0,1].
The (sigma,theta,t) box [0,1] x [0,1] x [0,2] covers ALL p in (0,4/5), ALL
D in (0, Dbar] (a superset of Gray: D_c < Dbar, slack 4%..121% over p --
A=5 scout S0c), ALL spectral angles s in (0,pi].  Cleared denominators are
const*(1+sigma^2)^k -- manifestly positive.

METHOD -- t-domain small-piece assembly (the committed A=3/A=4 path,
lemma_7_34_midband_chi_certificates.py / lemma_7_34_a4_chi_certificates.py):
  M = c Q is POLYNOMIAL in (p,D,w), c = p(1-p) G Gt,
  G = D^2 w + (D-4)(1-D), Gt = D^2 + (D-4)(1-D) w;  Berkowitz charpoly
...
EXACT MULTIPLIER GATE (P3a-P3g; the A=4 upgrade, kept verbatim):
    chat = p(p-1) B,   CrN = -B,   CrD = -(5D-4)^2,   with
    B(D,t) = (5D-4)^2 + 2 t D^2 (1-D)(4-D)        [G Gt = w B on |w|=1]
  =>  chat CrN = p(1-p) B^2  EXACTLY (polynomial identities, gates
  P3a-P3d,P3g), and B > 0 on the claimed domain {sigma in (0,1)} because
    4 - 5D = 4[(1-theta)(1+sigma^2) + 2 theta sigma]/(1+sigma^2) > 0
  ...
  sign flip is tolerated.  A value chain additionally checks
  target == (1+sg^2)^m (chat CrN)^{5-j} chi^(j)(Lambda) numerically to
  < 1e-20 relative at 8 points per target.

RESULT: all four certify AT THE ROOT BOX (no subdivision), all face towers
close (numbers from the recorded run of this script):
  C1 (j=4): target 367 terms,   strip (1-sg)^1, core 360,   min ~ 3.83
  C2 (j=3): target 2268 terms,  strip (1-sg)^2, core 2146,  min ~ 1.51e-2
  C3 (j=2): target 6784 terms,  strip (1-sg)^3, core 6496,  min ~ 3.40e-5
  C4 (j=1): target 14994 terms, strip (1-sg)^4, core 14206, min ~ 3.90e-8
Term counts for j <= 3 coincide EXACTLY with the A=3 AND A=4 assembly
counts (2268 / 6784 / 14994) -- the assembly's monomial support is
A-independent there; only the E1-driven j=4 target differs in support
(367, as at A=4).  The root-box minima are HEALTHIER than at A=4 by
1-2 orders (C4: 3.9e-8 vs 2.6e-9; C3: 3.4e-5 vs 4.5e-6; C2: 1.5e-2 vs
3.9e-3): on the correct domain the positivity is Bernstein-visible
without any subdivision, exactly as at A=3/A=4 -- the entire difficulty
was the domain, not the polynomial.

WHAT IS NOT CLAIMED (honest scope, mirroring the corrected A=3/A=4
headers):
  * cert5 = chi(Lambda) > 0 on the FULL superdomain is FALSE at A=5 as at
    A=3/A=4: the root-box Bernstein of the j=0 target has min ~ -3.8e24
    (scout run, 26210-term target) and the real-Perron pocket has an A=5
    analogue: theta_x(sigma) exists for sigma >~ 0.3 at t=2 (scout 3
    geography; theta_x from 0.965 at sg=0.3 down to 0.407 at sg=0.995),
    persisting to p -> 0 exactly as at A=3/A=4.  cert5-on-Gray needs a
    restricted separating curve; at A=5 a SINGLE quadratic curve is DEAD
    (the a-corridor intersection over sg is EMPTY: a must exceed 0.6467
    at sg=0.4 yet stay below 0.6036 at sg=0.995 -- scout G4) and the
    companion script lemma_7_34_a5_cert5_on_gray.py uses the TWO-PIECE
    splice curveM = 1-(3/4)sg^2 on (0,3/4] + curveP = (9-5sg^2)/10 on
    [7/10,1), exactly the A=3 on-gray + smallp splice pattern.
  * realness of the Q spectrum on the FULL superdomain is FALSE at A=5 --
    a genuinely new phenomenon vs A=3/A=4: the complexification island
    (W < 0 at w = -1, disc(f4) odd part) reaches s = pi INSIDE the strip,
    W < 0 for sg in ~[0.58,0.77] with bottom theta ~ 0.7275 at sg = 0.64,
    t = 2 (scouts 2b/4b; at A=4 the collision surface stayed 1.005 x Dbar
    ABOVE the superdomain).  The island clears curveM by ~+0.024 and
    curveP by ~+0.028, so a CURVE-RESTRICTED realness certificate remains
    possible -- but it is a separate deliverable, and the Budan-Fourier
    consequence (no real eigenvalue >= Lambda below the curves) is
    conditional on curve-restricted realness for interior angles.

CHECKS (default run, ~4 min; exact rational arithmetic unless marked):
...
# A=5 structural pieces (eta = D(1-w)(1-D)/G exactly; G Gt = w B on |w|=1)
...
# rationalized Gray superdomain (A=5):  p = (4/5)(1-sg^2),  D = th*Dbar
...
def build_pieces(repfn, prefix):
    ...
    ok &= repfn(f"{prefix} P3c EXACT: CrD == -(5D-4)^2",
    ...
    ok &= repfn(f"{prefix} P3e EXACT: 4-5D == 4[(1-th)(1+sg^2)+2 th sg]/(1+sg^2)",
                sp.cancel(sp.together(
                    4 - 5 * DSUB
                    - 4 * ((1 - th) * (1 + sg ** 2) + 2 * th * sg) / (1 + sg ** 2))) == 0)
    PB = dom_piece(BDT)
    if PB is None:
...
def run_target(j, PC, neg, name, repfn, seed, engine_check=False):
    ...
    for _ in range(90):
        ...
        if Dfr <= Fraction(1, 10 ** 9) or Dfr >= Fraction(4, 5):
            continue
...
def main():
    t00 = time.time()
    print("=" * 78)
    print("A=5 mid-band chi-derivative certificates C1..C4: chi'''', chi''', chi'',")
    print("chi' > 0 at Lambda = L/Cr on the full rationalized Gray superdomain:")
    print("sigma in [0,1] (all p < 4/5), theta in [0,1] (all D <= Dbar), t in [0,2]")
    ...
    print("A=5 superdomain (root boxes + closed faces theta=1, t=2).  NOT")
    print("claimed: chi(Lambda) > 0 full-domain (FALSE at A=5 as at A=3/A=4 --")
    print("real-Perron pocket, scout j=0 Bernstein min ~ -3.8e24); realness of")
    print("the spectrum on the full superdomain (FALSE at A=5: the s=pi")
    print("complexification island is INSIDE the strip, W < 0 for sg ~")
    print("[0.58,0.77], bottom theta ~ 0.7275 -- realness must be")
    print("curve-restricted).  Remaining for R_5(s) >= 3/2 on Gray:")
    print("cert5-on-Gray (TWO-PIECE splice curveM = 1-(3/4)sg^2 +")
    print("curveP = (9-5sg^2)/10, companion script), negative-side")
    print("certificates (companion script), curve-restricted interior-angle")
    print("realness.")
