...
\emph{Empirical validation plan for the framework paper, v0.8}

Pavlo Ratushnyi --- May 2026

\textbf{1. Purpose and scope}

This document specifies the experimental program required to convert
the theoretical claims of RNR Coding (current version v0.8) into
empirically verified statements. The main paper is theory-only by design: every quantitative
prediction in its §11 is labeled as a conjecture, every theorem is
conditional on stated assumptions, and Remark 5.6 invokes the manifold
...
The document covers four classes of empirical work: (a) coding-rate
...
specific natural data classes satisfy the conditions of Theorem 5.5
and Remark 5.6; (c) cross-platform reproducibility tests that
verify Theorems 10.1 and 10.2 hold for a specific reference
implementation; and (d) §7-extension validation (Hypotheses H15--H18)
covering the Chinchilla enwik9 prediction, fail-safe store-mode bound,
Zipf cache hit rate, and per-sub-block MAC tamper detection.
...
corpora, an RNR Type-I coder with the Code12 $E_{12}$-A mapping and the
shared 8--16 MB amortized byte-level transformer predictor (§4.3)
achieves coding
...
$o(N)$ for any of the data classes, then Theorem 5.5's separation
...
\textbf{2.3 Manifold-hypothesis quantification (Remark 5.6)}
...
quantification in Remark 5.6 does not apply to that class, and
...
the conditions of Theorem 10.1 (integer arithmetic, deterministic
non-linearity tables, fixed reduction order) produces byte-identical
archive outputs across the six test platforms enumerated in §8.2
(x86-64 Linux without AVX-512, x86-64 Linux with AVX-512, ARM64 Linux
on Apple Silicon, ARM64 macOS, RISC-V via QEMU, Windows x86-64 with
MSVC runtime), on a 100-block test corpus.
...
Definition 10.1) produces byte-identical archives across the same six
...
path, and below 1 millisecond on NPU path, for a separately-trained
small distilled byte-level transformer predictor (10 to 50 million
int8 parameters, used here in place of the shared §4.3 predictor to
exercise the lowest-latency NPU regime).
...
\textbf{2.11 Chinchilla-on-enwik9 end-to-end synthesis (Theorem 7.15)}

\textbf{H15 {[}quantifying{]}:} On enwik9 (first $10^9$ bytes of
English Wikipedia, standard Hutter Prize corpus), RNR with a
Chinchilla-70B-class transformer predictor produces a bitstream
archive of $79 \pm 0.04\,\mathrm{MB}$ (CLT-based concentration at
$\delta = 10^{-6}$ per T7.15's worked example, now rigorously backed
by the Berry--Esseen two-sided bound of T7.28 and the variance-aware
Bernstein one-sided bound of T7.25; $\approx 12.0\times$
\emph{bitstream-rate} compression, exclusive of the predictor
itself), with byte-granular random-access query cost
$\leq 50\,\mathrm{ms}$ per query on a single B100-class GPU. This
matches the published measurement of Delétang et al.\ (2024) Table 1.

\emph{Falsification:} (a) bitstream exceeds 80 MB at $\delta = 10^{-6}$
CLT-band falsifies the end-to-end synthesis composition of Theorems
7.2--7.5--7.14 \emph{at the CLT estimate}; the rigorous Azuma
worst-case bound (1.4 GB at $\delta = 10^{-6}$ for $W_N = 2048$
transformer) would not be falsified by any bitstream below
$\sim 1.5\,\mathrm{GB}$. (b) Self-contained (model + bitstream)
archive size at single-document scale not matching $\approx 140\,\mathrm{GB}$
expansion falsifies the side-information framing caveat. (c) Per-query
random-access latency exceeding $200\,\mathrm{ms}$ at $K = \Theta(\sqrt{N})$
falsifies the random-access cost bound $O(\sqrt{N} \cdot \mathrm{cost}(M'))$.

\textbf{2.12 Fail-safe store-mode upper bound (Theorem 7.22)}

\textbf{H16 {[}confirming{]}:} On adversarial inputs (random bytes,
already-compressed data, encrypted data), an RNR encoder with per-
sub-block store-mode fallback produces an archive of size
$\leq N \cdot \log_2 |\Sigma| + m$ bits ($\leq N + N/K$ bytes),
\emph{never exceeding} the raw source size by more than mode-flag
overhead.

\emph{Falsification:} any source/predictor combination for which the
encoder produces output strictly larger than $N \cdot \log_2|\Sigma|
+ m \cdot (1 + \epsilon)$ for negligible $\epsilon$ falsifies the
fail-safe guarantee. (The store-mode fallback bound is a hard worst-
case guarantee proven in T7.22; failure would indicate a bug in
encoder mode-selection rather than a theory gap.)

\textbf{2.13 Cache-aware decoder hit rate under Zipf workloads
(Theorem 7.20)}

\textbf{H17 {[}quantifying{]}:} For an LRU decoder cache of size $C$
sub-blocks against a Zipf-distributed query workload with parameter
$\alpha$ over $m$ sub-blocks, the empirical hit rate $\rho_C$ matches
the static-top theoretical formula:
$\rho_C \approx (C/m)^{1-\alpha}$ for divergent-normalizer $\alpha < 1$;
$\rho_C \to 1 - C^{1-\alpha}/((\alpha-1)\zeta(\alpha))$ for
convergent-normalizer $\alpha > 1$. Per-query amortized latency
$(1-\rho_C) \cdot K \cdot \mathrm{cost}(M')$ matches the predicted
amortization.

\emph{Falsification:} empirical hit rate deviation from the static-top
formula by more than $20\%$ at $C = 100, m = 10^6$ for $\alpha \in
\{0.7, 0.8, 1.1, 1.5\}$ on a sample of $Q \geq 10^4$ queries falsifies
the cache-amortization analysis.

\textbf{2.14 Per-sub-block MAC integrity (Theorem 7.23)}

\textbf{H18 {[}confirming{]}:} An RNR archive built with per-sub-block
HMAC-SHA256 (or Poly1305-AES, or AES-GCM tag, or BLAKE3-keyed) detects
every single-bit tampering of the encoded payload with detection
probability $\geq 1 - 2^{-128}$ per sub-block on uniformly random
tampering, and total archive size overhead is $m \cdot \tau$ bits.

\emph{Falsification:} any single-bit tampering of an encoded sub-block
that produces an output accepted as valid by the decoder falsifies
the MAC-based integrity guarantee. (Per Bellare--Namprempre 2000
ASIACRYPT LNCS 1976 PRF security, failure would indicate either a
broken MAC implementation or compromised key material.)
...
quantities are estimated to test the conditions of Theorem 5.5 and
Remark 5.6:
...
With 14 hypotheses and multiple coder/corpus pairs each, the
...
Hypothesis:           <H1..H14 from Section 2>
...
Remark 5.6 connects Theorem 5.5\textquotesingle s separation to
...
admit a manifold description; in that case Remark 5.6's
...
\item
  ...
  5.6\textquotesingle s predicted regime. The
...
replication. Whatever the outcome, the framework's theoretical content (the 25
formal results in v0.8 --- 24 theorems and 1 lemma, plus 8 definitions
and 4 remarks) is unchanged by experimental
outcomes; the empirical paper says only how well the framework's
