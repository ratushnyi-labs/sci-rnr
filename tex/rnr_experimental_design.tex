...
correlation TC($D_{N}$) and intrinsic dimension d) that test whether
...
measured total correlation TC($D_{N}$) is $\Theta(N)$, i.e., remains at least
10\% of N as N grows over the range 1 KiB to 16 KiB.

\emph{Falsification:} if measured TC($D_{N}$) decays as o(N) for any of the
...
Remark 5.7 does not apply to that class, and TC($D_{N}$) is not predicted
...
an RNR Type-III-A scheduler with calibrated $U_{m}$(t) achieves rate within
5\% of the per-tile oracle best mode. The per-mode prediction error
$\varepsilon_{t}$ averages below 0.1 bits per position.
...
predictor for U is undercalibrated and the 2*$\varepsilon$ bound is not approached
in practice.
...
\emph{Falsification:} if the measured rate exceeds $H(X \mid Y)$ +
...
\begin{itemize}
\item
  Total correlation TC($D_{N}$) via the Cmix entropy estimator at multiple
...
\item
  Per-mode prediction error $\varepsilon_{t}$ for Type-III-A, computed as the
  empirical maximum over modes of \textbar $U_{m}$(t) - $L_{m}$(t)\textbar{}
...
5. Run decoder on each A on each platform; emit $\hat{X}$.

6. Verify $\hat{X}$ == X on every (encoder\_platform, decoder\_platform)
pair.
\end{quote}
...
2004) with k nearest neighbors. For each sample point $x_{i}$ and each of
its k nearest neighbors x\_\{i,1\}, ..., x\_\{i,k\} (by Euclidean
distance in the embedding space):

\begin{quote}
$T_{k}$($x_{i}$) = distance to k-th nearest neighbor

d\_hat($x_{i}$, k) = (k - 1) / sum\_\{j=1..k-1\} log($T_{k}$($x_{i}$) /
$T_{j}$($x_{i}$))

d\_hat(k) = mean over all sample points of d\_hat($x_{i}$, k)
\end{quote}
...
\begin{itemize}
\item
  Estimate TC($D_{N}$) and intrinsic dimension d for each corpus (H3, H4).
\item
