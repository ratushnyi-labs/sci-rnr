...
\textbf{Theorems 7.2--7.24 --- RNR with byte-granular random access}

The §7.2--7.24 expansion delivers a 23-theorem framework bridging the
abstract RNR formalism (§§3--6) to a deployable neural-lossless-
compression pipeline. Headline results, organized by theme:
...
\[
L^{\mathrm{rep}}_{\mathrm{RNR}} \approx 79\,\mathrm{MB} \pm 0.7\,\mathrm{MB}
\quad (\text{12.0$\times$ bitstream-rate compression}).
\]
This matches Delétang et al.\ Table 1 measurement exactly.
...
minimal honest production SLO tail bound; LRU+IRM finite-batch
concentration is left as an open problem (§13.2).
\end{quote}
