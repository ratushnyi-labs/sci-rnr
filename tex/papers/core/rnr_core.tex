...
dual of the source R\'enyi spectrum (Theorem 7.31) --- and an
extreme-value law pinning the worst-case per-query read buffer to
$Kh+\sqrt{2KV\ln(N/K)}$ (Theorem 7.35), and a functional CLT fixing the
streaming-encoder write-buffer high-water-mark at the varentropy scale
$\sqrt{NV}$ (Theorem 7.36); it then
...
ratio excludes the $\approx$140\,GB model and turns net-positive
once the amortized archive volume exceeds the break-even
$V^*\!\approx\!152$\,GB).
...
$k$, predictor cost $\mathrm{cost}(M)$, and $\log L(R)$ are treated
as fixed (or polylogarithmic in $|X|$) parameters. Here
$\mathrm{cost}(M)$ is one per-position neural-predictor evaluation: it
is non-universal (architecture- and context-dependent, e.g.\
$\Theta(W)$ for a windowed transformer of context $W$; Lemma 5.1b)
and it is the dominant term of the per-query cost in any realistic
regime, so ``polylogarithmic in $|X|$'' bounds the dependence on
archive size with $\mathrm{cost}(M)$ held fixed, not the per-query
cost in absolute (wall-clock) terms. Under an additional probability-floor assumption
...
under an additional uniform-warm-rate assumption (the rung-2 and
exact-Markov rung-3 ceilings are consolidated in Corollary 10.2a).
We conjecture (heuristically, not as a theorem; this rests on the
warm-rate assumption of Corollary 10.2a, not the bare floor)
that the actual overhead is on the order of one percent for natural
data at typical parameters, but neither a tight upper bound in
general nor a measured value on a specific corpus is provided here;
this conjecture is the falsifiable subject of Hypothesis H10(c) in
the experimental-design companion (cross-linked from §11 of this paper),
whose pre-registered
test (sync-point ratio overhead below 2 percent at $K = 64$\,KiB)
would confirm or refute it. The
...
is an open empirical question (§11).
...
\item
  ...
  field of Theorem 7.15 --- a nine-part arc (Remarks 7.15a--7.15i). The
  ...
  restriction --- exact finite-block hidden-Markov Shannon entropy --- is now
  \emph{resolved}: poly-time-$\varepsilon$-approximable under filter stability
  (Remark 7.15c) and exactly polynomial for finite finite-prefix belief (Remark
  7.15f, strictly beyond finite-order Markov); every integer-order R\'enyi
  entropy exactly polynomial via a product-chain agreement identity, with the
  min-entropy NP-hard (HMM consensus) and the collision entropy $H_2$ a free
  poly lower bound (Remark 7.15d); yet the \emph{exact} value (the coefficient
  query) is \#P-hard --- $\mathrm{FP}^{\#P}$-complete --- by a \#SAT reduction
  (Remark 7.15g) that sidesteps the \emph{both} natural attacks 7.15e flagged
  (the read-once / $\#\mathrm L\subseteq\mathrm{FP}$ barrier, evaded through the
  entropy's group-by-output-and-$\log$ structure; the non-collapsing Blackwell
  measure). Thus the open middle is pinned --- exact \#P-hard yet
  $\varepsilon$-approximable under filter stability --- and the
  support-selection complexity is \emph{resolved for the operative high-order
  predictor} and now also for the bounded-order residual.
\item
...
§§7.14/7.25/7.28/7.29/7.33/7.31 from the sub-Gaussian body through the
central-limit and moderate-deviation scales to the exponential-rate tail, with
§7.35 the extreme-value (worst-case per-query read-buffer) axis and §7.36 a
functional CLT for the repair process (the streaming-encoder write-buffer
high-water-mark)); and §7.34 the operational \emph{rate-distortion} (lossy)
dispersion for a discrete memory source --- the binary symmetric Markov chain
attains $V_{\mathrm{op}}=V_{\mathrm{lossless}}$ throughout its Gray region
$0<D\le D_c(p)$ (the first discrete memory source with a proven operational
RD-dispersion), consolidated for the \emph{asymmetric} chain in
Theorem 7.34i and lifted to \emph{every finite alphabet} in Theorem 7.34m,
where the A-ary symmetric chain attains
$V_{\mathrm{op}}=V_{\mathrm{lossless}}^{(A)}=p(1-p)\log_2^2\tfrac{(1-p)(A-1)}p$
on its Gray region $0<D\le D_c^{(A)}(p)$. Here the \emph{converse}
$V_{\mathrm{op}}\ge V_{\mathrm{lossless}}$ is unconditional and fully general
(all source types, all balanced / group-difference distortions on each
distortion's own Gray region; the source-agnostic principle of Remark 7.34d,
extended in Remarks 7.34n/q), while the matching \emph{achievability} is fully
proven for the binary symmetric chain on its whole Gray region (Remark 7.34b)
and, for every finite alphabet, holds at leading order in the distortion $D$ and
all-$D$ on the memory bulk via the A-ary replica spectral inequality of
Lemma 7.34l --- whose $S_A$ pattern-quotient makes it an alphabet-independent
$\le5$-dimensional problem --- with a single 2nd-order residual (a rigorous
Chebyshev-coefficient $a_k$ bound) and the beyond-Gray ($D>D_c$) exact value
left open (the residual ledger in Remark 7.34m$'''$; the sharp replica
constant $\tfrac32$ is proven at $A=2$ and regionally certified for $A\ge3$,
Remarks 7.34m$''''$/7.34m$'''''$). Section 8 states the
...
$\approx 79\,\mathrm{MB}$ / $12.0\times$ figure of Theorem 7.15 is a
...
7.21, or the per-regime discussion above).
...
exception: it derives from Theorem 7.15 and is verified against a cited
...
\item
  ...
  problem combinatorially non-trivial. An early multi-LLM
  brainstorming session proposed two candidate structural obstacles ---
...
\textbf{{[}134{]}} Ryabko, B. Ya. (1984). Twice-Universal Coding.
Problems of Information Transmission, 20(3), 173-177.

\textbf{{[}135{]}} Willems, F. M. J., Shtarkov, Y. M., and Tjalkens,
