...
Sections 8--9, the practical core §§10.1--10.5, and Sections 11--13.
...
are distinct objects that share a number by this convention. Likewise,
Definitions 10.2/10.3, Theorems 10.3--10.6 and Remark 10.3a are blocks of
[RNR-II] (§§10.7--10.8), restated statement-only in the Prerequisites
below, and are distinct from this Part's own §10.3--§10.4 subsections.}
...
\textbf{P. Prerequisites (restated from Part II)}

This Part is self-contained except for the random-access architecture
blocks of §§10.7--10.8, which are stated and proved in [RNR-II] and
restated here statement-only, and two complexity remarks of the §7.15
arc. \emph{Caution on numbering:} this Part's own §10.3 and §10.4 are
\emph{sections}; the blocks below carry their original [RNR-II] numbers
per the series convention.

\emph{Definition 10.2 of [RNR-II] ($W$-bounded predictor; statement
only).} A predictor $M$ is $W$-bounded if its output $Y_i = M(C_i)$ at
position $i$ depends only on the previous $W$ bytes of public context,
$Y_i = M(X_{i-W}, \ldots, X_{i-1})$, when the context is built
incrementally from decoded sub-blocks; $W$ is a structural property of
$M$'s architecture, not an inference-time parameter. Fixed-window
transformers, order-$W$ $n$-gram and finite-state predictors, and
bounded receptive-field feedforward networks are $W$-bounded;
RNN/LSTM/state-space models are not in general --- Theorem 10.4 of
[RNR-II] covers those.

\emph{Definition 10.3 of [RNR-II] (sync point; statement only).} A sync
point at source position $P$ (a)~resets the predictor context to a
known initial state independent of bytes $[0, P)$, and (b)~flushes the
entropy coder at the encoder and resets it at the decoder, so decoding
can begin at the sync point's bit offset in the repair stream $R$ with
no prior bits of $R$. Under the default uniform spacing, sync points
sit at byte positions $jK$ (offsets implicit) and a seek index of at
most $\lceil |X|/K \rceil \cdot \lceil \log_2 L(R) \rceil$ bits maps
sync identifiers to bit offsets; [RNR-II] also defines a non-uniform
variant with explicitly stored offsets.

\emph{Theorem 10.3 of [RNR-II] (byte-granular random access; statement
only).} For any per-position sequential RNR coding mode (Type-I, the
per-byte sequential Type-III-B strategy (iii) of §6.3, or Type-III-A
composition with sync points aligned to tile boundaries --- whole-block
Type-II partition-rank coding is excluded and retains only sub-block
granularity, Remark 10.3a of [RNR-II]), with $M$ a $W$-bounded
predictor satisfying Theorem 10.1 of this Part, uniform sync spacing
$K$, and a predictor probability floor $\eta > 0$: reading $k$
consecutive bytes from any position costs one seek-index probe of
$O(\log L(R))$ bits plus at most $K + k$ decoded positions (of which at
most $K$ are discarded warm-up), i.e.\ $O(\log L(R) + (K + k) \cdot
\mathrm{cost}(M))$ total work.

\emph{Theorem 10.4 of [RNR-II] (non-$W$-bounded predictors; pointer).}
For RNN/LSTM/state-space predictors, a predictor state snapshot stored
at each sync point restores the Theorem 10.3 access pattern at the cost
of the snapshot storage. Theorems 10.5 and 10.6 of [RNR-II] give the
block-device interface and hardware-conformance corollaries.

\emph{Remarks 7.15c and 7.15g of [RNR-II] (pointers).} For a
hidden-state (HMM or neural) predictor, the exact finite-block coded
length is $\varepsilon$-approximable in polynomial time under filter
stability (Remark 7.15c of [RNR-II]) and $\mathrm{FP}^{\#P}$-complete to compute
exactly (Remark 7.15g of [RNR-II]); these are used by Remark 6.6c and in the proof
of Theorem 7.1b.
...
\item
  ...
  $O(K \cdot \mathrm{unrank}(K))$ per sub-block, Remark 10.3a of [RNR-II])
  --- polylogarithmic in archive size $|X|$ once sync-point spacing
  $K$, read window $k$, predictor cost $\mathrm{cost}(M)$, and
  $\log L(R)$ are held fixed (or polylog in $|X|$) --- via Theorem 10.3 of [RNR-II]
  ...
  Theorem 10.4 of [RNR-II] covers non-$W$-bounded predictors via state snapshots.
...
\item
  ...
  corollaries (Theorems 10.5 and 10.6 in §10.8 of [RNR-II]): an OS-level driver
  ...
  additionally $L(R) = \mathrm{poly}(|X|)$ (Theorem 10.5 of [RNR-II]). Note
  ...
  specification of Theorem 10.1 (Theorem 10.6 of [RNR-II]). Specific target
...
\item
  ...
  (Theorems 7.2--7.33, 7.35, 7.36 of [RNR-II]), which bridges the abstract framework
...
per Theorem 10.6 of [RNR-II]); two encoders that produce $U_m(t)$ in
...
(Definition 10.2 of [RNR-II], restated in the Prerequisites) that at each position $i$ outputs the order-$W$
...
sync points at spacing $K$ (Definition 10.3 of [RNR-II]) inserted to enable
...
excess analyzed in §10.7 of [RNR-II], with $S$ symbols per source byte for the
...
admits byte-granular random access via Theorem 10.3 of [RNR-II] at
...
access (Remark 10.3a of [RNR-II]). Lemma 5.1a shows that per-position sequential
...
the polylog advantage) or sub-block restructuring (Remark 10.3a of [RNR-II]).
...
The byte-granular random-access cost from Theorem 10.3 of [RNR-II] becomes
...
$M$ be a $W$-bounded predictor (Definition 10.2 of [RNR-II]) with $W \geq W'$
...
on sub-blocks of size $K$ starting at sync points (Definition 10.3 of [RNR-II]),
...
independence (e.g., share state across sync points via Theorem 10.4 of [RNR-II]
...
per-position sequential stream that admits the byte-granular access of Theorem 10.3 of [RNR-II], i.e.\ that theorem's
...
--- to compute (Remark 7.15g of [RNR-II]), $\varepsilon$-approximable in polynomial time
only under filter stability (Remark 7.15c of [RNR-II]); the bound's \emph{validity} is
...
(Theorem 10.3 of [RNR-II]). In particular the
...
value is $\mathrm{FP}^{\#P}$-complete (Remark 7.15g of [RNR-II]), and only
$\varepsilon$-approximable in poly time under filter stability (Remark
7.15c of [RNR-II]); the achievability above attains the realized scheme, not the exact
optimum). Hence
...
and model overhead'' caveat in Theorem 7.15 of [RNR-II] derives $V^*$; the table
...
\item[(e)] $\mu$, the metadata --- mode-flag stream and fallback
  ...
  index of §10.7 [RNR-II]. By Theorem 8.1 a fallback path is mandatory, so
...
overhead'' caveat of Theorem 7.15 [RNR-II] for the amortization framing. Under
...
with $\alpha \approx 0.34$ (cited and used in Theorem 7.21 of [RNR-II]).
...
\emph{Concentration of measure and martingales.} Theorem 7.14's ([RNR-II])
...
\emph{Cache analysis.} Theorem 7.20's ([RNR-II]) cache-aware decoder uses the
...
(convergent-normalizer $\alpha>1$). Theorem 7.24's ([RNR-II]) IRM hypothesis
...
\emph{Scaling laws for neural predictors.} Theorem 7.15's ([RNR-II]) numerical
...
$12.0\times$ raw compression). Theorem 7.21's ([RNR-II]) predictor-archive size
...
\emph{Authenticated encryption.} Theorem 7.23's ([RNR-II]) per-sub-block MAC
...
\emph{Content-defined chunking.} Theorem 7.17's ([RNR-II]) insertion/deletion
...
\emph{Functional and multi-source coding.} Theorem 7.12 of [RNR-II] (functional
compression) uses the characteristic-graph entropy of Orlitsky and
Roche (2001) \emph{IEEE Trans.\ Inf.\ Theory} 47(3):903; Theorem 7.9 of [RNR-II]
(successive refinement) uses Equitz and Cover (1991) \emph{IEEE
Trans.\ Inf.\ Theory} 37(2):269; Lemma 5.6h (graphical-model total
correlation) uses Lauritzen and Spiegelhalter (1988) \emph{J.\ Royal
Stat.\ Soc.\ B} 50(2):157 for the junction-tree entropy
decomposition.

\emph{Universal online compression.} Theorem 7.6 of [RNR-II] uses
...
\item
  ...
  including entropy-coder flush) and Theorem 10.3 of [RNR-II]. The achievable
  ...
  sub-block (Remark 10.3a of [RNR-II]). Non-$W$-bounded
...
\item
  ...
  hardware-target portability --- resolved by Theorem 10.5 of [RNR-II]
  ...
  $\mathrm{cost}(M)$, $\log L(R)$ parameter regime) and Theorem 10.6 of [RNR-II] (hardware-target
  portability: bit-exact predictor execution is guaranteed across
  any pair of targets each satisfying the (a)--(g) conformance
  conditions of Theorem 10.6 of [RNR-II]; whether specific named target classes
...
\item
  ...
  \emph{architecture}: Theorem 7.2 of [RNR-II] establishes the basic sub-block
  byte-granular RA construction with cost
  $O(\log L(R) + K \cdot \mathrm{cost}(M'))$ per query and total
  $N H_{M'} + m \delta_\infty + O(m \cdot p)$, with Theorem 7.5 of [RNR-II]
  ...
  side-information composition), and the lossy+ECC
  Corollary 7.11 of [RNR-II]. \emph{Multi-source and structural} are theorems:
  ...
  analogue and Corollary 7.27c of [RNR-II] recovering it on the predictor-faithful
...
\item
  ...
  2026).} A brainstorming session during the present manuscript
  ...
  follow-up independent adversarial review (against the 2020--2025
  literature) identified the fix via good
...
\item
  Sharpening the constant $\gamma_{\rm LRU}$ in Theorem 7.26's ([RNR-II])
  ...
  on LRU stack states, would tighten the constant. Theorem 7.26's ([RNR-II])
...
\item
  ...
  \emph{(1) Refutation for Markov-expressible sources (Theorem 7.27$'$ of [RNR-II],
  ...
  class (Corollary 7.27c of [RNR-II], proved).} The $\mathrm{cost}(M)$-bearing
  product $C_{\mathrm{work}} \cdot S \geq (1-\epsilon)(1-o(1)) N \cdot
  \mathrm{cost}(M) \cdot \log N$ holds exactly on the
  \emph{predictor-faithful} class (Definition 7.27b of [RNR-II]: dense predictor
  ...
  7.27e of [RNR-II]).} The refutation in piece (1) was stated for finite-order Markov
  sources. Corollary 7.27e of [RNR-II] computes the Ferragina--Venturini
  ...
  Remark 7.27d of [RNR-II](ii): for \emph{every fixed} stationary source --- including
  ...
  > 0$ (Corollary 7.27e of [RNR-II](b)); at deployment scale on byte data the escape
  is moreover practically closed, since $\rho_1(N) > 1$ bit/symbol for all
  $N \lesssim 2^{188} \approx 10^{57}$ (Corollary 7.27e of [RNR-II](c)). The residual
  genuinely-open
  ...
  Theorem 7.2 of [RNR-II] Shannon-near rate with $o(\sqrt{N})$ query time, or does a
  converse force $\Omega(\sqrt N)$ query there? Corollary 7.27f of [RNR-II] advances
  ...
  growing-horizon sources?} Corollary 7.27e of [RNR-II] removed the false premise
  (``neural $\Rightarrow$ FV closes''); Corollary 7.27f of [RNR-II] delimits the
...
\item
  ...
  adaptive predictors; Theorem 10.6 of [RNR-II] lifts Theorem 10.1 to explicit
  per-target conformance conditions (a)--(g); Theorems 7.16--7.18 of [RNR-II]
  ...
  Theorem 10.6 of [RNR-II], §7 the adaptive-optimizer conformance of Theorem 10.2,
...
\item
  Cache architecture and workload characterisation. Theorems 7.20 of [RNR-II] and
...
\item
  Per-sub-block MAC key management. Theorem 7.23 of [RNR-II] guarantees tamper
  ...
  policy document.
\item
  Parallel and quantised encoder deployment. Theorems 7.16--7.18 of [RNR-II]
  establish sub-block-parallel encoding with linear speedup; the
  coding-efficiency loss from INT8 and mixed-precision predictor
  quantisation is not analysed here. Achieving the theoretical speedups on
...
selection acts inside a sub-block of size $K=\Theta(\sqrt N)$ (Theorem 7.5 of [RNR-II]),
...
7.15a--7.15b of [RNR-II]).} The §6.9 picture above is for the Gaussian (bits-back-limit)
...
Remark 7.15b of [RNR-II] then shows this hardness is \emph{orthogonal} to the deployed
...
the optional non-contiguous selection lever. Remark 7.15c of [RNR-II] makes \emph{partial
...
7.15d of [RNR-II] then maps the complexity \emph{landscape} via the R\'enyi spectrum: every
...
approximation, NP-hard min-entropy above). Remark 7.15e of [RNR-II] then \emph{maps} this wall:
...
predicate-encoding \#P-hardness route (Remark 7.15a of [RNR-II]/V5) is self-defeating for
...
\textbf{{[}9{]}} Casel, K., Fernau, H., Gaspers, S., Gras, B., and
...
\textbf{{[}10{]}} Ga\'nczorz, M. (2018). Entropy bounds for grammar
compression. arXiv:1804.08547 (also presented at DCC 2018/2020).

\textbf{{[}11{]}} Gallager, R. G. (1962). Low-Density Parity-Check Codes. IRE
Transactions on Information Theory, 8(1), 21-28.

\textbf{{[}12{]}} Ko, C.-W., Lee, J., and Queyranne, M. (1995). An Exact Algorithm for
Maximum Entropy Sampling. Operations Research, 43(4), 684-691.

\textbf{{[}13{]}} Kozma, L., and Voderholzer, J. (2024). The
Computational Complexity of Tokenisation. arXiv:2411.08671.

\textbf{{[}14{]}} Je\.z, A. (2015). A really simple approximation of
smallest grammar. Theoretical Computer Science, 616, 141-150.

\textbf{{[}15{]}} Karzanov, A. V. (1998). Minimum 0-Extensions of Graph
Metrics. European Journal of Combinatorics, 19(1), 71-101.

\textbf{{[}16{]}} Nemhauser, G. L., Wolsey, L. A., and Fisher, M. L. (1978). An Analysis
of Approximations for Maximizing Submodular Set Functions --- I.
Mathematical Programming, 14(1), 265-294.

\textbf{{[}17{]}} Ohsaka, N. (2022). Some Inapproximability Results of MAP Inference
and Exponentiated Determinantal Point Processes. Journal of Artificial
Intelligence Research, 73, 709-735.

\textbf{{[}18{]}} Schwartz, R., and Tur, S. (2023). Improved hardness
of approximation for graph problems via Boolean PCPs.
arXiv:2104.11670 (also conference version, ICALP 2023).

\textbf{{[}19{]}} Cover, T. M. (1973). Enumerative Source Encoding. IEEE
Transactions on Information Theory, 19(1), 73-77.

\textbf{{[}20{]}} Cover, T. M., and Thomas, J. A. (2006). Elements of
Information Theory, 2nd edition. Wiley.

\textbf{{[}21{]}} Donoho, D. L. (2000). High-Dimensional Data Analysis:
The Curses and Blessings of Dimensionality. AMS Math Challenges Lecture.

\textbf{{[}22{]}} Duda, J. (2009). Asymmetric Numeral Systems.
arXiv:0902.0271.

\textbf{{[}23{]}} Frey, B. J. (1997). Bayesian Networks for Pattern
Classification, Data Compression, and Channel Coding. Ph.D. thesis,
University of Toronto.

\textbf{{[}24{]}} Frey, B. J. (1998). Graphical Models for Machine
Learning and Digital Communication. MIT Press.

\textbf{{[}25{]}} Hinton, G. E., and van Camp, D. (1993). Keeping Neural
Networks Simple by Minimizing the Description Length of the Weights.
Conference on Learning Theory.

\textbf{{[}26{]}} Hoogeboom, E., Peters, J. W. T., van den Berg, R., and
Welling, M. (2019). Integer Discrete Flows and Lossless Compression.
NeurIPS.

\textbf{{[}27{]}} Huffman, D. A. (1952). A Method for the Construction
of Minimum-Redundancy Codes. Proceedings of the IRE, 40(9), 1098-1101.

\textbf{{[}28{]}} Kingma, F. H., Abbeel, P., and Ho, J. (2019).
Bit-Swap: Recursive Bits-Back Coding for Lossless Compression with
Hierarchical Latent Variables. ICML.

\textbf{{[}29{]}} Knoll, B. (ongoing). Cmix: a lossless data compression
program. https://www.byronknoll.com/cmix.html.

\textbf{{[}30{]}} Levina, E., and Bickel, P. J. (2004). Maximum
Likelihood Estimation of Intrinsic Dimension. NIPS.

\textbf{{[}31{]}} Liveris, A. D., Xiong, Z., and Georghiades, C. N.
...
\textbf{{[}32{]}} Mahoney, M. (ongoing). Data Compression Programs and
the PAQ family. http://mattmahoney.net/dc/.

\textbf{{[}33{]}} Mentzer, F., Agustsson, E., Tschannen, M., Timofte,
R., and Van Gool, L. (2019). Practical Full Resolution Learned Lossless
Image Compression. CVPR.

\textbf{{[}34{]}} Pradhan, S. S., and Ramchandran, K. (2003).
...
\textbf{{[}59{]}} Lund, C., and Yannakakis, M. (1994). On the
Hardness of Approximating Minimization Problems. Journal of the ACM,
41(5), 960-981.

\textbf{{[}60{]}} Lehman, E., and Shelat, A. (2002). Approximation
...
\textbf{{[}61{]}} Berman, P., and Karpinski, M. (1999). On Some
...
\textbf{{[}62{]}} Storer, J. A., and Szymanski, T. G. (1982). Data
Compression via Textual Substitution. Journal of the ACM, 29(4),
928-951.

\textbf{{[}63{]}} Ar{\i}kan, E. (2009). Channel Polarization: A Method
...
\textbf{{[}64{]}} Azuma, K. (1967). Weighted Sums of Certain Dependent
Random Variables. T\^ohoku Mathematical Journal, 19(3), 357-367.

\textbf{{[}65{]}} Bellare, M. (2006). New Proofs for NMAC and HMAC:
...
\textbf{{[}66{]}} Bellare, M., Canetti, R., and Krawczyk, H. (1996).
...
\textbf{{[}67{]}} Bellare, M., and Namprempre, C. (2000). Authenticated
...
\textbf{{[}73{]}} Cybenko, G. (1989). Approximation by Superpositions
of a Sigmoidal Function. Mathematics of Control, Signals, and Systems,
2(4), 303-314.

\textbf{{[}74{]}} Del\'etang, G., Ruoss, A., Duquenne, P.-A., Catt, E.,
...
\textbf{{[}75{]}} Doshi, V., Shah, D., and M\'edard, M. (2007). Source
...
\textbf{{[}76{]}} Doshi, V., Shah, D., M\'edard, M., and Effros, M.
(2010). Functional Compression through Graph Coloring. IEEE
Transactions on Information Theory, 56(8), 3901-3917.

\textbf{{[}77{]}} Equitz, W. H. R., and Cover, T. M. (1991). Successive
Refinement of Information. IEEE Transactions on Information Theory,
37(2), 269-275.

\textbf{{[}78{]}} Fagin, R. (1977). Asymptotic Miss Ratios over Independent
References. Journal of Computer and System Sciences, 14(2), 222-250.

\textbf{{[}79{]}} Ferragut, A., Rodr{\'\i}guez, I., and Paganini, F.
(2018). Optimizing TTL Caches under Heavy-Tailed Demand. Queueing
Systems, 88(3-4), 207-241.

\textbf{{[}80{]}} Grossi, R., and Vitter, J. S. (2005). Compressed
...
\textbf{{[}88{]}} McLeish, D. L. (1974). Dependent Central Limit
Theorems and Invariance Principles. Annals of Probability, 2(4),
620-628.

\textbf{{[}89{]}} Muthitacharoen, A., Chen, B., and Mazi\`eres, D.
...
\textbf{{[}117{]}} Birch, B. J. (1962). Approximations for the Entropy
for Functions of Markov Chains. Annals of Mathematical Statistics,
33(3), 930-938.

\textbf{{[}118{]}} Manurangsi, P. (2017). Almost-Polynomial Ratio
...
\textbf{{[}141{]}} Muirhead, R. J. (1982). Aspects of Multivariate
Statistical Theory. Wiley.

\textbf{{[}142{]}} Diaconis, P., and Freedman, D. (1987).
