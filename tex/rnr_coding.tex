...
all predictor evaluations are deterministic and bit-identical to a
full decoding from position 0, so the output matches the original
$X$ on $[p, p+k')$. Total decoded positions: $p + k' - P \leq K + k$
since $P > p - K$. \hfill$\blacksquare$
...
calls can serve any read request as follows: let $p = \mathcal{L}
