...
$\alpha$ bounded), the log-ratio factor converges to $1$ and
$\gamma^{\mathrm{bit}}_{\mathrm{Char}}(H) \to \gamma_{\mathrm{SGP}}$
from above. Equivalently: bit-cost APX-hardness within the Charikar
class with constant $\gamma_{\mathrm{SGP}} - \varepsilon$ holds for
any $\varepsilon > 0$, for sufficiently large instances.}
...
relative perturbation is $O(1)$ inside the inner $\log_2$ and
$O(1/\log|V|)$ overall. Hence the log-ratio $\to 1$ as
$|V| + |E| \to \infty$.
\hfill$\blacksquare$
