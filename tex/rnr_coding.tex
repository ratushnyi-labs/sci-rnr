...
Then for any sub-block spacing $K = \omega(1)$ and $K = \mathrm{polylog}(N)$,
the RNR framework (per-position sequential coding under a
$W_N$-bounded predictor of context $W_N = O(\log N)$ outputting
true-conditional probabilities clipped to the $\eta$-floor, sync
points at uniform $K$-spacing, and Theorem 10.3 byte-granular access)
achieves expected total repair-stream length}
\[
...
\emph{\textbf{Proof.}} Combine the four building blocks:

(i) Lemma 5.1b: for $W = W_N = O(\log N)$ under exp-mixing (5.8),
$H_{W_N}(D_N) \leq H(D_N) + 1$. The per-position sequential
arithmetic coding under the $W_N$-bounded predictor outputs
expected total
$H_{W_N}(D_N) + \varepsilon_{\mathrm{ac}}^{\mathrm{total}} \leq
H(D_N) + 1 + \varepsilon_{\mathrm{ac}}^{\mathrm{total}}$ (Lemma 5.1a
plus the convergence).

(ii) Lemmas 5.1c-d and Corollary 5.1f: at sub-block size $K = \omega(W)$,
the sync-overhead per sync point is $\delta_W^{(h_W)} \leq \delta_\infty$
(Lemma 5.1d), and the total sync overhead is
$(m-1) \cdot \delta_\infty + O(1) = O((N/K) \cdot \delta_\infty)
= o(N)$ since $K = \omega(W_N) = \omega(\log N)$ and $\delta_\infty$ is
finite.

(iii) Lemma 5.6c: $H(D_N) = N \cdot h(X) + \delta_N$ where
$\delta_N \to \delta_\infty < \infty$ (Crutchfield--Feldman) by the
summability implied by (5.8). Hence $H(D_N) = N \cdot h(X) + O(1)$.

(iv) Theorem 10.3: byte-granular random-access at cost
$O(\log L(R) + (K + k) \cdot \mathrm{cost}(M))$ per query, polylog in
$N$ in the stated parameter regime.

Combining (i)-(iv):
$L_{\mathrm{RNR}}(X^N) \leq H(D_N) + 1 + \varepsilon_{\mathrm{ac}}^{\mathrm{total}}
+ (m-1) \delta_\infty + O(1)
= N h(X) + O(1) + (m-1) \delta_\infty + \varepsilon_{\mathrm{ac}}^{\mathrm{total}}
= N h(X) + o(N) + \varepsilon_{\mathrm{ac}}^{\mathrm{total}}$
for $K = \omega(\log N)$ (so $m \delta_\infty = o(N)$). Dividing by
$N$: $L_{\mathrm{RNR}}/N \to h(X)$.
\hfill$\blacksquare$
