...
joint dictionary-and-predictor learning (Theorem 6.7), plus an
APX-hardness transfer for the latter (Theorem 6.8); the analogous
inapproximability for $E_{12}$ is open. Worked numerical examples are
...
with cost $O(\log(N/K) + (K + k) \cdot \mathrm{cost}(M))$ for
$W$-bounded predictors (Theorem 10.3) --- polylogarithmic in archive
size $N$ once the sync-point spacing $K$, read window $k$, and
predictor cost are treated as fixed parameters. The estimated
sync-point compression overhead at typical parameters (Section
10.7.4) is on the order of one percent for natural data, but a tight
upper bound on this overhead in general is not established; in the
worst case the cold-context prefix dominates for small $k$. The
