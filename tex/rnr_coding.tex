...
lower bound Theorem 6.2). A 24-theorem byte-granular random-access
...
and functional RNR variants (Theorems 7.6--7.12), proves a
Doob-martingale concentration bound for archive-size deviation
(Theorem 7.14), and synthesises an end-to-end prediction for
Chinchilla-class neural LMs ($\approx$79\,MB on enwik9 at 0.664\,bpb;
Theorem 7.15, with an explicit model-overhead caveat noting that the
compression ratio excludes the $\approx$140\,GB model).
...
\item
  ...
  parameter values.
\item
  A 24-theorem RNR-with-random-access framework (Section 7.1--7.24)
...
and the 24-theorem RNR-with-random-access framework
...
The 24-theorem §7 framework draws on several established lines outside
...
\item
  ...
  resistance under standard PRF assumptions), and T7.24 (revised)
  (minimal honest decoder-latency Hoeffding tail bound under IRM
  workload + static Perfect-LFU cache + bounded $T_{\max}$). The
  §7.24 history (LRU version retracted under codex disproof,
  rewritten under restricted hypotheses, survived round 2)
  illustrates the validation discipline applied throughout.
  All 24 theorems + 2 corollaries verified numerically in
...
\item
  Tightened concentration of $L^{\mathrm{rep}}_{\mathrm{RNR}}$ under
  polynomially-slow mixing. Theorem 7.14 (as revised after
  codex 2026-05 adversarial review) gives the Doob-martingale tail
  bound with deviation factor $(W_N + 1 + \rho/(1-\rho)) \cdot
  \log_2(1/\eta)$, where $\rho$ is the exp-mixing constant from
  hypothesis (5.8). Under \emph{constant} $\rho$ this is
  asymptotically tight (the $(W_N+1)$ predictor-context term
  dominates), but for processes whose mixing slows polynomially
  with $N$ (where $1/(1-\rho)$ becomes super-constant in $N$), the
  mixing-tail term dominates and the bound becomes loose. A clean
  concentration analysis under polynomial mixing --- bypassing the
  $\rho/(1-\rho)$ factor via Rio (2000) or Samson (2000)
  concentration-for-mixing-processes machinery --- would extend
  Theorem 7.14 to a broader source class. Either a positive result
  (tighter bound) or a converse (showing the $\rho/(1-\rho)$ factor
  is necessary) would close this question.
\end{itemize}
