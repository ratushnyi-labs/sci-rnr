...
with the explicit inapproximability constant
$\gamma_{\mathrm{SGP}} = 8569/8568$ inherited from Charikar et al.\
(2005, Theorem 1) via the symbol-count bijection (Theorem 6.8);
...
\item
  ...
  case with explicit constant $\gamma_{\mathrm{SGP}} = 8569/8568$
  inherited from Charikar et al.\ (2005, Theorem 1) via the
  symbol-count bijection (Theorem 6.8). The 8569/8568 constant
  derives from the Berman-Karpinski (1999) bound on
  Vertex-Cover-3 inapproximability propagated through the
  $15|V|+3|E|+k$ encoding overhead. We do not claim hardness of
...
\textbf{Lemma 5.1a (Per-position sequential recoding achieves joint
entropy with byte-granular random access).} \emph{Let $X \sim D_N$ be
a block of $N$ symbols over a $\sigma$-symbol alphabet, and let $M$
be a $W$-bounded predictor (Definition 10.2) that at each position
$i$ outputs the chain-rule conditional distribution
$P(x_i \mid x_{i-W}, \dots, x_{i-1}, C)$. Per-position sequential
arithmetic coding of $X$ under these conditionals achieves expected
total length $H(D_N) + \varepsilon_N \cdot N$ bits (the joint
entropy, up to entropy-coder redundancy), and Theorem 10.3 sync
points enable byte-granular random access at this rate.}

\emph{\textbf{Proof.}} By the chain rule of entropy,
$H(D_N) = \sum_i H(X_i \mid X_{<i})$. Arithmetic coding under the
chain-rule conditionals achieves $\sum_i H(X_i \mid X_{<i}) +
\varepsilon_N \cdot N$ bits (one $\varepsilon_N$ per position; see
Cover and Thomas 2006, Theorem 5.4.1). Under the $W$-bounded
hypothesis, each $P(x_i \mid x_{<i}, C)$ depends only on the last
$W$ bytes, so the coding is per-position sequential (in the sense of
Theorem 10.3's scope). Inserting sync points per Definition 10.3
and applying Theorem 10.3 directly gives byte-granular random access
at $O(\log L(R) + (K+k) \cdot \mathrm{cost}(M))$ time per query.
\hfill$\blacksquare$

\emph{Consequence for Theorem 5.4.} Theorem 5.4's $\Theta(N)$
amortized separation between factorized and joint coding of the
linear-code family was stated using whole-block partition-rank
coding, which gives only sub-block-granular random access (Remark
10.3a). Lemma 5.1a shows the \emph{same rate} is achievable via
per-position sequential coding under the chain-rule conditional
predictor of the linear code, provided the chain-rule predictor is
$W$-bounded. For local linear codes (e.g., LDPC with bounded-degree
parity-check structure), $W$ equals the maximum check-node radius and
the chain-rule predictor is $W$-bounded by construction; for general
linear codes the chain-rule predictor may be globally non-local and
$W$-bounded recoding requires sub-block restructuring (recovering
only sub-block-level joint structure, as in Remark 10.3a). Thus the
random-access tax for the $\Theta(N)$ separation is the gap between
the global chain-rule rate and the best $W$-bounded chain-rule
rate, which is a property of the data class.
...
injections $\{1, \dots, N\} \to \{1, \dots, K\}$ where $K = \alpha N$
for a fixed $\alpha > 1$. (This models a database page of $N$ rows
with a primary-key column drawn from a value range of size
$\alpha N$, with the UNIQUE constraint that all PK values are
distinct.) Then}
\[
\mathrm{TC}(D_N) \;=\; N \cdot \bigl[\log_2 e - (\alpha-1) \log_2(\alpha/(\alpha-1))\bigr] + O(1).
\tag{5.5}
\]
\emph{In particular, $\mathrm{TC}(D_N) = \Theta(N)$ for any fixed
$\alpha > 1$. The coefficient $c(\alpha) =
\log_2 e - (\alpha-1) \log_2(\alpha/(\alpha-1))$ is strictly
positive and decreases monotonically from $c(2) = \log_2(e/2)
\approx 0.4427$ (tightest constraint, $K = 2N$) toward $0$ as
$\alpha \to \infty$ (the constraint becomes vacuous).}
...
\end{aligned}
\]
The coefficient $c(\alpha) = \log_2 e - (\alpha-1) \log_2(\alpha/(\alpha-1))$
satisfies $c(\alpha) > 0$ for all $\alpha > 1$ (since $(\alpha-1) \log_2(\alpha/(\alpha-1))$
is increasing in $\alpha$ and approaches $\log_2 e$ from below as
$\alpha \to \infty$) and $c(\alpha) \to 0$ as $\alpha \to \infty$ (the
UNIQUE constraint becomes vacuous when $K \gg N$). \hfill$\blacksquare$
...
when symbols are encoded sequentially. The general
dependent-coordinate case is open; we give one positive partial
result below for the independent-coordinate case, where top-$Q$ is
provably optimal.}

\textbf{Lemma 5.7a (Top-$Q$ is optimal for independent positions).}
\emph{Suppose the predictor's joint law factorizes: the events
$\{i \in M_v\}$ for $i \in \{1, \dots, N\}$ are mutually
independent, with marginals $q_i = \Pr(i \in M_v)$, and assume
$A_s$ is non-decreasing on the feasible range $\alpha \in \{0, \dots,
k_v\}$. Then the support $S_v^\star$ of size $s$ that minimizes
$\mathbb{E}_{M_v \sim Q}[A_s(|M_v \setminus S_v|)]$ is obtained by
selecting the $s$ positions with the largest marginal probabilities
$q_i$.}

\emph{\textbf{Proof.}} For an arbitrary size-$s$ subset $S_v$, the
random variable $\alpha_{S_v} = |M_v \setminus S_v| = \sum_{i \notin
S_v} \mathbf{1}[i \in M_v]$ is a sum of $N - s$ independent
Bernoullis with parameters $\{q_i : i \notin S_v\}$. We show that
the top-$Q$ choice $S_v^\star$ gives an $\alpha$ that is first-order
stochastically dominated by the $\alpha$ from any other size-$s$
choice; hence for any non-decreasing $A_s$,
$\mathbb{E}[A_s(\alpha_{S_v^\star})] \leq \mathbb{E}[A_s(\alpha_{S_v})]$.

Take any $S_v \neq S_v^\star$. Pick a position $i^\star \in S_v^\star
\setminus S_v$ (which has $q_{i^\star} \geq q_j$ for all $j \notin
S_v^\star$ by top-$Q$ definition) and $i \in S_v \setminus S_v^\star$
(so $q_i \leq q_{i^\star}$). Let $S_v' = (S_v \setminus \{i\}) \cup
\{i^\star\}$; this $S_v'$ is closer to $S_v^\star$ by exactly one
swap, and we show $\alpha_{S_v'}$ stochastically dominates
$\alpha_{S_v}$ from below (i.e., $\alpha_{S_v'} \preceq_{\mathrm{st}}
\alpha_{S_v}$).

Couple the two random variables: define independent Bernoullis
$B_j$ with parameters $q_j$ for $j \neq i, i^\star$, plus independent
$B_i \sim \mathrm{Bern}(q_i)$ and $B_{i^\star} \sim
\mathrm{Bern}(q_{i^\star})$. Then $\alpha_{S_v} = B_i + \sum_{j
\notin S_v, j \neq i^\star} B_j$ (since $i^\star \notin S_v$) and
$\alpha_{S_v'} = B_{i^\star} + \sum_{j \notin S_v', j \neq i} B_j$
(since $i \notin S_v'$). The sums over $j \neq i, i^\star$ on both
sides are identical. The difference is $B_i$ vs $B_{i^\star}$: since
$q_i \leq q_{i^\star}$, we have $B_i \preceq_{\mathrm{st}}
B_{i^\star}$, hence by coupling $B_{i^\star} \succeq B_i$ pointwise
under a standard monotone coupling, so $\alpha_{S_v'}
\succeq_{\mathrm{st}} \alpha_{S_v}$.

Wait, we want $\alpha_{S_v}$ to dominate $\alpha_{S_v^\star}$ (so
that swapping toward $S_v^\star$ \emph{decreases} $\alpha$). Indeed
$\alpha_{S_v} - \alpha_{S_v'} = B_i - B_{i^\star}$, and under the
coupling $B_i \leq B_{i^\star}$ pointwise, so
$\alpha_{S_v'} \leq \alpha_{S_v}$ pointwise. Hence
$\mathbb{E}[A_s(\alpha_{S_v'})] \leq \mathbb{E}[A_s(\alpha_{S_v})]$
for non-decreasing $A_s$. Iterating this swap argument until we
reach $S_v^\star$, we obtain
$\mathbb{E}[A_s(\alpha_{S_v^\star})] \leq
\mathbb{E}[A_s(\alpha_{S_v})]$. \hfill$\blacksquare$

\textbf{Corollary 5.7b.} \emph{When predictor positions are
empirically (approximately) independent --- e.g., when the
predictor is a per-position softmax with no joint structure ---
top-$Q$ support selection is optimal up to the
approximate-independence gap. The gap is exactly the
multi-information $\mathrm{TC}$ of the joint law of
$\mathbf{1}[i \in M_v]$ across $i$, which Theorem 5.5 has already
identified as the operational separator between factorized and
joint coding.}

\emph{General-case status.} For dependent joint laws of $M_v$, top-$Q$
is provably suboptimal (Remark 5.7's counterexample). The
...
and APX-hard with constant $\gamma_{\mathrm{SGP}} = 8569/8568$; the fully
...
is APX-hard with inapproximability ratio at least
...
bijection of Theorem 6.7. The APX-hardness of the unrestricted
joint problem is left open for the same reason as in Theorem 6.7:
the feasible set is strictly larger and the objective function
differs, so neither inclusion nor a direct gap-preserving reduction
is available.}
...
Cover on graphs with $|E| \geq (3/2)|V|$ and minimum vertex cover
$k \geq (1/3)|V|$, encoded as the grammar-input string
...
$P = NP$ then gives, by direct algebra,
\[
\gamma_{\mathrm{SGP}} \;=\; \frac{15|V| + 3|E| + (145/144)k}{15|V| + 3|E| + k}
\bigg|_{|E| = (3/2)|V|,\, k = (1/3)|V|}
\;=\; \frac{8569}{8568}.
\]
...
that this constant inherits to the unrestricted bit-cost
objective, since the latter problem's APX-hardness is itself open
(see §13.2).

Together, Theorems 6.7 and 6.8 show that the restricted normal-form
recursive dictionary problem under symbol-count objective is
NP-hard and APX-hard. We do not claim this extends to all
...
\item
  ...
  with the explicit inapproximability constant
  $\gamma_{\mathrm{SGP}} = 8569/8568$. The constant derives from
  Berman-Karpinski's (1999) bound on Vertex-Cover-3 propagated
  through the $15|V|+3|E|+k$ encoding overhead in the Charikar et
  al.\ reduction; the algebra is reproduced in the verification
  script \texttt{scripts/verify/thm\_6\_8\_sgp\_constant.py} in
  the companion repository. Hardness of the \emph{unrestricted}
...
\item
  ...
  top-$Q$ is provably optimal when the predictor's position
  indicators $\mathbf{1}[i \in M_v]$ are mutually independent
  (verified in
  \texttt{scripts/verify/lemma\_5\_7a\_independent\_top\_q.py}).
  Corollary 5.7b notes the gap is exactly the multi-information
  $\mathrm{TC}$ of the joint indicator law. For dependent joint
  laws, top-$Q$ is provably suboptimal (Remark 5.7 counterexample
  at $N=7, k=2, s=2$). A formal NP-hardness proof for the general
  dependent case (likely via reduction from densest-$k$-subgraph)
  and a polynomial-time approximation algorithm are both open.
...
\item
  ...
  the analogous constant for $E_{12}$ is open. (A sharper
  $\gamma_{\mathrm{SGP}}$ for SGP itself --- e.g., via direct
  reduction from MAX-3SAT-3 rather than via Vertex Cover --- is a
  separate open question in the SGP literature.)
\item
