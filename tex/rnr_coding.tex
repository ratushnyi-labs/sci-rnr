...
sub-block-based byte-granular random access).} \emph{For a stationary
source $X$ on a finite alphabet, any byte-granular random-access
coding scheme that encodes the archive as $m$ independently-decodable
sub-blocks of size $K$ (so $N = mK$) and uses no cross-block context
satisfies the lower bound}
\[
L_{\mathrm{synced}} \;\geq\; m \cdot H(X^K) \;=\; m \cdot H(D_K),
\]
\emph{matched by Lemma 5.1c's construction at}
$L_{\mathrm{synced}} = m \cdot (K \cdot h(X) + \delta_{W'})$
\emph{for order-$W'$ Markov $X$ with $K \geq W'$, and by Lemma 5.1d's
upper bound at $m \cdot (K \cdot h_W + \delta_W^{(h_W)}) \leq m \cdot
(K \cdot h_W + \delta_\infty)$ for exp-mixing stationary $X$. In the
order-$W'$ Markov case the upper and lower bounds coincide:
...
\emph{\textbf{Proof.}} The byte-granular random-access requirement
forces sub-block independence: a query at position $p$ must be
decodable from the sub-block containing $p$ alone (plus possibly a
constant-size shared header), without invoking other sub-blocks. By
Shannon's source coding theorem (Cover \& Thomas 2006 Theorem 5.4.1),
the expected length of any such encoding of a single sub-block of size
$K$ is at least the joint entropy $H(D_K) = H(X_1, \ldots, X_K)$ of
the sub-block law (under stationarity, this equals the marginal of any
length-$K$ window of $X$). With $m$ independent sub-blocks:
$L_{\mathrm{synced}} \geq m \cdot H(D_K)$. Substituting the chain-rule
expansion $H(D_K) = K \cdot h(X) + \delta_W$ for order-$W'$ Markov $X$
with $K \geq W' = W$ (Lemma 5.6c case (a) on the sub-block law),
