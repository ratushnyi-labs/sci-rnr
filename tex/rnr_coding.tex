...
\emph{(a) GPU batch encoding.} Modern GPUs (NVIDIA H100, AMD MI300X)
have $P \sim 10^4$ parallel cores. For a $1\,\mathrm{GB}$ archive
with $K = 1024$, $m = 10^6$: full GPU utilisation ($P = 10^4$) gives
$100\times$ speedup over single-core encoding. Wall-clock encoding
time on H100: $\sim 1\,\mathrm{second}$ ($10^6$ sub-blocks of $1024$
bytes each, transformer inference at $\sim 10\,\mu\mathrm{s}$/sub-block).
