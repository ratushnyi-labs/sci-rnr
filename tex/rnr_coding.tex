...
\item
  ...
  > 0$ for the distinct-cardinality (uniqueness-constrained) class,
  and Lemma 5.6b establishes the exact identity
  $\mathrm{TC}(D_N) = N \cdot I(X_1; Y) - H(Y) + H(Y \mid X^N)$ for
  hidden-class iid sources (a single latent $Y$ with conditionally
  iid observations), recovering $\Theta(N)$ scaling whenever the
  per-position mutual information $I(X_1; Y) > 0$ and the latent
  classes are mutually distinguishable. Together these two lemmas
  cover the canonical ``natural-data'' patterns of column-permutation
  invariance and latent-topic mixture, with explicit constants in
  both cases. Whether $\mathrm{TC}(D_N) = \Theta(N)$ extends to
  further natural-data classes (Markov chains beyond bigrams, hidden
  Markov models, log-likelihood-ratio sources) remains an open
  empirical question (Remark 5.6 sketches a manifold-hypothesis
  intuition but does not establish a rigorous entropy bound for
  those).
\item
...
follows from a standard Chernoff-type estimate. If
$D_{\mathrm{TV}}(p_y, p_{y'}) \geq \delta > 0$ for all
$y \neq y'$, then the Bayes-optimal classifier for $Y$ given $X^N$
errs with probability $O(\exp(-c \delta^2 N))$ for some
$c > 0$ depending on the prior $\Pr(Y)$ (Bretagnolle--Huber 1979,
LeCam 1973; see also Tsybakov \emph{Introduction to Nonparametric
Estimation} 2009, Chapter 2). Fano's inequality then bounds
$H(Y \mid X^N) \leq h_2(P_e) + P_e \cdot \log_2(|\mathcal{Y}| - 1)
\to 0$ as $N \to \infty$ where $P_e$ is the error probability.
...
for Lemma 5.6b). Further classes (Markov chains, hidden Markov
models, factor graphs over expanders) admit similar treatments
but with strictly more involved bookkeeping; they remain open as
explicit lemmas.
...
\item
  ...
  natural-data classes --- partially resolved. Two explicit
  ...
  conditionally iid samples), with $H(Y \mid X^N) \to 0$
  exponentially when the per-class distributions are mutually
  distinguishable. As a companion, the stationary Markov chain
  has $\mathrm{TC}(D_N) = (N-1) \cdot I(X_1; X_2)$ (chain-rule
  identity, also verified in
  \texttt{scripts/verify/lemma\_5\_6b\_hidden\_class\_tc.py}). Both
  proofs extend additively along independent column blocks.
  ...
  structured classes (natural images at a given resolution, full
  English text at a given vocabulary with long-range structure, or
  general hidden Markov models with non-trivial hidden state) is
