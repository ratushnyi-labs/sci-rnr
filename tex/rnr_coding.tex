...
multi-information identity $\mathrm{TC}(D_N)$ and quantified for
natural data by the manifold hypothesis; an optimal-support
characterization for Type-II partition coding; a tight $2\varepsilon$
approximate-oracle bound for Type-III-A mode selection with a
matching lower bound; an information-theoretic invariance result for
hierarchical decomposition in Type-III-B; bits-back integration into
the framework's code-length decomposition with exact KL accounting;
NP-hardness and APX-hardness of joint dictionary-and-predictor
learning in Type-III-C via reduction from the Smallest Grammar
Problem; sufficient conditions for bit-exact decoder reproduction
under both fixed and adaptive predictors via deterministic integer
arithmetic, with the adaptive case relying on a class of compatible
online optimizers drawn from the low-precision-training literature;
and a random-access (seek-and-read) capability in time independent of
archive size for $W$-bounded predictors, at a sync-point compression
cost of approximately one percent, matching the seekable
functionality of dictionary-based formats while preserving the
neural-ratio advantage and exceeding the capability of existing
neural compressors which lack random access entirely. We do not claim
universal dominance --- no lossless code can map every input of
length $n$ to a strictly shorter one --- and the framework is
evaluated conditionally on data classes for which side information
reduces residual uncertainty by more than the model and repair
overhead costs.
...
\item
  ...
  via a linear-code construction, generalized by Theorem 5.6
  ...
  for natural data by Remark 5.7 (manifold-hypothesis quantification
  with empirical intrinsic-dimension estimates).
\item
  An optimality characterization for Type-II support sets
  (Proposition 5.5): optimal $S_v$ is a prefix of positions sorted by
...
\item
  ...
  Proposition 5.5: the optimal $S_v$ is a prefix of positions sorted
...
\item
  ...
  natural-data submanifolds. Theorem 5.6 establishes that the
  separation between factorized and joint coding equals
  $\mathrm{TC}(D_N)$, and Remark 5.7 predicts this is $\Theta(N)$
