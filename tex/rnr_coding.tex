...
\item
  ...
  \log_2(1/\eta) = O((|X|/K) \log N \cdot \log_2(1/\eta))$ for
  $m = \lceil |X|/K \rceil$ sync points; this overhead is sub-linear
  in $|X|$ only when sync-point spacing $K = \omega(\log N)$
  (e.g., $K = \mathrm{polylog}(|X|)$ gives $\Delta_{\mathrm{sync,total}}
  = \mathrm{polylog}(|X|)$). The per-query random-access cost is
...
$\mathrm{polylog}(N)$; for fixed $K = O(1)$ the overhead is
...
\item
  ...
  sources: choosing $W = O(\log N)$ brings $H_W$ within $O(1)$ of
  the joint entropy $H(D_N)$, giving total length
  $H(D_N) + O(1) + \mathrm{polylog}(|X|)$ at $K = \mathrm{polylog}(|X|)$.
