...
\item
  ...
  parameter values.
\item
  A 23-theorem RNR-with-random-access framework (Section 7.2--7.24)
...
and the 23-theorem RNR-with-random-access framework
...
\;\leq\;
2 \exp\!\Bigl(-\frac{t^2}{2 N \cdot (W_N + 1)^2 \cdot \log_2^2(1/\eta)}\Bigr).
\]
\emph{The $(W_N+1)$ factor reflects the dependency of $\ell_k$
($k \in [n, n+W_N]$) on $X_n$ via the $W_N$-bounded predictor context,
which widens the martingale-difference bound.
Equivalently, for $t = c \cdot \sqrt{N} \cdot (W_N+1) \cdot \log_2(1/\eta)$
the deviation probability is $\leq 2 \exp(-c^2/2)$. Taking $c = \sqrt{2
\log(2/\delta)}$ gives a $1 - \delta$ confidence bound: with
probability at least $1 - \delta$,}
\[
\bigl|\,L^{\mathrm{rep}}_{\mathrm{RNR}}(X^N) - N \cdot h(X) - m \cdot \delta_\infty - O(1)\,\bigr|
\;\leq\;
\sqrt{2 N \log(2/\delta)} \cdot (W_N + 1) \cdot \log_2(1/\eta).
\]
...
$S_N := \sum_n \ell_n$ and the martingale
$M_n := \mathbb{E}[S_N \mid X_1, \ldots, X_n] - \mathbb{E}[S_N]$.
The martingale differences $D_n := M_n - M_{n-1}$ capture the change
in conditional expectation induced by revealing $X_n$. Under the
$W_N$-bounded predictor, $X_n$ enters the context window of $\ell_k$
for $k \in [n, n + W_N]$, so $|D_n| \leq (W_N + 1) \cdot \log_2(1/\eta)$
(the $(W_N+1)$ subsequent log-loss terms can each change by at most
$\log_2(1/\eta)$ when conditioning on $X_n$).
...
ensures that the martingale-difference framework applies without
needing strong independence (Bennett 1962 \emph{J.\ Amer.\ Stat.\ Assoc.}\
57(297):33 for finite-variance versions; McDiarmid 1989 \emph{Surveys
in Combinatorics} Cambridge 148:148--188 ``On the method of bounded
differences'' for the bounded-differences variant under weak
dependence).
...
$N = 1\,\mathrm{GB}$, $\eta = 2^{-16}$, $\delta = 10^{-6}$: deviation
$\leq \sqrt{2 \cdot 10^9 \cdot 28} \cdot 16 = 3.8 \,\mathrm{MB}$
(roughly $0.4\%$ of archive). Practical predictability.
...
\;\pm\;
\underbrace{\sqrt{2 N \log(2/\delta)} \cdot W_N \cdot p}_{\substack{\text{concentration} \\ O(\sqrt{N} \log(1/\delta) \cdot W_N p)}}
\;+\;
...
\item Concentration at $1\sigma$ ($\delta \approx 0.32$, so $\log(2/\delta)
\approx 1.84$): $\sqrt{2 \cdot 10^9 \cdot 1.84} \cdot 32 \approx
1.94 \cdot 10^6$ bits $\approx 244 \,\mathrm{kB}$
(or $\sim 0.3\%$ of archive).
\item Concentration at $\delta = 10^{-6}$ ($\log(2/\delta) \approx 14.5$):
$\sqrt{2 \cdot 10^9 \cdot 14.5} \cdot 32 \approx 5.45 \cdot 10^6$ bits
$\approx 665 \,\mathrm{kB}$ ($\sim 0.8\%$ of archive).
\item Predicted total: $\approx 79 \pm 0.7 \,\mathrm{MB}$ at $\delta
= 10^{-6}$ ($\sim 12.0\times$ compression vs $1 \,\mathrm{GB}$ raw text,
...
$\sqrt{2 N \log(2/\delta)} \cdot W_N \cdot p$ (with $W_N = O(\log N)$
factor reflecting predictor-context memory; see Theorem 7.14's
$(W_N+1)$-bound). Combine all terms.
...
arrival processes is in Ferragut et al.\ 2017 \emph{Queueing Systems}
88:207, Theorem~3 (general timer-based-cache hit-rate, with LRU as
a limit).
...
times) is left to future work (cf.\ Fagin 1977 \emph{Comm.\ ACM}
20(11):846; Hendricks 1972; King 1971 \emph{J.\ ACM}).
...
enwik9 source, with $\pm 0.7\,\mathrm{MB}$ Azuma deviation at
$\delta = 10^{-6}$. This is a $12.0\times$ \emph{bitstream-rate}
...
The 23-theorem §7 framework draws on several established lines outside
...
follows the Independent Reference Model of Fagin (1977) \emph{Comm.\
ACM} 20(11):846--851; foundational LRU+IRM analysis is in Hendricks
(1972) and finite-batch deviation bounds for paging are in King
(1971) \emph{J.\ ACM} 18(1):95--117. Ferragut, Rodriguez, and
Paganini (2017) \emph{Queueing Systems} 88:207 give modern
timer-based-cache hit-rate analysis under general arrival processes.
...
\emph{Scaling Laws for Neural Language Models}, arXiv:2001.08361,
whose data-scaling exponent was estimated at $\approx 0.076$. The
...
\emph{Fast Software Encryption}; BLAKE3 in Aumasson et al.\
(2020/2022) ``BLAKE3: one function, fast everywhere''.
...
\item
  ...
  results in this direction are Fagin (1977) \emph{Comm.\ ACM}
  20(11):846--851 ``Asymptotic miss ratios over independent
  references'' and Hendricks (1972) on LRU stationary analysis;
  King (1971) \emph{J.\ ACM} 18(1):95--117 gives finite-batch
  deviation bounds for paging algorithms but not in the form needed
  for direct application here. A clean LRU+IRM tail bound matching
  Theorem 7.24's accessibility would be a useful contribution.
\item
