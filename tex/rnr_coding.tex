...
$L_{\mathrm{synced}} \geq m \cdot H(D_K)$
\emph{by Shannon's source coding theorem per sub-block. For
order-$W'$ Markov sources with $W' \leq W$ and $K \geq W'$, this
equals $H(D_N) + (m-1) \cdot \delta_{W'}$ exactly (matching (a)
ignoring the $\varepsilon_{\mathrm{ac}}^{\mathrm{total}}$ coder
redundancy and the $N \cdot (h_W - h)$ term which vanishes for
order-$W'$ Markov). For general exponentially mixing non-Markov
sources, the lower bound is $m \cdot H(D_K) = H(D_N) + m \delta_K -
\delta_N$, weakly matching (a) up to $\delta_K - \delta_W^{(h_W)}$
slack; the achievability and lower bound coincide tightly only in
the order-$W'$ Markov sub-case.}

\emph{(c) Open extensions.} \emph{(i) Non-sub-block-based schemes
(state-snapshot framework of Theorem 10.4): the lower bound (b) does
not apply, and the trade-off between sub-block-independence excess
$(m-1) \cdot \delta_W$ and state-snapshot storage $m \cdot
L(\mathrm{state})$ (architecture-dependent, ranging from
$\Theta(W \log_2 \sigma)$ for explicit context-byte storage to
the predictor's internal hidden-state size for general models) is
engineering-determined per use case.
