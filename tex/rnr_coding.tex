...
Theorem 6.5 is itself a universal transform is left open (Remark 6.6b).
...
\to h(X)$ whenever the dictionary is built universally (the
greedy-MDL-builder universality being the one residual sub-question,
Remark 6.6b). Type-III-A has both a \emph{relative} converse --- the
...
per-tile-expressive \emph{and} the tiles are conditionally independent
given $Y$, the deficit otherwise being exactly the inter-tile total
correlation $\mathrm{TC}(X_1,\ldots,X_T\mid Y)$ --- the genuine price of
the modular per-tile architecture. Complexity results include
NP-hardness of the
...
crossover budget being characterised in Remark 6.9c. Worked numerical examples are
...
$\mathbb{E}[L_{\mathrm{IIIA}}]\ge H(X\mid Y)$ whose slack is exactly the
inter-tile total correlation $\mathrm{TC}(X_1,\ldots,X_T\mid Y)$
(Theorem 6.2b). A 27-theorem byte-granular random-access
...
extends to universal (with a tight $d/2$ minimax-regret constant,
Theorem 7.6$'$), Slepian--Wolf, lossy, successive-refinement,
and functional RNR variants (Theorems 7.6--7.12), proves
concentration bounds for archive-size deviation via a trilogy of
sharpening estimates --- Azuma's martingale inequality
(Theorem 7.14), a variance-aware Bernstein refinement (Theorem 7.25,
replacing the worst-case Lipschitz $B$ by the per-position-cost std
$\sigma_\ell$ for an approximate $500\times$ tightening on enwik9),
and a non-asymptotic Berry--Esseen CLT giving two-sided confidence
intervals at Kolmogorov rate $O(N^{-1/2}(\log N)^2)$ (Theorem 7.28)
--- and
...
\item
  ...
  the greedy-MDL builder's own universality left open (Remark 6.6b),
...
\item
  ...
  parameter values.
\item
  A 27-theorem RNR-with-random-access framework (Section 7.1--7.28)
  ...
  sharpened to the tight $d/2$ minimax-regret constant via the
  Shtarkov--Rissanen pointwise-redundancy form in Theorem 7.6$'$ ---
  ...
  per-miss cost. Theorem 7.25 sharpens the Theorem 7.14 concentration
  to a variance-aware Bernstein bound (per-position-cost std
  $\sigma_\ell$ replacing the worst-case Lipschitz $B$, an approximate
  $500\times$ tightening on enwik9); Theorem 7.26 closes the LRU+IRM
  finite-batch concentration question left open by Theorem 7.24, via
  Lezaud's Markov-chain Chernoff bound on the LRU stack-distance
  chain; Theorem 7.27 establishes a tight space-time product converse
  $C(K)\cdot S(K)=\Theta(N\log N\cdot\mathrm{cost}(M))$ for the
  sub-block random-access class, matching the Theorem 7.5
  achievability (with Theorem 7.27$'$ refuting the general-scheme
  analogue via a Ferragina--Venturini $O(1)$-access escape for
  finite-order-Markov sources, and Corollary 7.27c recovering the
  product on the predictor-faithful class); and Theorem 7.28 gives a
  non-asymptotic Berry--Esseen CLT for $L^{\mathrm{rep}}$ under
  exp-mixing, yielding two-sided confidence intervals at Kolmogorov
  rate $O(N^{-1/2}(\log N)^2)$. The full framework has been adversarially cross-
...
(§§7.1--7.28, organized as: §§7.2--7.5 basic byte-granular
random-access architecture; §§7.6--7.11 source-coding extensions
including the tight $d/2$ minimax-regret refinement (Theorem 7.6$'$)
and the lossy + ECC = lossless corollary; §§7.12--7.15
...
covers state-snapshot schemes; and §7.28 a non-asymptotic
Berry--Esseen CLT for the repair length, completing the
Azuma/Bernstein/Berry--Esseen concentration trilogy of §§7.14,
7.25, 7.28). Section 8 states the
...
unambiguous-CFL enumerative encoder (Lemma 13.4.3a; §13.4.3). The
questions exposed by the §7 expansion --- LRU+IRM finite-batch
concentration and tightened concentration under slow mixing --- are
themselves now resolved within §7 (Theorems 7.26 and 7.25/7.28
respectively).
...
$X_1,\ldots,X_T$ and let $Y$ be decoder-reproducible side information.
For any uniquely-decodable lossless Type-III-A code reproducing $Y$,}
\[
\mathbb{E}[L_{\mathrm{IIIA}}] \;\ge\; H(X \mid Y).
\tag{6.2b.1}
\]
\emph{Moreover the per-tile-oracle target of Theorem 6.1, with each tile
coded by an entropy-coding mode, satisfies the chain}
\[
\sum_{t=1}^{T} \min_{m} \mathbb{E}[L_m(t)]
\;\ge\; \sum_{t=1}^{T} H(X_t \mid Y)
\;\ge\; H(X \mid Y),
\tag{6.2b.2}
\]
\emph{where (a) the first inequality is equality iff the mode set is
\emph{per-tile-expressive} --- for each tile $t$ some mode attains
$\mathbb{E}[L_m(t)] = H(X_t \mid Y)$ up to the coder redundancy
$\varepsilon$ --- and (b) the second is equality iff the tiles are
\emph{conditionally independent given $Y$}, the deficit being exactly
the inter-tile total correlation}
\[
\sum_{t} H(X_t \mid Y) - H(X \mid Y)
\;=\; \mathrm{TC}(X_1,\ldots,X_T \mid Y) \;\ge\; 0.
\tag{6.2b.3}
\]
\emph{Consequently Type-III-A attains the absolute floor $H(X\mid Y)$
(within the $2\sum_t \varepsilon_t$ of Theorem 6.1) \textbf{iff the
modes are per-tile-expressive AND the tiles are conditionally
independent given $Y$}; otherwise it overpays by the sum of the
mode-inexpressiveness gap and the inter-tile total correlation
$\mathrm{TC}(X_1,\ldots,X_T \mid Y)$.}
...
H(X \mid Y)$. For (6.2b.2), each tile is encoded by a single mode; an
entropy-coding mode has $\mathbb{E}[L_m(t)] \ge H(X_t \mid Y)$ (no code
for $X_t$ beats its conditional entropy), so $\min_m \mathbb{E}[L_m(t)]
\ge H(X_t \mid Y)$, with equality iff some mode matches the tile's
conditional law --- per-tile expressiveness. Summing over tiles and
applying the chain rule $H(X \mid Y) = \sum_t H(X_t \mid X_{<t}, Y)$
together with ``conditioning reduces entropy'' $H(X_t \mid X_{<t}, Y)
\le H(X_t \mid Y)$ gives the second inequality, with equality iff
$X_t \perp X_{<t} \mid Y$ for every $t$, i.e.\ conditional
independence; the deficit is the total correlation (6.2b.3) by
definition (Watanabe 1960). The optimality characterization follows by
comparing the Theorem 6.1 achievability (per-tile oracle $+ 2\sum_t
\varepsilon_t$) with the floor $H(X\mid Y)$. \hfill$\blacksquare$

\emph{Remark 6.2b (Type-III-A is a per-tile scheme: the inter-tile
gap is genuine, not removable by better prediction).} Unlike the
Type-III-C asymptotic optimality (Theorem 6.6c), Type-III-A is
\emph{not} generally tight to $H(X \mid Y)$. The $2\varepsilon$
machinery of Theorems 6.1--6.2 controls only the \emph{mode-selection}
loss; even a perfect predictor ($\varepsilon = 0$) and a fully
expressive mode set leave the inter-tile total correlation
$\mathrm{TC}(X_1,\ldots,X_T \mid Y)$ on the table, because a per-tile
coder encodes each tile in isolation and cannot exploit cross-tile
dependence. Type-III-A is therefore absolutely rate-optimal precisely
on sources whose tiles are conditionally independent given $Y$
(e.g.\ memoryless or tile-block-i.i.d.\ sources); on
dependent-tile sources it is the right tool only when the tiling is
chosen coarse enough that the residual cross-tile correlation is
negligible, or when composed with a cross-tile model (Type-III-C's
dictionary, or a larger predictor context). This honest gap ---
$\Theta(\mathrm{TC})$, the same $\Theta(N)$ quantity studied in §5.6
for correlated sources --- is the price of the modular per-tile
architecture, and it completes the Type-III-A converse cell: a
\emph{relative} converse on mode selection (Theorem 6.2, tight at
$2\varepsilon$) and an \emph{absolute} converse on rate (Theorem 6.2b,
tight iff expressive-and-conditionally-independent).
...
greedy marginal-MDL builder of Theorem~6.5 is itself universal is a
separate question we do not resolve (Remark~6.6b).
...
\emph{tightness} it certifies). The matching \emph{achievability}
...
greedy marginal-MDL builder of Theorem 6.5 is itself universal remains
the open sub-question of Remark 6.6b. With Theorems 6.6b and 6.6c the
...
The 27-theorem §7 framework draws on several established lines outside
...
\begin{enumerate}
\item[(i)] (Unconditional, all $\alpha \geq 0$.) $\mathcal A$ is a
...
\item[(ii)] (CNF subproblem, all $\alpha \geq 0$.) Against the
...
\item[(iii)] (General optimum, super-constant $\alpha$.) Granting the
...
\end{enumerate}
Consequently the additive-rule-cost approximability ratio lies in the
...
\emph{What this does \emph{not} establish (corrected after codex
adversarial review, 2026-06).} The upper bound alone does
...
after codex adversarial review (see Lemma~13.4.2a), this is an
