...
10.7.4) is bounded by $W \cdot (\text{cold-context rate} - \text{asymptotic rate})$
per sync point (Section 10.7.4); we conjecture it is on the order of
one percent for natural data at typical parameters, but neither a
tight upper bound in general nor a measured value on a specific
corpus is provided here. The
