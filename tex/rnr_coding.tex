...
applies instead to the combined $(\mathrm{state}, \mathrm{payload})$
object: the total charged archive length is still bounded below by
$m \cdot H(D_K)$, but the payload alone can be smaller than $H(D_K)$
because the state carries cross-block context. The engineering
