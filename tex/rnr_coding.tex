...
bound (ii) thus gives
\[
\Delta(N) = 4N - \left[(4 - c)N + \varepsilon_N\right] = cN - \varepsilon_N = \Theta(N)
\]
for any fixed $c \in (0, 4)$, once the one-time generator-matrix
description $L(G) = 4(4-c)N^2$ is amortized across $B_N = \Omega(N^2)$
blocks (the per-block model contribution then absorbs into the
$O(1)$ overhead). \hfill$\blacksquare$
...
path achieves expected length $H(B \mid C)$. The expected cost is
independent of the tree shape $T$; it depends only on the joint
distribution $P(B \mid C)$. \emph{Caveat:} this invariance fails if
the decisions are coded with fixed branch probabilities (e.g., 1/2
each) instead of the induced conditional --- in that case the depth
of $T$ dominates, and a left-skewed tree pays substantially more than
a balanced one. The theorem applies only to coders that use the
\emph{exact} induced conditional probability at each node.}
...
$\tau$ be a candidate token of byte length $\ell$. Fix a parser that
produces a non-overlapping token cover of the corpus and let $f$ be
the \emph{realized} count of $\tau$ in that parse (the
non-overlapping selected parse frequency, which can be strictly less
than the raw occurrence count when $\tau$ self-overlaps; e.g., for
$\tau = $ \texttt{aa} in the string \texttt{aaa}, the raw count is
$2$ but $f = 1$). Let $\mathrm{code\_cost}(\tau)$ be the average bit
length to encode an occurrence of $\tau$ under the predictor and
entropy coder (treated as a fixed cost in this comparison),
$\mathrm{dictionary\_cost}(\tau)$ the bit length to describe $\tau$
in the dictionary header, and $\mathrm{parse\_overhead}(\tau)$ the
additional per-occurrence boundary or interval bookkeeping introduced
by including $\tau$. The net change in total Type-III-C code length
resulting from admitting $\tau$ to the dictionary $D$ is}
\[
...
(Charikar et al.\ 2005), the restricted Type-III-C problem is NP-hard.

For the unrestricted Type-III-C problem we observe that the
restricted case sits inside it: any feasible solution to the
restricted problem is also feasible for the unrestricted problem (the
restricted problem only further constrains $M$ to be the trivial
constant predictor and $D$ to be a normal-form recursive dictionary,
both of which are admissible in the unrestricted problem). Because
the restricted and unrestricted problems share the same objective
function $L_{\mathrm{sym}}$ on the restricted feasible region, the
restricted optimum is an upper bound on the unrestricted optimum;
conversely the unrestricted optimum cannot be larger. An exact
polynomial-time solver for the unrestricted problem, run on a
restricted instance, would by construction return a solution whose
objective is at most the restricted optimum and hence determines the
restricted optimum (which equals the SGP optimum by the bijection).
The unrestricted problem is therefore at least as hard. The
reduction is computable in linear time in $|s|$. \hfill$\blacksquare$
...
threshold, contradicting $P \neq NP$. The unrestricted Type-III-C
problem is at least as hard (by the inclusion argument of Theorem
6.7), so the same APX-hardness transfers. \hfill$\blacksquare$
...
specific figure here. For the unrestricted Type-III-C objective
(with bit-cost bookkeeping rather than symbol-cost), the inherited
constant is at most $\gamma_{\mathrm{SGP}}$ divided by the
worst-case per-symbol encoding ratio
...
a specific encoding convention, and verifying which source paper
