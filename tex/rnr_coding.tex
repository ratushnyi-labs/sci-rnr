...
lower bound Theorem 6.2). A 25-theorem byte-granular random-access
...
store mode, and per-sub-block integrity via PRF-based MAC
(Theorems 7.16--7.24). The framework's deployment-relevant feature is random-access decoding
with cost
...
\item
  ...
  parameter values.
\item
  A 25-theorem RNR-with-random-access framework (Section 7.1--7.25)
...
and the 25-theorem RNR-with-random-access framework
(§§7.2--7.24, organized as: §§7.2--7.5 basic byte-granular
...
numerical synthesis; §§7.16--7.24 practical extensions covering
...
fail-safe store-mode fallback, cryptographic integrity, and an
IRM-restricted decoder-latency tail bound). Section 8 states the
...
\boxed{
\;
L^{\mathrm{rep}}_{\mathrm{RNR}}^{M'}(X^N)
\;=\;
...
with rigorous concentration band $\pm 1.4\,\mathrm{GB}$
(Azuma worst-case, theoretical) and practical concentration band
$\pm 0.04\,\mathrm{MB}$ (CLT-based empirical), giving
$\sim 12.0\times$ compression vs $1\,\mathrm{GB}$ raw text,
matching Del{\'e}tang et al.\ 2024 Table 1 measurement.
...
$\alpha(k) \leq C \exp(-c k)$. Assume the $\eta$-floor and
...
per-position-cost variance under the stationary law. Then for every
$\delta \in (0, 1)$, with probability at least $1 - \delta$,
\[
  \bigl| L^{\mathrm{rep}}(X) - \mathbb{E}[L^{\mathrm{rep}}(X)] \bigr|
  \;\leq\; C_1 \cdot \sigma_\ell \cdot \sqrt{N \cdot (W_N + 1) \cdot
  \log(2/\delta)}
  \;+\; C_2 \cdot B \cdot \log(N) \cdot \log(2/\delta),
\]
where $C_1, C_2$ are absolute constants depending only on the mixing
rate $c$ via the inverse-summable-mixing factor $K_\phi := 1 + 2
\sum_{k \geq 1} \alpha(k) \leq 1 + 2C/(1 - e^{-c})$.}

\emph{Scope and significance.} Theorem 7.25 replaces Azuma's
worst-case Lipschitz factor of $B = \log_2(1/\eta)$ in Theorem 7.14
by the variance proxy $\sigma_\ell$, which is typically
$\sigma_\ell \approx 2$ bits for English text (empirical, $B/\sigma_\ell
\approx 16$) versus the worst-case $B = 32$ used by Azuma. For a
Chinchilla-class transformer with $W_N = 2048$ on enwik9 at
$\delta = 10^{-6}$, the sub-Gaussian term yields
$C_1 \sigma_\ell \sqrt{N(W_N+1)\log(2/\delta)} \approx 1.3\,\mathrm{MB}$
(absolute constant $C_1 \in [1, 2]$ depending on the specific
Merlev\`ede-Peligrad-Rio constants, giving a conservative bound of
$\approx 2.6\,\mathrm{MB}$), versus Theorem 7.14's Azuma worst-case
of $\approx 1.3\,\mathrm{GB}$ --- a roughly $500\times$ improvement.
The bound remains $\sim 50$--$70\times$ loose against the practical
CLT estimate of $\approx 42\,\mathrm{kB}$, the residual gap stemming
from the conservative $(W_N+1)$ effective-mixing factor versus the
distribution-specific long-run covariance $\sum_k \mathrm{Cov}(\ell_0,
\ell_k)$. The bound applies under exp-mixing; the analogous
polynomial-mixing extension requires $\sum_k \alpha(k) < \infty$
(i.e.\ $\alpha(k) \leq C k^{-\beta}$ with $\beta > 1$).

\emph{\textbf{Proof.}} The per-position-cost sequence $(\ell_i)$
inherits the mixing of $X$, with $\alpha$-mixing coefficient at lag
$k$ bounded by $\alpha(\max(0, k - W_N))$ (because $\ell_i$ depends
only on $X_{i - W_N}, \ldots, X_i$, so $\ell_i$ and $\ell_{i+k}$ are
generated by disjoint coordinate sets for $k > W_N + 1$ and therefore
inherit the original $\alpha$-mixing rate of the underlying $X$
sequence; for $k \leq W_N + 1$ they share dependent coordinates and
mixing is bounded by $1$). The induced summable-mixing factor for
$(\ell_i)$ is $K_\phi^{(\ell)} \leq (W_N + 1) + 2 C/(1 - e^{-c}) =
O(W_N + 1/c)$.

Applying Merlev\`ede-Peligrad-Rio (2009, Theorem 1, ``Bernstein
inequality and moderate deviations under strong mixing conditions'')
to the bounded centred sequence $(\ell_i - \mathbb{E}[\ell_i])$ with
exp-mixing rate $c$, per-term bound $B$, and effective variance
$N \sigma_\ell^2 K_\phi^{(\ell)} = O(N \sigma_\ell^2 (W_N + 1))$, we
obtain
\[
  \Pr\bigl(\,|L^{\mathrm{rep}} - \mathbb{E}[L^{\mathrm{rep}}]| \geq \varepsilon\bigr)
  \;\leq\; \exp\!\Bigl(\,
    -\tfrac{c \varepsilon^2}{N \sigma_\ell^2 (W_N+1) + B \varepsilon \log(N)}
  \Bigr) .
\]
Setting the right-hand side equal to $\delta$ and inverting in
$\varepsilon$ gives the bilinear form
\[
  \varepsilon \;\leq\; C_1 \sigma_\ell \sqrt{N (W_N + 1) \log(2/\delta)}
  + C_2 B \log(N) \log(2/\delta),
\]
which is the claim. \hfill$\blacksquare$

\emph{Remark 7.25a (regime of dominance).}
In the sub-Gaussian regime ($\varepsilon \ll N \sigma_\ell^2 (W_N+1)
/ (B \log N)$), the first term dominates and the bound scales like
$\sigma_\ell \sqrt{N (W_N+1) \log(2/\delta)}$. In the Bernstein-tail
regime (very small $\delta$ relative to $\sigma_\ell^2 / B^2$), the
second term dominates and the bound scales like $B \log N \log(2/\delta)$.
For the enwik9 example at $\delta = 10^{-6}$, the sub-Gaussian regime
holds and the bound is determined entirely by $\sigma_\ell$, not by
$B$. This is the source of Theorem 7.25's improvement over Theorem 7.14.

\emph{Remark 7.25b (relation to Theorem 7.14 and 7.15).}
Theorem 7.14 uses Azuma's inequality on the Doob martingale of
$L^{\mathrm{rep}}$ with the worst-case per-step bound
$(W_N + 1 + \rho/(1-\rho)) \log(1/\eta)$, which is rigorous but
uses $B$ rather than the variance proxy $\sigma_\ell$. Theorem 7.25
replaces Azuma by Bernstein for the per-position sequence $(\ell_i)$,
yielding the variance-aware scaling $\sigma_\ell \sqrt{N(W_N+1)}$ vs.\
$B(W_N+1)\sqrt{N}$ --- an improvement by factor $B / \sigma_\ell
\approx 16$ for English text. The end-to-end Chinchilla-class
prediction of Theorem 7.15 thus admits a tighter rigorous concentration
band: the $1.4\,\mathrm{GB}$ Azuma worst-case quoted in §11.7 becomes
$1.5\,\mathrm{MB}$ under Theorem 7.25's hypotheses, much closer to the
$\approx 43\,\mathrm{kB}$ practical CLT estimate. Theorem 7.14
remains the operational bound when only the Doob-martingale increments
can be controlled (non-mixing sources where Bernstein hypotheses fail);
Theorem 7.25 is the preferred bound for mixing sources where
$\sigma_\ell$ can be measured or upper-bounded. The end-to-end
Chinchilla-class prediction of Theorem 7.15 thus admits a tighter
rigorous concentration band: the $1.3\,\mathrm{GB}$ Azuma worst-case
quoted in §11.7 becomes approximately $1.3$--$2.6\,\mathrm{MB}$ under
Theorem 7.25's hypotheses (with the precise constant depending on the
specific Merlev\`ede-Peligrad-Rio formulation), within $\sim 60\times$
of the $\approx 42\,\mathrm{kB}$ practical CLT estimate.
...
The 25-theorem §7 framework draws on several established lines outside
...
\item
  ...
  $L^{\mathrm{rep}}$ via Merlev\`ede-Peligrad-Rio 2009, replacing
  Azuma's worst-case Lipschitz $B$ by the per-position-cost std
  $\sigma_\ell$: for Chinchilla-class enwik9, the rigorous
  $\delta = 10^{-6}$ deviation drops from $\approx 1.3\,\mathrm{GB}$
  (Azuma, T7.14) to $\approx 1.3$--$2.6\,\mathrm{MB}$ (Bernstein, T7.25),
  a $\approx 500\times$ improvement and within $\sim 60\times$ of the
  practical CLT estimate; an earlier draft invoking Samson 2000 on
  $L^{\mathrm{rep}}$ as a separable Lipschitz functional was
  self-rejected on cold review because each coordinate $x_j$ affects
  $(W_N+1)$ terms in the sum, making the direct Samson bound worse
  than Azuma). The §7.24 history
  ...
  All 25 theorems + 2 corollaries verified numerically in
...
\item
  Finite-batch concentration of total decode latency under LRU and
  IRM workloads. Theorem 7.24 (revised) establishes a Hoeffding tail
  bound under the restricted hypotheses of IRM workloads and
  \emph{static} Perfect-LFU caching (no eviction dynamics).
  Adversarial review during the revision cycle (codex 2026-05)
  produced an explicit counterexample showing that LRU miss
  indicators are not iid Bernoulli: the workload
  ``$A, B, A, B, \ldots$'' against an LRU cache of size $C = 1$
  produces $\Theta(Q)$ miss-count deviation rather than the
  $\Theta(\sqrt{Q})$ Hoeffding scaling. A correct finite-batch tail
  bound for LRU under IRM (let alone non-IRM workloads) requires
  separate concentration analysis for the LRU Markov chain over
  cache states, with mixing-time-dependent constants. Foundational
  results in this direction are Fagin (1977) \emph{J.\ Comput.\ Sys.\
  Sci.} 14(2):222--250 ``Asymptotic miss ratios over independent
  references'' and Hendricks (1972) \emph{J.\ Appl.\ Prob.}
  9(1):231--233 on the LRU-stack stationary distribution; King (1971)
  \emph{Proc.\ IFIP Congress 1971}, pp.\ 485--490 gives the classical
  demand-paging algorithm analysis but does not yield finite-batch
  deviation bounds in the form needed for direct application here.
  A clean LRU+IRM tail bound matching Theorem 7.24's accessibility
