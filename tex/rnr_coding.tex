...
is a pre-trained \emph{shared} predictor: choose a coded set $S$ of
sub-block positions, code $X_S$ and reconstruct $X_{\bar S}$ from $M'$;
with the ideal predictor the per-block residual is, by the chain rule,
\[
C_{\mathrm{sel}}(S) \;=\; H(X_S\mid X_{\bar S})
\;=\; H(X) - M(\bar S), \qquad M(T):=H(X_T),
\]
the exact discrete mirror of Lemma 6.9's $h(X_S\mid X_{\bar S})=\tfrac12
\log_2\det(2\pi e\,\Sigma_{S\mid\bar S})$ with Shannon entropy replacing
differential entropy / log-det, so minimising $C_{\mathrm{sel}}$ over
size-$|\bar S|$ reconstruction sets is again a maximum-entropy-sampling
(MESP) selection. Four facts (each verified numerically in
...
is a difference $H(X)-M(\bar S)$ (V2).
...
\emph{sufficient} condition here, not shown necessary: for an \emph{arbitrary}
subset of an order-$w\ge 2$ chain the order-$1$ reduction of (a) provably
fails (the induced process is hidden-state, witnessed by
$I(X_{t_k};X_{t_{k-2}}\mid X_{t_{k-1}})>0$ in V3b$'$), but whether its exact
block entropy nonetheless admits a poly-time algorithm we leave \emph{open} ---
this noncontiguous bounded-order case sits between the poly endpoints (a)/(b)
and the \#P-hard general case (c), and our construction settles neither side
for it;
