...
encoder-chosen byte positions, byte offsets stored explicitly per
entry, used when token-edge alignment or other encoder constraints
require it). Under non-uniform spacing each entry additionally
stores a $\lceil \log_2 |X| \rceil$-bit source position, raising
the seek index size to
...
arithmetic-code length is bounded above by $\log_2(1/\eta)$. The per-sync-point excess cost
...
|\tau|$ as detailed above. This is a capability that a capability that
