...
lower bound Theorem 6.2). A 27-theorem byte-granular random-access
framework (§7) establishes Shannon-rate achievability with
$K = \Theta(\sqrt{N})$ optimal sub-block sizing (Theorems 7.2--7.5),
matched by a tight space-time product converse $C(K) \cdot S(K) =
\Theta(N \log N \cdot \mathrm{cost}(M))$ for sub-block-based
decoders (Theorem 7.27),
extends to universal, Slepian--Wolf, lossy, successive-refinement,
and functional RNR variants (Theorems 7.6--7.12), proves
concentration bounds for archive-size deviation via Azuma's
martingale inequality (Theorem 7.14) and a variance-aware
Bernstein refinement (Theorem 7.25, replacing the worst-case
Lipschitz $B$ by the per-position-cost std $\sigma_\ell$ for an
approximate $500\times$ tightening on enwik9), and
synthesises an end-to-end prediction for Chinchilla-class neural
LMs ($\approx$79\,MB on enwik9 at 0.664\,bpb; Theorem 7.15, with
an explicit model-overhead caveat noting that the compression
ratio excludes the $\approx$140\,GB model).
Practical extensions cover parallel encoding with linear speedup,
streaming, cache-aware decoding under Zipf workloads, fail-safe
store mode, per-sub-block integrity via PRF-based MAC, and
LRU+IRM finite-batch concentration via Lezaud's Markov-chain
Chernoff bound (Theorems 7.16--7.26). The framework's deployment-relevant feature is random-access decoding
with cost
...
\item
  ...
  parameter values.
\item
  A 26-theorem RNR-with-random-access framework (Section 7.1--7.26)
  that bridges the abstract framework to a practical neural-LM-based
  compression pipeline. Theorems 7.2--7.5 establish the basic
  byte-granular random-access architecture; Theorems 7.6--7.10 cover
  source-coding extensions (universal online with KT-style regret,
  successive refinement, functional compression via Orlitsky--Roche
  characteristic-graph entropy, Shannon rate--distortion lossy mode,
  Slepian--Wolf / Wyner--Ziv side-information composition); Corollary
  7.11 records the standard observation that lossy compression with
  an outer error-correcting code recovers losslessness up to the
  ECC rate gap; Theorems 7.12--7.13 cover multi-source joint coding
  via Lauritzen--Spiegelhalter junction-tree entropy decomposition
  and tree-MRF adaptive sub-block sizing; Theorem 7.14 establishes
  Azuma--Hoeffding concentration of the repair-stream length
  $L^{\mathrm{rep}}$ around its expectation, with deviation factor
  $(W_N+1+\rho/(1-\rho)) \cdot \log_2(1/\eta)$ tracking
  predictor-context plus source-mixing-tail contributions; Theorem 7.15
  is the explicit end-to-end synthesis with the
  Chinchilla-class numerical prediction $\approx 79\,\mathrm{MB}$ on
  enwik9 (12.0$\times$ bitstream rate, matching Delétang et al.\
  2024 Table 1); Theorem 7.16 gives adaptive per-region $K_r$ for
  piecewise-stationary sources; Theorem 7.17 covers content-defined
  chunking for insertion/deletion robustness; Theorems 7.18--7.19 cover
  parallelizable encoding and $O(K)$-memory streaming encoder;
  Theorem 7.20 quantifies cache-aware decoder amortization under
  workload locality with explicit Zipf static-top-cache hit-rate
  asymptotics; Theorem 7.21 establishes the predictor-archive
  size trade-off via the Hoffmann 2022 scaling law $\alpha \approx 0.34$
  giving sub-linear optimal $L^*(N) = \Theta(N^{1/(1+\alpha)})$;
  Theorem 7.22 is the fail-safe-with-store-mode worst-case bound
  $L^{\mathrm{rep}} \leq N \log_2|\Sigma|$ under any source and
  predictor; Theorem 7.23 adds per-sub-block authenticated-encryption
  integrity via PRF-based MAC at $O(\tau \cdot m)$ overhead with
  cryptographic forgery resistance; and Theorem 7.24 (revised after
  retraction of an earlier LRU-based version) gives a minimal honest
  Hoeffding tail bound on total decode latency under the restricted
  hypotheses of IRM workload, static Perfect-LFU cache, and bounded
  per-miss cost. The full framework has been adversarially cross-
  validated across three independent model families (Claude / Gemini
  Pro 3.1 / OpenAI gpt-5.5) with all surviving statements verified
  numerically in \texttt{scripts/verify/thm\_7\_*}.
\item
...
rather than through standalone worked examples. Section 7 covers
composition, mode selection, runtime parameter selection (§§7.1--7.2),
and the 27-theorem RNR-with-random-access framework
(§§7.1--7.27, organized as: §§7.2--7.5 basic byte-granular
random-access architecture; §§7.6--7.11 source-coding extensions
including the lossy + ECC = lossless corollary; §§7.12--7.15
multi-source, concentration, and end-to-end Chinchilla-class
numerical synthesis; §§7.16--7.26 practical extensions covering
adaptive sub-block sizing, content-defined chunking, parallel and
streaming encoders, cache-aware decoder, predictor-archive scaling,
fail-safe store-mode fallback, cryptographic integrity, IRM-restricted
decoder-latency tail bound, and LRU+IRM Markov-chain Chernoff
concentration; §7.27 sub-block space-time product converse matching
the T7.5 achievability). Section 8 states the
...
including new questions exposed by the §7 expansion such as
LRU+IRM finite-batch concentration and tightened concentration under
slow mixing), and items explicitly deferred to engineering or empirical
work (§13.3).
...
\begin{itemize}
\item Fixed $K$, $N \to \infty$: $\mathrm{TC}(D_N) \to \log_2(K!) -
...
large $K$.
\item $K = \alpha N$ with $\alpha \in (0, 1)$ constant:
$\mathrm{TC}(D_N) = \tfrac{1}{2} \log_2(2 \pi N \alpha (1-\alpha))
+ O(1/N) = \tfrac{1}{2} \log_2 N + O(1)$.
\end{itemize}
\emph{Both regimes are sub-linear: $\mathrm{TC}(D_N) = o(N)$.}}
...
with rigorous concentration bands of $\pm 1.3\,\mathrm{GB}$
...
$(W_N + 1 + \rho/(1-\rho))$-bound and its codex-driven correction
...
$\approx 1.3\,\mathrm{GB}$ --- a $\approx 300\times$--$630\times$
improvement.
...
in the deviation scale (or equivalently $\approx 300\times$--$630\times$
...
concentration band $\pm 1.3\,\mathrm{GB}$ (Azuma, T7.14) or
...
\]
\emph{The bound is invariant in $K$: increasing $K$ trades $C(K)$ up
linearly while reducing $S(K)$ reciprocally, and vice versa. The
geometric-mean balance point $K^{**} = \sqrt{N \cdot \log_2 N /
\mathrm{cost}(M)}$ (worst-case parameters, no operational
...
sequential predictor evaluation entirely, at the cost of
abandoning the Shannon-near rate of Theorem 7.2. The general
converse (across all random-access schemes, not just sub-block) is
listed as an open problem in §13.2.
...
\rceil$ sync points, the sync-overhead storage $S(K)$ satisfies
\[
  S(K) \;\geq\; m \cdot \bigl\lceil \log_2 L(R) \bigr\rceil
  \;\geq\; \frac{N}{K} \cdot \bigl\lceil \log_2(N \cdot h_{\min})
  \bigr\rceil .
\]

\emph{Proof of Lemma 7.27b.} By Definition 10.3 the seek index has
exactly $m = \lceil N/K \rceil$ entries, each storing a bit-offset
pointer into $R$ that must distinguish among $L(R) + 1$ possible
values (offsets in $\{0, 1, \ldots, L(R)\}$). By Shannon's noiseless
source-coding lower bound (Cover-Thomas 2006, Theorem 5.3.1) applied
to an injective mapping from $m$ distinct sync-point indices to
$L(R) + 1$ offset values, the worst-case index size is at least
$m \cdot \lceil \log_2(L(R) + 1) \rceil$ bits (or $m \cdot \log_2 m
+ m \cdot \log_2((L(R)+1)/m)$ in the average case via succinct
encoding --- Munro-Raman 2001 \emph{SIAM J.\ Comput.}\ 31(3):762 ---
but the worst-case-per-entry bound is $\lceil \log_2 L(R) \rceil$).
Under the Theorem 7.2 rate hypothesis $L(R) \geq N \cdot h(X)$, and
$h(X) \geq h_{\min} > 0$, so $L(R) \geq N \cdot h_{\min}$ and
$\lceil \log_2 L(R) \rceil \geq \lceil \log_2(N \cdot h_{\min})
\rceil = (1 - o(1)) \log_2 N$. \hfill$\square$

\emph{Combining Lemmas 7.27a and 7.27b.} Multiply the two lower
bounds:
\begin{align*}
C(K) \cdot S(K) &\;\geq\; \bigl(K \cdot \mathrm{cost}(M)\bigr)
...
for any integer $K \geq 1$. The product is $K$-invariant: the factor
...
$C(K) \cdot S(K) \geq (1 - o(1)) N \cdot \mathrm{cost}(M) \cdot \log_2 N$.
...
\emph{Non-sub-block converse (open).} The theorem applies only to
...
listed as an open problem in §13.2: the lower bound there is
presently information-theoretic --- any scheme must store at least
$N \cdot h(X) + o(N)$ bits per Theorem 7.2 --- but a matching
time-space product bound across general decoders is not pinned
down. A plausible conjecture: the product
$(\text{query work}) \cdot (\text{total side storage})$ is bounded
below by $\Omega(N \cdot \mathrm{cost}(M))$ for any decoder attaining
Shannon-near rate, with Theorem 7.27 being the sub-block-restricted
version with an additional $\log N$ factor from the seek-index
identifier-length floor; verifying or refuting this conjecture
remains open.
...
\textbf{11.7 Large-scale neural predictors (Theorem 7.15 instantiation)}

The end-to-end synthesis Theorem 7.15 gives an \emph{explicit
...
The 26-theorem §7 framework draws on several established lines outside
...
\item
  ...
  numerical end-to-end synthesis --- resolved by the §7.2--7.24
  framework (24 theorems + 2 corollaries):
  ...
  deviation drops from $\approx 1.3\,\mathrm{GB}$ (Azuma, T7.14) to
  $\approx 2.1$--$4.2\,\mathrm{MB}$ (Bernstein, T7.25), a
  $\approx 300\times$--$630\times$ improvement and within
  ...
  T7.24's static-LFU formulation sidestepped). The §7.24 history
  (LRU version retracted under codex disproof, rewritten under
  restricted hypotheses, survived round 2) illustrates the
  validation discipline applied throughout.
  All 26 theorems + 2 corollaries verified numerically in
  \texttt{scripts/verify/thm\_7\_*} and adversarially
  cross-validated by three independent model families
  (Claude / Gemini Pro 3.1 / OpenAI gpt-5.5).
\end{itemize}
...
\item
  ...
  counterexample of codex 2026-05 against $C=1$ --- remains
...
\item
  ...
  (\emph{Theorem 7.27 sub-block restriction}). Theorem 7.27
  establishes a tight product bound $C(K) \cdot S(K) \geq
  (1 - o(1)) N \cdot \mathrm{cost}(M) \cdot \log N$ for any
  sub-block-based random-access scheme satisfying
  Definition 10.3. The converse for the \emph{general} random-access
  class --- arbitrary decoders that need not partition the archive
  into sub-blocks --- is open. The natural candidates for such
  schemes are succinct-data-structure (Munro-Raman 2001) or
  cell-probe (Pătraşcu-Demaine 2006 \emph{SIAM J.\ Comput.}\
  35(4):932; Larsen 2012 \emph{52nd FOCS}, pp.\ 293--301)
  decoders with non-sequential predictor invocation, and the
  open question is whether such schemes can strictly beat the
  $N \cdot \mathrm{cost}(M) \cdot \log N$ product lower bound
  while still achieving the Theorem 7.2 Shannon-near rate
  $L^{\mathrm{rep}}_{\mathrm{RNR}} = N h(X) + o(N)$. A negative
  answer (impossibility) would close the random-access converse
  in full generality; a positive answer (constructive example)
  would refine Theorem 7.27's scope and motivate a new
  optimization regime beyond the $K = \sqrt{N}$ sub-block balance.
\end{itemize}
...
\item
  ...
  adaptive predictors; Theorems 7.16--7.18 additionally require that
  sub-block-parallel encoding produce bit-identical archives regardless
  of thread count or scheduling order. A reference implementation
  meeting all three, plus a conformance test suite that verifies
  cross-platform identity on a representative corpus, is required for
  deployable archives. This is software engineering work, not
  mathematics. Out of scope for this paper; the natural artifact is a
  separate code release accompanied by a conformance specification.
\item
...
demonstrably failed (multi-LLM adversarial validation 2026-05; codex
counterexamples and Gemini structural critiques), we explicitly
...
(c)~\emph{Predictor absorption applies to the MSGP grammar
itself, not only to the reduction algorithm.} The OP2(b)
amortised objective is $L_{\mathrm{amort}}(M,D,\{R_i\}) :=
L(M) + L(D) + \frac{1}{N} \sum_{i=1}^{N} L(R_i \mid Y,M,D)$,
not the MSGP grammar size $|G_{\mathrm{MSGP}}|$. The naive
transfer requires $|G_{\mathrm{MSGP}}| \leq c \cdot N \cdot
L_{\mathrm{amort}}$ for some constant $c$, but a free predictor
$M$ of size $L(M) = O(\mathrm{poly}(n))$ suffices to memorise
the entire optimal $k$-MSGP grammar for the family --- because
the family is poly-time samplable and the optimal grammar is a
deterministic function of the family description $d_n$
($|d_n| = \mathrm{poly}(n)$). Set $M$ to encode $d_n$ together
with the poly-time algorithm computing $G^* = G_{\mathrm{MSGP}}^*(
\mathcal{F}_n)$ from $d_n$; then $L(R_i \mid Y, M, D) =
O(\log N)$ (index $i$ into $G^*$). The MSGP grammar-size gap
$\gamma_{\mathrm{SGP}} = 8569/8568$ on $|G_{\mathrm{MSGP}}|$
\emph{vanishes} on $L_{\mathrm{amort}}$: the optimal MSGP
solution and any $(8569/8568 - \varepsilon)$-approximation
$G'$ differ by a multiplicative factor on $|G_{\mathrm{MSGP}}|$,
but $L(M)$ stores the grammar in $O(\mathrm{poly}(n))$ bits
\emph{regardless of which approximation $G$ vs.\ $G'$ is fixed
inside $M$}, since both have description $O(\mathrm{poly}(n))$
bits when the family is poly-time-samplable. The amortised gap
collapses to $1 + O(\log(1/\varepsilon)/N) \to 1$ as $N \to
\infty$.

\textbf{Lemma 13.4.2a (unified amortised-objective absorption).}
\emph{Let $\Pi$ be a minimisation problem on poly-time-samplable
input families $\mathcal{F}_n = \{x_i\}_{i \in [N(n)]}$ with
$N(n) = 2^{\Theta(n)}$ and $|d_n| = \mathrm{poly}(n)$ bits of
family description. Suppose $\Pi$'s output $G^*(\mathcal{F}_n)$
has size $|G^*| \leq \mathrm{poly}(N \cdot n)$ and is computable
from $d_n$ in time $\mathrm{poly}(N \cdot n)$. Then for any
multiplicative constant $\gamma > 1$, the amortised-family
bit-cost objective
$L_{\mathrm{amort}}(M,D,\{R_i\}) := L(M) + L(D) + \frac{1}{N}
\sum_{i=1}^{N} L(R_i \mid Y, M, D)$
admits a free predictor $M^*$ with $L(M^*) = O(\mathrm{poly}(n))$
such that the multiplicative gap between
$L_{\mathrm{amort}}(M^*, \cdot, \cdot)$ achieved by $M^*$
encoding the optimal $G^*$ versus $M'$ encoding any
$\gamma$-approximation $G'$ of $G^*$ is $1 + O(\log \gamma /
N) \to 1$ as $N \to \infty$. Hence APX-hardness of $\Pi$ with
any constant gap $\gamma$ on the $\Pi$-objective does
\emph{not} transfer to APX-hardness of the amortised-family
bit-cost objective.}

\emph{Proof sketch.} Let $G^*$ be the optimal $\Pi$-output on
$\mathcal{F}_n$. Define $M^*$ as the deterministic poly-time
algorithm that hard-codes $d_n$ and on input $i \in [N]$
computes $G^*$ from $d_n$ in $\mathrm{poly}(N \cdot n)$ time,
then outputs the $i$-th component. Then $L(M^*) = O(|d_n|) +
O(1)$ (for the poly-time algorithm description) $= O(\mathrm{poly}(n))$
bits, and $L(R_i \mid Y, M^*, D) = O(\log N)$ (the index $i$).
Total: $L_{\mathrm{amort}}(M^*, \cdot, \cdot) =
O(\mathrm{poly}(n)) + 0 + \frac{1}{N} \cdot N \cdot O(\log N) =
O(\mathrm{poly}(n) + \log N)$. For a $\gamma$-approximation
$G'$ (which still has $|G'| \leq \gamma \cdot |G^*| =
\mathrm{poly}(N \cdot n)$), the analogous $M'$ has $L(M') =
L(M^*) + O(\log \gamma)$ since the only difference is which
$\gamma$-approximation algorithm is invoked, and any standard
$\gamma$-approximation has poly-time description of length
$O(\log \gamma) + O(1)$. The amortised values differ by an
additive $O(\log \gamma)$, giving a multiplicative gap on
$L_{\mathrm{amort}}$ of $1 + O(\log \gamma / (\mathrm{poly}(n)
+ \log N)) \to 1$ as $N \to \infty$ for any fixed
$\gamma$. $\Box$

\emph{Consequence (unification of all three attack vectors).}
Lemma~13.4.2a unifies items~1, 2, and the present item~3 under
a single structural obstacle: \emph{poly-time-samplable families
admit poly-time predictors that absorb any optimisation gap of
polynomial-size structure into $L(M)$, leaving the amortised
gap at $1+o(1)$.} Item~1 (predictor absorbs the reduction
algorithm $d_n$), item~2 (predictor absorbs the salting
function), and item~3 (predictor absorbs the optimal MSGP
grammar $G^*$, which is itself poly-time-computable from
$d_n$) are three instantiations of the same lemma. The
``GSGP is the only non-absorption attack'' characterisation of
the previous status update is incorrect: MSGP collapses one
level deeper than items~1--2, because the predictor absorbs
the optimal solution to the optimisation problem itself.
\emph{Verdict (closure of attack vector~3).} Attack vector~3
(GSGP / multi-string SGP) is closed negatively. The MSGP
grammar-size APX-hardness (via separator reduction to
single-string SGP and Casel et al.\ 2020) holds but does not
transfer to the amortised-bit-cost objective. The remaining
worst-case combinatorial route to OP2(b)-APX is the
cryptographic route via Liu--Pass / OWF (§13.2), which yields
average-case decision hardness, \emph{not} worst-case
multiplicative APX-hardness. No worst-case combinatorial-poly\-no\-mial-time route to OP2(b)-APX is currently known;
Lemma~13.4.2a is the structural obstacle across all three
attack vectors of this list.
\end{enumerate}
...
\emph{Updated 2026-05-28:} after the negative results on items
1--3 of the attack-vector list (predictor absorption of the
reduction algorithm, of the salting function, and of the optimal
MSGP grammar, respectively, all unified by Lemma~13.4.2a), no
combinatorial-poly\-no\-mial-time route in this list survives.
...
distinct strings in an ambiguous CFL is \#P-hard in general, since
$\#$DNF-SAT reduces to it (Bertoni-Goldwurm-Sabadini 1991, \emph{Theoret.\
Comput.\ Sci.}\ 86:373--391). Most natural data classes admit
...
claims of closure that did not survive adversarial multi-LLM
validation (notably: K-restricted ``closures'' that excluded the
...
\textbf{{[}51{]}} Ar{\i}kan, E. (2009). Channel Polarization: A Method
...
\textbf{{[}52{]}} Azuma, K. (1967). Weighted Sums of Certain Dependent
Random Variables. T\^ohoku Mathematical Journal, 19(3), 357-367.

\textbf{{[}53{]}} Bellare, M. (2006). New Proofs for NMAC and HMAC:
...
\textbf{{[}54{]}} Bellare, M., Canetti, R., and Krawczyk, H. (1996).
...
\textbf{{[}55{]}} Bellare, M., and Namprempre, C. (2000). Authenticated
...
\textbf{{[}61{]}} Cybenko, G. (1989). Approximation by Superpositions
of a Sigmoidal Function. Mathematics of Control, Signals, and Systems,
2(4), 303-314.

\textbf{{[}62{]}} Del\'etang, G., Ruoss, A., Duquenne, P.-A., Catt, E.,
...
\textbf{{[}63{]}} Doshi, V., Shah, D., and M\'edard, M. (2007). Source
...
\textbf{{[}76{]}} McLeish, D. L. (1974). Dependent Central Limit
Theorems and Invariance Principles. Annals of Probability, 2(4),
620-628.

\textbf{{[}77{]}} Muthitacharoen, A., Chen, B., and Mazi\`eres, D.
...
\textbf{{[}105{]}} Birch, B. J. (1962). Approximations for the Entropy
for Functions of Markov Chains. Annals of Mathematical Statistics,
33(3), 930-938.

\textbf{{[}106{]}} Manurangsi, P. (2017). Almost-Polynomial Ratio
