...
\emph{Remark on the $(W_N+1)$ factor.} The $(W_N+1)$ factor in the
bound above arises specifically from the Doob-martingale construction
$M_n := \mathbb{E}[L^{\mathrm{rep}} \mid \mathcal{F}_n] - \mathbb{E}[L^{\mathrm{rep}}]$,
whose differences $D_n = \mathbb{E}[L^{\mathrm{rep}} \mid \mathcal{F}_n]
- \mathbb{E}[L^{\mathrm{rep}} \mid \mathcal{F}_{n-1}]$ are bounded by
$(W_N+1) \log_2(1/\eta)$ because $X_n$ enters the context of $\ell_n,
\ell_{n+1}, \ldots, \ell_{n+W_N}$. An alternative construction --- the
compensated martingale $\tilde{M}_N = \sum_{k=1}^N (\ell_k -
\mathbb{E}[\ell_k \mid \mathcal{F}_{k-1}])$ --- has Azuma increments
bounded by $\log_2(1/\eta)$ without the $(W_N+1)$ factor, but
concentrates a different quantity (a centered sum, not
$L^{\mathrm{rep}} - \mathbb{E}[L^{\mathrm{rep}}]$). For the stated bound
on the deviation of the encoder output around its expectation, the
$(W_N+1)$ factor is the operational consequence of the
bounded-differences inequality (McDiarmid 1989); under exp-mixing the
factor may be loose but is non-removable for the Doob construction.
...
$1\,\mathrm{GB}$ enwik9 source --- yielding a \emph{negative} stand-alone
compression ratio for a single archive of this size. The headline
...
\item \emph{(Zipf hot-key with skew $\alpha$)}: For the static-popularity
LRU hit rate under Zipf rank distribution $p_k \propto k^{-\alpha}$ over
$m$ items (Breslau et al.\ 1999 \emph{Proc.\ IEEE INFOCOM},
``Web caching and Zipf-like distributions: evidence and implications''),
the hit rate is the cumulative probability of the top-$C$ items
$\rho_C = \sum_{k=1}^C p_k / \sum_{k=1}^m p_k$, with two asymptotic
regimes: \emph{(heavy-tailed)} $0 < \alpha < 1$ gives
$\rho_C \approx (C/m)^{1-\alpha}$ (so $\rho_C \to 0$ as $m \to \infty$
for fixed $C/m$ small); \emph{(light-tailed)} $\alpha > 1$ gives
$\rho_C \to 1 - O(C^{1-\alpha} / \zeta(\alpha))$ saturating near $1$
as $C \to \infty$ (the typical web-cache regime, $\alpha \in [0.7,
1.1]$ in Breslau measurements; values near $\alpha = 1$ require
the $\alpha = 1$-harmonic limit
$\rho_C \approx \log C / \log m$).
For Ferragut et al.\ 2017 \emph{Queueing Systems} 88:207, see their
Theorem~3 for the general timer-based-cache hit-rate analysis under
general arrival processes (which includes LRU as a limit).
...
$\alpha = 1.5$ (heavily skewed, lighter-than-Breslau-empirical) and
$m = 10^6$, $C = 100$: $\rho_C \approx 1 - C^{1-\alpha}/\zeta(\alpha)
\approx 1 - 100^{-0.5}/2.61 \approx 0.96$ (96\% hit rate). For the
typical Breslau measurement range $\alpha \in [0.7, 0.85]$ (heavy-
tailed web traces), $\rho_C \approx (C/m)^{1-\alpha}$: at $C=100, m=
10^6, \alpha=0.8$ this gives $\rho_C \approx (10^{-4})^{0.2} \approx
0.16$ (16\% hit rate, modest but still better than uniform $C/m
= 10^{-4}$). Amortized cost reduced by factor $1/(1-\rho_C)$, e.g.\
$\sim 25\times$ for $\alpha=1.5$ regime, $\sim 1.2\times$ for the
$\alpha=0.8$ heavy-tailed web-trace regime.
