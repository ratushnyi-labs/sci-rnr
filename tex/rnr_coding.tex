...
attainability is currently provable.
...
reduced below $(m-1) \cdot \delta_\infty$? The answer depends on
whether global-header amortization can ``leak'' sub-block-correlation
information across sync boundaries; we leave this as an open question
in §13 (Open Problems).
...
\item
  ...
  \emph{Padding-bit calibration (new direction, 2026).} A
  multi-agent brainstorm during the present manuscript revision
  identified a novel attack angle that neutralises the
  injective-labeling obstacle without trying to evade it: reduce
  from Min-$2$-CSP via Baby-PIH gap-amplification
  (Guruswami--Ren--Sandeep, CCC 2024,
  drops.dagstuhl.de/entities/document/10.4230/LIPIcs.CCC.2024.27)
  with hypercube alphabet $\{0,1\}^k$ native to $E_{12}$, taking
  $F_{v_x, v_y} = 1/\deg(v_x)$ (the row-stochastic constraint-incidence
  Markov kernel of $\Psi$, satisfying row-stochasticity \emph{by
  construction} rather than retrofit). Take $m = k + \lceil \log_2 n \rceil$:
  the first $k$ bits encode the CSP assignment (where the constant-gap
  lives), the last $\lceil \log_2 n \rceil$ bits encode the vertex ID
  (fixed by the reduction). Since every vertex pays exactly its
  $\log n$ identifier-Hamming cost uniformly, the
  injective ``packing noise'' becomes a labeling-independent additive
  constant rather than a gap-erasing background. Parallel-repetition
  amplification of the Min-$2$-CSP gap is enabled by the row-uniform
  graph structure post-repetition. The composition with a
  Hadamard-style projection (or ghost-vertex slack relaxation from
  the same brainstorm) converts the CSP gap into an $E_{12}$
  constant-Hamming gap. The two structural obstacles (row-stochastic
  $F$ + injective labeling) are circumvented \emph{simultaneously}
  by the same construction: the first because $F$ is row-stochastic
  natively from $\Psi$'s incidence kernel; the second because the
  identifier bits act as a uniform calibration offset rather than
  a counted constraint. This is the cleanest currently-known
  research path; full formalisation is beyond the scope of this paper.
