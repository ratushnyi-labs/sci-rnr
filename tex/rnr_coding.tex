...
\Delta_{m-1} < |X|$ subject to three invariants: (i) the first
sync point $\Delta_0 = 0$ (so every query position $p \in [0, |X|)$
has some prior sync point); (ii) maximum gap
$\Delta_{j+1} - \Delta_j \leq K$; (iii) minimum gap
$\Delta_{j+1} - \Delta_j \geq K/2$ between consecutive sync-point
positions, byte offsets stored explicitly per entry, used when
token-edge alignment or other encoder constraints require it). The max-gap invariant ensures that the latest sync
...
(Definition 10.3). So the worst-case total is at most
$(|X|/K) \cdot S \cdot W \cdot \log_2(1/\eta)$ bits under uniform
spacing or $(2|X|/K) \cdot S \cdot W \cdot \log_2(1/\eta)$ bits
under non-uniform spacing, with $S$ as defined above ($S = 1$ for
Type-I and direct byte arithmetic, $S = 2$ for per-byte sequential
Type-III-B).
...
(Section 10.7), giving a worst-case total of $(|X|/K) \cdot S \cdot
W \cdot \log_2(1/\eta)$ bits over the archive for uniform spacing
(or $(2|X|/K) \cdot S \cdot W \cdot \log_2(1/\eta)$ for non-uniform
under Definition 10.3, where $S$ is
