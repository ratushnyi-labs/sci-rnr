...
and zero under causal-context modes (Theorem 6.2b). A 27-theorem
...
\tag{6.2b.1}
\]
\emph{Moreover the \emph{committed-mode per-tile floor}
$R_{\mathrm{tile}} := \sum_t \min_m \mathbb{E}[L_m(t)]$ --- the minimum
expected length when one mode is committed per tile from its
decoder-reproducible context (the operative flag-free regime of §6.2,
where the predictor fixes $\hat m(t)$ ahead of the data) --- satisfies the
chain}
\[
R_{\mathrm{tile}}
...
\emph{per-tile-expressive} --- for each tile some mode attains
$\mathbb{E}[L_m(t)] = H(X_t \mid C_t)$ up to the coder redundancy
$\varepsilon$; (b) the second is equality iff every tile's mode context
is \emph{causally sufficient}, $H(X_t \mid C_t) = H(X_t \mid X_{<t}, Y)$
for all $t$; and (c) the last step is the chain rule, exactly. The total
slack above the floor is the \emph{context deficit}}
\[
...
inter-tile total correlation $\sum_t H(X_t \mid Y) - H(X \mid Y) =
\mathrm{TC}(X_1,\ldots,X_T \mid Y)$. Consequently Type-III-A attains the
absolute floor $H(X\mid Y)$ (within the $2\sum_t \varepsilon_t$ of
Theorem 6.1) \textbf{iff the modes are per-tile-expressive AND causally
context-sufficient}; otherwise it overpays by the mode-inexpressiveness
gap plus the context deficit.}
...
(6.2b.2), each tile is encoded by a single mode whose context
$C_t^{(m)} \subseteq (Y, X_{<t})$ is decoder-reproducible. A
uniquely-decodable mode satisfies $\mathbb{E}[L_m(t)] \ge
H(X_t \mid C_t^{(m)}) \ge H(X_t \mid X_{<t}, Y)$, the second step because
conditioning on the larger set $(Y, X_{<t}) \supseteq C_t^{(m)}$ cannot
increase entropy; hence $\min_m \mathbb{E}[L_m(t)] \ge
H(X_t \mid X_{<t}, Y)$. Summing and applying the chain rule
$\sum_t H(X_t \mid X_{<t}, Y) = H(X_1,\ldots,X_T \mid Y) = H(X \mid Y)$
gives the floor. Writing $C_t$ for the context of the cost-minimizing
mode at tile $t$, the first inequality of (6.2b.2) is equality iff that
mode is expressive at $C_t$, and the second iff $C_t$ is causally
...
by definition (Watanabe 1960). \hfill$\blacksquare$

\emph{Remark 6.2b$'$ (why the committed-mode floor, not the hindsight
oracle of Theorem 6.1, is the operative rate).} Theorem 6.1 bounds the
realized cost against the \emph{hindsight} envelope $\sum_t \min_m
L_m(t)$, in which the mode is chosen after seeing the tile. Its
expectation obeys $\mathbb{E}[\sum_t \min_m L_m(t)] \le \sum_t \min_m
\mathbb{E}[L_m(t)] = R_{\mathrm{tile}}$ (because $\mathbb{E}[\min] \le
\min[\mathbb{E}]$), and can in fact dip \emph{below} $H(X\mid Y)$: the
hindsight envelope is not by itself a uniquely-decodable code, since the
realized selection $m^\star(t)$ must be communicated. The selection
information --- the per-tile mode flags charged whenever the encoder
departs from the predicted $\hat m(t)$ --- is exactly what lifts the
realized $\mathbb{E}[L_{\mathrm{IIIA}}]$ back to $\ge H(X\mid Y)$ via
(6.2b.1). The operative Type-III-A of §6.2 commits to the predictor's
$\hat m(t)$ ahead of the data, so its expected cost is at least
$R_{\mathrm{tile}} \ge H(X\mid Y)$; the absolute floor and the
context-deficit characterization therefore bind the operative scheme
directly, independent of the optimistic hindsight envelope. The
optimality characterization follows by comparing this committed-mode
cost (and the additional $2\sum_t \varepsilon_t$ selection loss of
Theorem 6.1) with the floor.
