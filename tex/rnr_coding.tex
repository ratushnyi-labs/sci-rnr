...
\]
breaking ties by lexicographic order on the mode index, and encodes
each tile with the selected mode unless an override is present. The
selection is deterministic and decoder-reproducible. The archive
stores the per-tile residual streams and, when the encoder departs
from the predicted argmin, override flags as metadata.
