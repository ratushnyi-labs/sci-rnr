...
\lceil N/K \rceil$ sub-blocks and $W = W_N = \lceil
\log_{1/\rho}(N C) \rceil$ as in Lemma 5.1b, the total expected length
satisfies}
\[
...
\]
For the $\delta_W^{(h_W)} \leq \delta_\infty$ bound: since
$H(X_i \mid X_{<i}) \geq h_W$ for $i \leq W$ (more conditioning gives
lower entropy, and $X_{<i}$ for $i \leq W$ is a sub-context of
$X_1^W$ used to define $h_W$), each summand is non-negative.
...
summability hypothesis, Lemma 5.6c (b)). Multiplying by $m$ and
comparing to $H(D_N) = N \cdot h(X) + \delta_\infty - o(1)$
(asymptotic form, Lemma 5.6c): the excess is $N \cdot (h_W - h) +
