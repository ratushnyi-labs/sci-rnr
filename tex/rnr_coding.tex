...
that parse (the non-overlapping selected parse frequency, which can
be strictly less than the raw occurrence count when $\tau$
self-overlaps; e.g., for $\tau = $ \texttt{aa} in the string
\texttt{aaa}, the raw count is $2$ but $f = 1$). Let
...
same reason as in Theorem 6.7: the objective changes and the
inclusion is non-trivial.}
...
subblock $i$. Suppose further that the predictor uses a
\emph{context-reset boundary} at each subblock break: the input to
$M$ for subblock $i + 1$ is a function only of public boundary
state (a stored state snapshot in the archive header, or a fixed
context-reset value), not of the recovered bytes of subblock $i$.
Finally, suppose $H_v$ is a cryptographic hash with negligible
collision probability $\delta$ on the considered set of candidate
subblocks. Then the probability that an incorrect $\hat{X}_i$ is
...
Theorem 10.1. Suppose the encoder updates $M$ online via a compatible
online optimizer (Definition 10.1) with gradient computed by
deterministic integer backpropagation. Suppose the order in which
subblocks are used for adaptation is pinned by the model description
and identical on encoder and decoder. Suppose further the
\emph{causal-update condition}: the encoder's adaptation step at
subblock boundary $t$ uses only data already available to the decoder
at that boundary (i.e., bytes from subblocks $0, \ldots, t$ all of
which the decoder has already verified per Theorem 9.1 before
predicting subblock $t+1$). Then at every subblock boundary, the
encoder's and decoder's parameter states are bit-identical, and the
resulting predictor outputs are bit-identical. \emph{Caveat:} without
the causal-update condition (e.g., if the encoder updates using the
current byte before encoding it), the decoder cannot replicate the
same update sequence, and parameter states diverge despite
deterministic backprop and pinned order.}
...
$O(\log(N/K) + (K + k) \cdot \mathrm{cost}(M))$. \emph{This is
polylogarithmic in archive size $N$ only when $K$ and $k$ are held
fixed or grow at most polylogarithmically in $N$; for $K = \Theta(N)$
or $k = \Theta(N)$ the cost is $\Theta(N \cdot \mathrm{cost}(M))$,
no better than full sequential decode}. When $(K+k) \cdot
\mathrm{cost}(M) \gg \log(N/K)$ the second term dominates and the
cost is $O((K+k) \cdot \mathrm{cost}(M))$, independent of $N$ in
...
random access requires storing a snapshot at each sync point of
\emph{all} boundary state that affects $M$'s output for subsequent
positions --- namely the predictor's recurrent hidden state, any
adaptive parameter snapshot (if Theorem 10.2 is in effect), and the
entropy-coder boundary state (range/state for arithmetic coding or
ANS state). Let $L(\text{state})$ be the bit length of this combined
boundary snapshot. The seek index grows to
$O((N/K) \cdot (\log L(R) + L(\text{state})))$ bits. The random-access
cost per query is $O\bigl(\log(N/K) + L(\text{state}) + (K + k) \cdot
\mathrm{cost}(M)\bigr)$, where the $L(\text{state})$ term covers
loading and deserializing the snapshot before decoding can resume.}
...
$(1 + L(\text{state}) / \log L(R))$. To random-access bytes
$[N, N+k)$, locate the latest sync point $P \leq N$ (seek-index
lookup $O(\log(N/K))$), read and deserialize the boundary snapshot
(cost $O(L(\text{state}))$), restore the decoder's predictor,
adaptive-parameter, and entropy-coder boundary states, and decode
forward as in Theorem 10.3 (cost $O((K+k) \cdot \mathrm{cost}(M))$).
...
positions. A block-device driver that maps logical-block-address-
indexed read requests (LBA $\mathcal{L}$, length $k$ bytes) to RNR
random-access calls can serve any read request in time
$O(\log(N/K) + (K + k) \cdot \mathrm{cost}(M))$ and working memory
$O(K + k + L(M) + L(\mathrm{seek\_index}))$, where $L(M)$ is the
predictor description size (kept resident in the driver) and
$L(\mathrm{seek\_index})$ is the seek-index size (typically
$O((N/K) \log L(R))$). The cost is polylogarithmic in archive size
$N$ only in the parameter regime where $K$ and $k$ are held fixed
or grow at most polylogarithmically in $N$; outside that regime the
...
$N$ runs in $O(\log(N/K) + (K + k) \cdot \mathrm{cost}(M))$ time and
requires $O(K + k)$ bytes of decode-window working memory (warmup
buffer plus output buffer). The driver additionally keeps the
predictor weights ($L(M)$ bits) and the seek index resident for
constant-time per-query lookup; these are fixed at archive-mount
time, not per-query. The translation from $(\mathcal{L}, k)$ to
$(N, k)$ is a constant-time arithmetic operation. The total
...
the forward pass. Under (a)-(f), the output $M(C)$ computed on $T$
is bit-identical to that computed on any other conforming target.
The choice of target affects throughput and latency but not the
produced bytes; the engineering spec lists the conformance tests
that establish (a)-(f) for each named target class.}
