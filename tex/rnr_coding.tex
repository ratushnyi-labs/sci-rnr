...
\item
  ...
  $K$: at $K = \mathrm{polylog}(|X|)$ the per-query cost is polylog
  but the total sync overhead is $O(|X|/\mathrm{polylog}(|X|))$
  (sub-linear but not polylog); at $K = |X|/\mathrm{polylog}(|X|)$
  the total overhead drops to $O(\mathrm{polylog}(|X|))$ but
  per-query cost grows to $\Theta(|X|/\mathrm{polylog})$. The two
  cannot be simultaneously polylog. The per-query random-access cost
...
$K = \omega(\log N)$: for $K = \mathrm{polylog}(N)$ the overhead is
$O(N \log N / \mathrm{polylog}(N))$ (sub-linear but \emph{not}
polylog); polylog total overhead would require
$K = N/\mathrm{polylog}(N)$, which is incompatible with polylog
per-query random-access cost. For fixed $K = O(1)$ the overhead is
$\Theta(N \log N \log_2(1/\eta))$, linear in $N$ with a log factor.
...
\item
  ...
  at the polylog-per-query operating point $K = \mathrm{polylog}(|X|)$,
  the total sync overhead is $O(|X|/\mathrm{polylog}(|X|))$ — sub-linear
  in $|X|$ but not polylog. Polylog total overhead would require
  $K = |X|/\mathrm{polylog}(|X|)$, which makes per-query cost polynomial.
