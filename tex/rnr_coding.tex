...
out). A sufficient condition for $E_{\mathrm{HMM}} > 0$ is that
(i) the hidden chain $(Y_n)$ has non-trivial temporal structure
(some transition probability $P_{y y'} \neq \pi_{y'}$) and (ii) the
emission laws are not all identical, so that observations
distinguish hidden states. Identical emissions or iid hidden chain
with each emission depending only on the current state can each give
iid observations and $E_{\mathrm{HMM}} = 0$ (counterexample: take
$P = \pi \mathbf{1}^T$, i.e., $Y_n$ is iid with distribution $\pi$,
combined with any emission law: the marginal $(X_n)$ is then iid).}
...
Functions of Markov Chains}, AMS 33(3): 930--938). The filter
...
i.e., the observed process $(X_n)$ is iid. This can occur in several
non-equivalent ways: (a) all emission laws $\{p_y\}$ are identical,
making $X_n$ iid given $Y_n$ and hence marginally; (b) the hidden
chain $(Y_n)$ is iid (i.e., $P$ has identical rows equal to $\pi$),
making the joint $(Y_n, X_n)$ iid in $n$ and hence $X_n$ iid; or
(c) more generally, the filter $\mu_n$ remains constant in $n$ so
that observations carry no temporal information. The lemma claims
the equivalence with iid $(X_n)$ only; it does not claim that
non-identical emissions alone suffice. \hfill$\blacksquare$
