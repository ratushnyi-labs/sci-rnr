...
\emph{Let $(Y_n)_{n \geq 1}$ be a stationary irreducible aperiodic
Markov chain on a finite hidden state space $\mathcal{Y}$ with
transition matrix $P$ and stationary distribution $\pi$, and let
$(X_n)_{n \geq 1}$ be observations on a finite alphabet
$\mathcal{X}$ conditionally independent given $(Y_n)$ with emission
laws $p_y(x) = \Pr(X_n = x \mid Y_n = y)$. The observation process
...
under $X_n \sim \sum_y \mu_n(y) p_y$. The HMM is informative
(equivalently, $E_{\mathrm{HMM}} > 0$) if and only if the
emission laws $\{p_y\}$ are not all identical; in that case
$\mathrm{TC}(D_N) = \Theta(N)$ with slope $E_{\mathrm{HMM}}$.}
...
i.e., $(X_n)$ is iid, which under conditional iid given $(Y_n)$
forces all $\{p_y\}$ identical (otherwise the prior over $Y$ shifts
based on observations, creating dependence). \hfill$\blacksquare$
