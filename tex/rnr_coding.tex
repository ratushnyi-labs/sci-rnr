...
\item
  ...
  zero on sources without joint structure). Three explicit
  ...
  hidden-class iid sources (latent-topic mixture); and Lemma 5.6c
  ...
  chains (temporal correlation). Together these three lemmas cover
  the three foundational mechanisms by which natural data
  accumulates linear TC. Whether $\mathrm{TC}(D_N) = \Theta(N)$
  extends to further classes (non-stationary chains, hidden Markov
  models, factor graphs over expanders) remains an open empirical
  question.
\item
...
\item
  ...
  sequential rate $H_W(D_N)$ within $O(1)$ of $H(D_N)$, so the
  total length is $H(D_N) + O(1) + \Delta_{\mathrm{sync,total}} =
  H(D_N) + O(\log N \cdot \log_2(1/\eta))$ — a logarithmic, not
  linear, rate gap. The per-query random-access cost is still
  polylog in $|X|$ provided the predictor architecture admits
  context $W = \Theta(\log N)$ at $\mathrm{poly}(\log N)$ per
  evaluation (transformers with windowed attention pay
...
inapproximability gap, attained on a concrete star family).}
\emph{For non-trivial tree instances $(T, m)$ with
$n = |V(T)| \geq 3$ and $\Delta = \max_v \deg_T(v) \geq 1$, the
Theorem 4.3 reduction yields the relative inapproximability ratio}
\[
\frac{\mathrm{OPT}_{\mathrm{NO}}}{\mathrm{OPT}_{\mathrm{YES}}} \geq 1 + \frac{1}{n-1},
\]
\emph{and there exists a NO-instance family (degree-$(n-1)$ stars
$K_{1, n-1}$ with $m = \lceil \log_2 n \rceil$ when
$\lceil \log_2 n \rceil = n - 1$, i.e.\ $n = 2, 4$) on which the
optimal $C_F(E)$ achieves the upper-side bound $B + 2/\Delta$
exactly. The varying-parameter learned-code labeling problem is
therefore NP-hard to approximate within multiplicative ratio
$1 + 1/(n - 1)$, an inverse-linear factor that vanishes as
$n \to \infty$.}

\emph{Proof.} The lower bound $\mathrm{OPT}_{\mathrm{NO}} \geq B +
2/\Delta$: in the Theorem 4.3 reduction the threshold is $B = 2(n-1)/\Delta$;
the YES case (tree embeds in $Q_m$) has $C_F(E) = B$ exactly; in any
NO case Hamming distances on tree edges are positive integers and at
least one edge has $d_H \geq 2$ (otherwise the labeling would be a
hypercube subgraph embedding), contributing an extra $2/\Delta$ to
$C_F$. The ratio is therefore
\[
...
The tight NO-family witness: take $T = K_{1, n-1}$ (star with one
center and $n-1$ leaves) at $n = 4$, $\Delta = 3$, $m = 2$. Here
$|V(T)| = 4 = 2^m$ so an injective labeling exists, but $K_{1,3}$
does not embed in $Q_2$ at $d_H = 1$ on every edge (only $2 = m$
of the $3$ leaves can be at distance $1$ from a fixed center; the
third must be at distance $\geq 2$). Direct computation:
$\mathrm{OPT} = (2/3) \cdot (1 + 1 + 2) = 8/3$, and
$B = 2 \cdot 3 / 3 = 2$, giving ratio $4/3 = 1 + 1/3 = 1 + 1/(n-1)$
exactly (verified in
\texttt{scripts/verify/remark\_4\_3c\_wagner\_corneil\_gap.py}). For
$n = 2$ and $n = 3$ the bound is trivial or has $\Delta = 1$; for
$n \geq 5$ the bound holds but is no longer tight on stars (some NO
instances have a strictly larger gap). As $n \to \infty$ the gap
ratio $1 + 1/(n-1)$ tends to $1$, so no
\emph{constant}-gap inapproximability is obtained from the
Theorem 4.3 reduction alone. \hfill$\blacksquare$

\emph{Implications for OP4.} A constant-gap inapproximability for
$E_{12}$ requires \emph{gap amplification} beyond the
inverse-linear ratio of Theorem 4.3. The three research paths
sketched in §13.2 are studied known mechanisms for such
amplification in linear-code / lattice / hypercube geometries:
(a) PCP-style reduction from APX-hard Label Cover via the
Arora-Babai-Stern-Sweedyk (1997) Nearest Codeword Problem framework;
(b) bounded-distance decoding hardness (Dumer-Micciancio-Sudan
2003); (c) Closest Vector Problem hardness (Khot 2005) specialized
to $\mathbb{F}_2^{12}$. Wagner-Corneil-based amplification via
parallel/product repetition is also a logically valid path but the
required composition with PCP-style verifiers does not appear in the
literature for this source problem; if achievable, it would give
constant-gap inapproximability directly. Closing OP4 in any of
these directions remains research-scale open.
...
$\Delta_{\mathrm{sync,total}} = m \cdot S \cdot W \cdot \log_2(1/\eta)$
quantified above (which is $o(N)$ for typical $W,S$ choices). For
...
\cdot \log_2(1/\eta) = O(m \cdot S \cdot \log N \cdot \log_2(1/\eta))$
is a logarithmic, not linear, function of $N$. The byte-granular
random-access cost from Theorem 10.3 becomes $O(\log L(R) + (K + k)
\cdot \mathrm{cost}(M_{W_N}))$ per query. The per-position
...
logarithmic sync-point overhead. The hypothesis (5.8) is
...
coder leaves $\approx 0.7$ Mbit (order-$1$ bigram approximation) to
$\approx 2.9$ Mbit (long-range) of joint structure unexploited. The
verification script
...
Together Lemmas 5.6a, 5.6b, 5.6c constitute Open Problem 5's
resolution for the three structurally cleanest natural-data
patterns. Further classes (general non-stationary Markov chains,
hidden Markov models, factor graphs over expanders) admit similar
treatments but with strictly more involved bookkeeping; they remain
open as explicit lemmas.
...
multiplicative ratios are not preserved by affine maps in general). The
approximability of either form is open and is tied to the long-standing
open approximability of densest-$k$-subgraph (best known algorithm
$O(n^{1/4 + \varepsilon})$, Bhaskara et al.\ 2010); we record only the
exact-optimization hardness here.
...
naive transfer is unsound: a bit-cost approximation algorithm need
not produce a grammar that approximates $L_{\mathrm{sym}}$, since
the algorithm can trade off $L_{\mathrm{sym}}$ against $K$ (which
affects the log factor) in ways that minimize bit-cost without
minimizing $L_{\mathrm{sym}}$. The exact APX-hardness of the
bit-cost objective for the restricted Type-III-C problem, with
any explicit inherited constant, is therefore an open question
(listed in §13.2), distinct from the symbol-count result of
Theorem 6.8.
...
\item
  ...
  Type-II --- complexity dichotomy fully established. Lemma 5.7a
  establishes that top-$Q$ is provably optimal under the
  permutation-like (conditional Bernoulli) joint law of $M_v$,
  where atom weights factorize as
  $\Pr(M_v = T) \propto \prod_{i \in T} q_i$ for
  ...
  counterexample at $N=7, k=2, s=2$), top-$Q$ can be provably
  suboptimal, and Lemma 5.7c establishes that the
  optimization problem is NP-hard. The reduction is from CLIQUE
  on graphs with $|V_G| \leq 3k+1$ to a combinatorial decision
  ``does some $|S|=s$ have $n_0(S) \geq D$?'' at $N = 3s+1$, then
  to the cost-based problem via the positive-affine collapse at
  $N = 3s+1$ where the algebraic identity $A_s(1) = A_s(2)$ reduces
  the three-way $(n_0, n_1, n_2)$ cost decomposition to a function
  of $n_0$ alone (verified in
  \texttt{scripts/verify/lemma\_5\_7c\_np\_hardness\_clique\_reduction.py}).
  What remains open is the approximation side: the
  positive-affine reduction does not preserve multiplicative
  approximation ratios, so neither hardness nor approximability of
  approximate optimization follows directly; the question is tied
  to the long-standing open approximability of
  densest-$k$-subgraph (best known algorithm
  $O(n^{1/4+\varepsilon})$ via Bhaskara et al.\ 2010).
  The arithmetic-coding benchmark under the same $Q$ is a separate,
  settled result and remains the comparison baseline.
\item
  Hardness of the unrestricted joint Type-III-C problem
  $(M, D, R \mid Y)$ --- open. Theorems 6.7 and 6.8 establish
  NP-hardness and APX-hardness ($\gamma = 8569/8568$ under unbounded
  alphabet) for the restricted dictionary-only case under the
  symbol-count objective $L_{\mathrm{sym}}$. The following remain
  open: (a) bit-cost APX-hardness for the restricted case (the
  naive transfer fails because bit-cost approximation algorithms can
  trade off $L_{\mathrm{sym}}$ against the nonterminal count $K$
  appearing in the multiplicative $\log$ factor); (b) APX-hardness
  of the unrestricted joint problem with free predictor $M$, which
  adds the dimension of paying $L(M)$ bits to gain $L(R \mid Y)$
  savings (a padding-based reduction strategy is plausible but
  must control how a non-trivial predictor exploits structure in
  the corpus, which standard incompressibility arguments do not
  directly address, since an unrestricted predictor can specialize
  to the SGP-encoded prefix and gain unbounded compression there
  while ignoring an incompressible suffix). The inherited constant
...
\item
  ...
  natural-data classes --- partially resolved. Three explicit
  ...
  conditionally iid samples); and Lemma 5.6c gives
  ...
  asymptotic bounds).
  All three proofs are verified at machine precision in
  \texttt{scripts/verify/lemma\_5\_6\{a,b,c\}*}; the proofs extend
  additively along independent column blocks.
  ...
  structured classes (natural images at a given resolution, general
  hidden Markov models with non-trivial hidden state, non-stationary
  processes) is open and likely requires
...
\item
  ...
  space. All three require non-trivial adaptation to the fixed
  source-alphabet (256-byte) constraint of the $E_{12}$ problem and
  remain research-scale open.
\item
