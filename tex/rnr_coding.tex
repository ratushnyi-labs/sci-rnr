...
the cross-entropy excess over the true conditional is at most
$\varepsilon_{\mathrm{ac}}^{\mathrm{total}} = O(\log(1/\eta))$ per
stream (the standard practical assumption, e.g.\ Cover \& Thomas
2006 Theorem 13.4 on arithmetic coding with finite-precision
floors); (C3) auxiliary metadata $L(\Theta)$ is $\mathrm{polylog}(N)$.
...
\leq C \rho^K / (1 - \rho)$ for some $C, \rho \in (0, 1)$ (Lemma 5.6c (b)
plus exp-decay (5.8)). With $K \geq W_N = \Omega(\log N)$,
$\rho^K = O(N^{-c})$ for some $c > 0$, hence
$m \cdot (\delta_\infty - \delta_K) = O(m \cdot N^{-c}) =
O(N^{1-c} / K) = O(N^{1-c} / \log N) = o(1)$
(in fact $o(N^{1-c})$, which is $o(1)$ when $c \geq 1$ and otherwise
$o(N^{1-c})$; in either case dominated by the $O(1)$ term below for
the matching claim).
...
(by stationarity, equivalent to a per-position divergence). Then RNR
...
\]
the exact cross-entropy decomposition (Cover \& Thomas 2006 §5). Summing
...
is typically $1.0$--$1.5$ bits/byte versus Shannon's lower-bound estimate
$h(X) \approx 0.6$--$1.3$ bits/byte for English (Shannon 1951; modern
LLM estimates: e.g., DeepMind Chinchilla 70B reaching ~1.04 bpb on
enwik9), so $\varepsilon_{\mathrm{KL}} \approx 0.1$--$0.5$ bpb in
practice. As predictor quality improves with model scale and training
data, $\varepsilon_{\mathrm{KL}} \to 0$ (per the universal-approximation
results of Shannon-McMillan and the empirical scaling laws of
Kaplan--Hoffmann), bringing RNR's empirical rate arbitrarily close to
Shannon's lower bound $h(X)$. \emph{Practical interpretation:} the
gap $h(M') - h(X) = \varepsilon_{\mathrm{KL}}$ is the ``predictor
...
$K^* = \Theta(\sqrt{N})$ and the storage overhead $C(K^*) - N \cdot h(X) -
N \cdot \varepsilon_{\mathrm{KL}} = \Theta(\sqrt{N})$; the per-query cost
is also $\Theta(\sqrt{N})$. This is the classical
$\sqrt{N}$ time--space trade-off of indexed search structures
(Bentley \& Yao 1976, Manber \& Myers 1993 for suffix arrays;
Belazzougui--Navarro 2014 for compressed indexes), now expressed in
the RNR-compression setting.}
