...
$Y = M_I(C)$ be decoder-reproducible side information. Any uniquely
decodable lossless code for $X$ whose decoder uses only the codeword
and $Y$ has expected length at least $H(X \mid Y)$. Conversely, an
\emph{ideal} Type-I class-and-payload coder driven by the true
conditional law of the Code12 error variable $E = E_{12}(X) \oplus Y$
...
models match the true conditional law.}

\emph{\textbf{Proof.}} The converse is Shannon's noiseless source
coding theorem applied to the conditional distribution
$P(X \mid Y)$ (Cover and Thomas 2006, Theorem 5.4.1). For
achievability, fix an injective Code12 map and define
...
chain rule. An entropy coder driven by these conditional laws attains
...
$|S_N| = 2^d = 2^{(4-c)N}$. Define $D_N$ as the uniform distribution
on $S_N$.
...
\varepsilon_N$ bits, i.e., savings of $4N - d = cN$ bits per block
\emph{before} model-header amortization. Describing the generator
matrix $G$ itself costs $L(G) = d \cdot 4N = 4(4-c)N^2$ bits, paid
once and absorbed into the $L(M_{\text{pos}})$ term of (3.1). When
the same $G$ is reused across $B_N$ blocks, the per-block model
contribution is $L(G)/B_N$; taking $B_N = \Omega(N^2)$ (e.g.,
$B_N = N^2$) makes this $O(1)$ bits per block, absorbed into the
$O(\log N)$ overhead. For fewer blocks the per-block cost is
...
$\mathrm{TC} = cN$ for the linear-code family (sum of marginals
$\sum_i H_{\text{marg}}(X_i) = 4N$ minus joint entropy
$H(D_N) = (4-c)N$). For general
...
\textbf{Proposition 5.7 (Support selection and arithmetic benchmark).}
\emph{Fix $v$, $k_v$, and a support size $s$. Write
$A_s(\alpha) = \log_2 \binom{s}{k_v - \alpha} + \log_2 \binom{N - s}{\alpha}$.
If $A_s(\alpha)$ is non-decreasing on the feasible escape counts
$\alpha$ that can occur under $Q$, then among supports of size $s$
the top-$s$ positions sorted by $Q_v(i)$ minimize the expected
Theorem 5.3 residual. Without this monotonicity condition, no universal
prefix-optimality statement is claimed.}

\emph{Moreover, if $Q$ is the true conditional distribution used by
the decoder, per-position arithmetic coding achieves expected length
$H(X \mid Q) + \varepsilon_N$, and any support-set code using the
same side information has expected length at least $H(X \mid Q) - O(1)$.
For an arbitrary misspecified predictor $Q$, the arithmetic-coding
rate is the cross-entropy
$\mathbb{E}_D [-\sum_i \log_2 Q_{X_i}(i)]$, which equals the
conditional entropy only when $Q$ matches the true conditional law
and is not a lower bound on all other codes in general.}

\emph{\textbf{Proof.}} Under the stated monotonicity condition,
swapping a lower-$Q_v$ included position $i$ with a higher-$Q_v$
excluded position $j$ stochastically decreases $\alpha_v$, hence
cannot increase $\mathbb{E}[A_s(\alpha_v)]$. Iterating the swap
argument terminates at the top-prefix configuration.
The arithmetic-coding statement is Shannon's converse when $Q$ is
the true conditional law (Cover and Thomas 2006, Theorem 5.4.1);
under misspecification the expected length is cross-entropy by
definition. \hfill$\blacksquare$
...
bound. Type-III-C with continuous latents is therefore a strict
generalization of Type-III-C with a discrete dictionary, with the
dictionary case as the degenerate limit $q \to \delta$ at the nearest
discrete token.}
...
NP-hard).} \emph{Given a corpus $X$, jointly choosing a token
dictionary $D$ and predictor $M$ to minimize Type-III-C description
length is NP-hard. This remains true in the restricted normal-form
case where $M$ is fixed to the trivial constant predictor, the repair
stream is the distinguished start token, and $D$ is a recursive
straight-line dictionary measured by symbol count.}
...
(Charikar et al.\ 2005, Theorem 3.4), the restricted Type-III-C
problem is NP-hard, and the unrestricted problem is at least as hard.
The reduction is computable in linear time in $|s|$.
\hfill$\blacksquare$
...
Prabhakaran, Sahai and Shelat (2005, Theorem 3.4) prove an explicit
constant inapproximability bound for SGP: no polynomial-time algorithm
achieves approximation ratio smaller than $8569/8568$ unless $P = NP$.
Their proof is a constant-gap reduction from MAX-3SAT-3, not a
Set-Cover / Lund--Yannakakis logarithmic inapproximability theorem.
Combined with the exact bijection above, a polynomial-time
$\alpha$-approximation for the restricted Type-III-C problem with
$\alpha < 8569/8568$ would yield a polynomial-time approximation to
SGP below the Charikar et al.\ threshold, contradicting $P \neq NP$.
The unrestricted Type-III-C problem is at least as hard, so the same
inapproximability bound transfers. \hfill$\blacksquare$

\textbf{Remark on the constant.} The $8569/8568$ figure is the
explicit threshold proved by Charikar et al.\ for SGP; for the
unrestricted Type-III-C objective (with bit-cost bookkeeping rather
than symbol-cost), the inherited constant is at most this value
divided by the worst-case per-symbol encoding ratio
$\lceil \log_2(|\Sigma|+K) \rceil$, which is bounded on any fixed
family of instances. We claim only that \emph{some} constant $\gamma >
1$ exists in the general case; pinning down the optimal constant for
a specific encoding convention is left to follow-up work.
...
\[
b_\ell \geq \lceil \log_2(d_\ell \cdot W_\ell \cdot I_{\ell-1}) \rceil + 1,
\]
...
any input to layer $\ell$; (iii)~all non-linearities are deterministic
...
$b \geq \lceil \log_2(d_{\max} \cdot W_{\max} \cdot I_{\max}) \rceil + 1$,
where $d_{\max} = \max_\ell d_\ell$ is the maximum fan-in. Without
inter-layer renormalization, intermediate magnitudes can grow
recursively: $I_\ell \leq d_\ell \cdot W_\ell \cdot I_{\ell-1}$ in
the worst case, so $I_\ell \leq I_0 \prod_{r \leq \ell} d_r W_r$ and
depth enters multiplicatively, not linearly. In practice, with
quantized activations clipped to $I_{\max} \leq 2^{16}$ and weight
entries bounded by $W_{\max} \leq 2^{16}$ at each layer, $b = 32$
suffices for any fan-in $d_{\max} \leq 2^{16}$, which covers all
