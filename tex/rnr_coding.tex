...
inverse-linear ratio of Theorem 4.3. Studied amplification
mechanisms in the linear-code / lattice / hypercube literature
include: (a) PCP-style reduction from APX-hard Label Cover via the
Arora-Babai-Stern-Sweedyk (1997) Nearest Codeword Problem framework;
(b) bounded-distance decoding hardness (the relevant survey is
Dumer, Micciancio, Sudan 2003 \emph{Hardness of Approximating the
Minimum Distance of a Linear Code}, which defines BDD and leaves
its computational complexity as an open problem; subsequent BDD
hardness comes from Liu, Lyubashevsky, Micciancio 2008 and
Bennett, Golovnev, Stephens-Davidowitz 2017); (c) Closest Vector
Problem hardness in the lattice geometry: the canonical CVP
hardness paper is Dinur, Kindler, Raz, Safra 2003 (constant-factor
inapproximability for general lattices), with the CVP-with-preprocessing
(CVPP) variant analyzed by Alekhnovich, Khot, Kindler, Vishnoi 2005,
specialized to $\mathbb{F}_2^{12}$. A fourth mechanism is
\emph{graph-powering / tensor-product amplification} in the
Dinur (2007) PCP-style framework: if a Wagner-Corneil-like
tree-embedding instance could be composed with a robust PCP
verifier (Dinur \emph{The PCP theorem by gap amplification}, JACM
54(3):12, 2007), the inverse-linear gap could in principle be
amplified to a constant. None of these constructions is known to
realize gap-preserving reduction from any APX-hard problem directly
into the byte-alphabet $E_{12}$ geometry; closing OP4 remains open.
