...
\emph{\textbf{Proof.}}
Under IRM, the joint process $(\mathrm{Cache}_t, X_t)_{t \geq 1}$
is a homogeneous Markov chain on the finite state space
$\mathcal{S} := \{(S, x) : S \subset [N_{\rm dist}], |S| = C,
x \in [N_{\rm dist}]\}$ (cardinality $\binom{N_{\rm dist}}{C} \cdot
N_{\rm dist}$, finite). The transition law is: $(S_t, X_t) \to
(S_{t+1}, X_{t+1})$ where $X_{t+1} \sim \pi$ independently;
$S_{t+1} = S_t \setminus \{\mathrm{oldest}(S_t)\} \cup \{X_{t+1}\}$
if $X_{t+1} \notin S_t$, else $S_{t+1}$ equals $S_t$ with
$X_{t+1}$ moved to most-recent position (standard LRU update rule).

\emph{Irreducibility.} For any target state $(S', x') \in \mathcal{S}$,
the deterministic query sequence $X_{t+1}, X_{t+2}, \ldots, X_{t+C+1}$
chosen to be (any ordering of $S' \cup \{x'\}$) drives the chain
into state $(S', x')$ regardless of the starting state. Each such
specific query has probability $\geq \pi_{\rm min} > 0$ under IRM,
so the chain reaches $(S', x')$ from any state in $\leq C + 1$
steps with probability $\geq \pi_{\rm min}^{C+1} > 0$. Hence the
chain is irreducible.

\emph{Aperiodicity.} For any state $(S, x)$, the self-loop
$(S, x) \to (S, x)$ via $X_{t+1} = x$ (followed by $X_{t+2} = x$
again) has probability $\pi_x^2 > 0$, giving period 1.
...
The spectral gap of the additive reversibilization $\tilde{P} =
(P + P^*)/2$ (where $P^*$ is the time-reversal of $P$ w.r.t.\
$\pi^*$) is strictly positive: $\gamma_{\rm LRU} := 1 -
\lambda_2(\tilde{P}) > 0$, by the standard finite-chain argument
(LPW Corollary 12.6).
...
from the chain being finite, irreducible, and aperiodic (cf.\ proof
above). Explicit lower bounds on $\gamma_{\rm LRU}$ are more subtle.
A crude Dobrushin-coupling lower bound: by the irreducibility
argument above, the chain reaches any target state in $\leq C+1$
steps with probability $\geq \pi_{\rm min}^{C+1}$, so the coupling
time from any pair of starting states is bounded by a geometric
random variable, giving (via Markov's inequality and the standard
Dobrushin-spectral-gap relationship) $\gamma_{\rm LRU} \geq
\pi_{\rm min}^{C+1}$. For Zipf popularity $\pi_i \propto i^{-\alpha}$
on $[N_{\rm dist}]$, $\pi_{\rm min} = N_{\rm dist}^{-\alpha} /
\zeta_{N_{\rm dist}}(\alpha)$, giving $\gamma_{\rm LRU} \geq
(N_{\rm dist}^{-\alpha}/\zeta(\alpha))^{C+1}$ --- positive but
exponentially small in $C$, hence not practically useful for
moderate cache sizes. Tighter bounds via LRU-specific spectral
structure (the Knudsen-Tsitsiklis 1990 ``stack distance''
representation; coupling-from-the-past analyses for the LRU
permutation chain; Aldous-Diaconis 1987 cutoff phenomena for
related permutation chains) are open for future work. The
qualitative $\sqrt{Q}$-Hoeffding scaling of Theorem 7.26 holds
for any positive $\gamma_{\rm LRU}$, regardless of the explicit
constant.
