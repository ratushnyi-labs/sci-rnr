...
\emph{Remark on cold-context excess characterization.} A natural attempt
is to sharpen the worst-case bound
$\Delta_{\mathrm{sync, total}} \leq m \cdot S \cdot W \cdot \log_2(1/\eta)$
of Lemma 5.1a by computing the expected cold-context excess exactly
for stationary sources, using the $\delta_W$ quantity from Lemma 5.6c.
An earlier draft of this paper contained a Lemma 5.1c claiming
$\Delta_{\mathrm{sync, per-pt}} = \delta_W$ exactly for stationary
processes, with $\delta_W$ bounded by Crutchfield--Feldman's excess
entropy. That lemma was retracted after adversarial review identified a
fundamental conflation: the $W$-bounded predictor's warm-region rate is
$h_W := H(X_{W+1} \mid X_1^W)$, not the entropy rate $h(X)$; equality
$h_W = h$ holds only for order-$\leq W$ Markov sources. For general
stationary mixing sources, $h_W > h$, and the warm-region contribution
$(K - W) \cdot h_W$ differs from $(K - W) \cdot h(X)$ by an additive
$(K-W) \cdot (h_W - h)$ term. Lemma 5.1b's choice $W = O(\log N)$ makes
this difference $O(1)$ per position via the (5.8) decay, but the exact
$\delta_W$ identity requires order-$\leq W$ Markov hypothesis to hold
without correction. The cold-context excess analysis is more subtle
than first appearance suggests; a correct quantitative formulation is
left as a refinement to OP1 and recorded in §13.2.
