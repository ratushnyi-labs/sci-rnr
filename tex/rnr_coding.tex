...
the T7.5 achievability, with Theorem 7.27$'$ \emph{refuting} the
general-scheme converse for finite-order-Markov sources (Ferragina
--Venturini $O(1)$-access escape) and Corollary 7.27c recovering the
$\mathrm{cost}(M)$ product on the predictor-faithful class, which
covers state-snapshot schemes). Section 8 states the
...
identifier-length floor and survives into Corollary 7.27c. For genuinely
neural (non-finite-order-Markov) predictors the Ferragina--Venturini
escape closes (Remark 7.27d(ii)) and the existence of any model-free
$O(1)$-access Shannon-near structure reverts to open.
...
\item
  ...
  Theorem 7.27 establishes a tight product bound $C(K) \cdot S(K) \geq
  ...
  open problem (genuinely open).} The refutation in piece (1) relies on
  the source being finite-order Markov, so that $H_k \to h(X)$ at fixed
  $k$. For \emph{genuinely neural} predictors $M$ whose conditionals are
  not finite-order Markov, the Ferragina--Venturini surrogate $H_k$ is
  not Shannon-near at any fixed $k$ (Remark 7.27d(ii)), and the escape
  closes. Whether \emph{some other} model-free succinct random-access
  structure can attain the Theorem 7.2 Shannon-near rate with
  $o(\sqrt{N})$ query time --- and hence beat the predictor-faithful
  product --- for non-Markov neural-compressible sources is open. The
  open question is thereby \emph{sharpened} from ``is there a general
  converse?'' (answer: not universally --- piece (1)) to ``is there a
  model-free Shannon-near random-access structure for non-finite-order
  -Markov sources, or does some converse force $\Omega(\sqrt N)$ query
  there?''. A negative answer (no such structure) would restore a
  general converse on the neural-predictor sources that motivate RNR; a
  positive answer would exhibit a second escape beyond Ferragina
  --Venturini.
\end{itemize}
...
Codex ``Pareto-frontier-flat'' analysis (the achievable $L_{\rm
...
all $N$. This is exactly the Gemini-flagged ceiling-discontinuity
