...
\item
  ...
  occurrence forfeiting its dictionary saving (at most $8 \cdot
  |\tau|$ bits per crossed token, since the raw-literal baseline
  costs $8\ell$ bits for an $\ell$-byte token). At most one token
  crosses any given boundary, so over $|X|/K$ sync points the
  worst-case loss is bounded by $8 \cdot |X| \cdot
...
typical $K$ and Type-I/II/III-B/III-B coders under the
