...
\emph{Remark 5.6i-a (TC vs.\ sum of pairwise MIs).} A natural
attempt at upper-bounding TC by pairwise MIs --- $\mathrm{TC}(D_N)
\stackrel{?}{\leq} \sum_{i < j} I(X_i; X_j)$ --- \emph{fails in
general}. For $K$-sparse uniform on $N = 100$, $K = 10$: pairwise MI
$I(X_i; X_j) \approx 7.5 \times 10^{-5}$ bits and
$\sum_{i<j} I \approx 0.37$ bits, but $\mathrm{TC} \approx 2.92$ bits
--- TC \emph{exceeds} the pairwise sum by a factor of $\sim 8\times$.
This is because the global cardinality constraint is a higher-order
dependence that does not decompose into pairs: $K$-sparse vectors
have approximately-independent pairs but a strict joint constraint.
The correct upper bound is the multi-information chain rule
$\mathrm{TC}(D_N) = \sum_{k=2}^{N} I(X_k; X_{1:k-1})$, which captures
the full joint dependence including higher-order interactions.
...
\emph{Updated 2026-05-27:} after the negative result on
amortised-family predictor-absorption transfer (item 1 status), the
only remaining combinatorial-polynomial-time route in this list is
GSGP (Carrascosa et al.\ 2016) APX-hardness, currently itself an
open problem. The cryptographic route (Liu--Pass / OWF, §13.2)
closes a different problem (average-case decision on samplable
distributions), not free-predictor worst-case APX-hardness.
Structurally novel reductions outside the SGP/grammar template
would also qualify.
