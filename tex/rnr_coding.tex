...
crossover budget being characterised in Remark 6.9c. Its discrete, deployed
counterpart (a transformer's autoregressive field; Remarks 7.15a--7.15e)
inherits a \#P-hard value oracle with no Gaussian analogue, yet keeps causal
coding and random access exactly polynomial (the hardness confined to the
optional non-contiguous lever); the residual bounded-order exact problem ---
exact finite-block hidden-Markov entropy --- is bracketed in
$[\,\text{poly},\mathrm{FP}^{\#P}\,]$ (poly-$\varepsilon$-approximable under
filter stability, integer-R\'enyi-exact, min-entropy NP-hard) with both natural
attacks obstructed. Worked numerical examples are
...
concentration bounds for archive-size deviation via a sequence of
sharpening estimates spanning the central-limit and large-deviation
scales --- Azuma's martingale inequality
...
a non-asymptotic Berry--Esseen CLT giving two-sided confidence
intervals at Kolmogorov rate $O(N^{-1/2}(\log N)^2)$ (Theorem 7.28),
a second-order (dispersion) coding rate
$Nh+(N/K)\delta_\infty+\sqrt{NV}\,Q^{-1}(\varepsilon)$ (Theorem 7.29),
and a large-deviations reliability function giving the exact
archive-overflow exponent with a matching lower bound --- the Legendre
dual of the source R\'enyi spectrum (Theorem 7.31) --- and
...
\item
  The \emph{discrete}, deployed counterpart of the above for the neural-LM
  field of Theorem 7.15 --- a five-part arc (Remarks 7.15a--7.15e). The
  §6.9 Gaussian maximum-entropy-sampling support selection transfers to a
  transformer's discrete autoregressive field with \emph{unconditional}
  submodularity (the noise-floor calibration disappears; Remark 7.15a), but
  with a \#P-hard value-oracle obstacle that has no Gaussian analogue. That
  hardness is \emph{orthogonal} to the deployed encoder: causal coding and
  byte-granular random access are exactly polynomial (one forward pass per
  symbol; Remark 7.15b), the \#P-hardness living only in the \emph{optional}
  non-contiguous support-selection lever. The bounded-order exact-oracle
  restriction is a clean new open problem (exact finite-block hidden-Markov
  Shannon entropy), here \emph{bracketed} on all sides:
  poly-time-$\varepsilon$-approximable under a filter-stability hypothesis
  (Remark 7.15c); every integer-order R\'enyi entropy exactly polynomial via
  a product-chain agreement identity, with the min-entropy NP-hard (HMM
  consensus) and the collision entropy $H_2$ a free poly lower bound (Remark
  7.15d); and the exact oracle pinned in $[\,\text{poly},\mathrm{FP}^{\#P}\,]$
  with \emph{both} natural attacks --- predicate-encoding hardness and
  history-filtering algorithm --- provably obstructed (Remark 7.15e: the
  read-once / $\#\mathrm L\subseteq\mathrm{FP}$ barrier and the non-collapsing
  Blackwell measure). Thus the support-selection complexity is \emph{resolved
  for the operative high-order predictor} and \emph{mapped} for the
  theoretical bounded-order residual.
\item
...
covers state-snapshot schemes; §7.28 a non-asymptotic
Berry--Esseen CLT for the repair length; §7.29 a second-order
(dispersion) coding rate; and §7.31 a large-deviations reliability
function (the exact archive-overflow exponent with a matching lower
bound, the R\'enyi-spectrum dual), extending the deviation hierarchy
§§7.14/7.25/7.28/7.29/7.31 from the sub-Gaussian body through the
central-limit scale to the exponential-rate tail). Section 8 states the
...
entropy-rate floor (Theorem 6.6c) gives $H(X^N) \ge N h(X)$. Hence
...
\emph{cold-context} random-access interplay of Theorem 7.29 does not --- an honest
obstruction).} For a
...
memory --- which (unlike the lossless long-run varentropy that Theorem 7.28 supplies
...
neural-LM field of Theorem 7.15 and record two effects with no Gaussian
