...
\item
  ...
  sources with predictor context $W \geq W'$: the per-sync excess
  equals exactly $\delta_{W'} = H(X^{W'}) - W' h(X)$ (a finite
  source-specific constant), and total sync excess over $H(D_N)$ is
  $(m-1) \cdot \delta_{W'}$ — strictly tightening Lemma 5.1a's
  worst-case $W \log_2(1/\eta)$ per-sync factor whenever $\delta_{W'}$
  is smaller (typical for natural-data Markov sources).
...
natural-data Markov sources: e.g.\ for an order-$1$ binary Markov
chain with $E \approx 0.7$ bits/byte (Shannon 1951 bigram estimate
for English) and $W = O(\log N) = 20$, $S = 1$, $\eta = 2^{-12}$, the
worst-case Lemma 5.1a bound is $\sim 240$ bits/sync versus
Lemma 5.1c's tight value $\sim 0.7$ bits/sync, a $\sim 300\times$
improvement.}
...
\item
  ...
  Markov sources with predictor context $W \geq W'$, where
  $\delta_{W'} = H(X^{W'}) - W' h(X)$ is finite and typically much
  smaller than $W \log_2(1/\eta)$ for natural-data Markov sources.
...
\item
  ...
  process is not iid.
  All four proofs are verified at machine precision in
