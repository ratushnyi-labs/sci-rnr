...
\item
  ...
  Lemma 5.1b strengthens this for exponentially-$\beta$-mixing
  stationary sources (covering irreducible-aperiodic finite-state
  Markov chains, hidden Markov models with geometrically ergodic
  hidden chains, and any process with exponentially decaying
  conditional mutual information of the form
  $I(X_n; X_{1:n-W-1} \mid X_{n-W:n-1}) \leq C \rho^W$): for these,
  choosing $W = O(\log N)$ brings the per-position sequential rate
  $H_W(D_N)$ within $O(1)$ of $H(D_N)$, so the total length is
  $H(D_N) + O(\log N)$ plus $\Delta_{\mathrm{sync,total}} = O(\log N
  \cdot \log_2(1/\eta))$ — a logarithmic, not linear, rate gap, with
  per-query random-access cost still polylog in $|X|$.
...
\textbf{Lemma 5.1b (Logarithmic $W$ suffices for exponentially-mixing
stationary sources).} \emph{Let $X = (X_n)_{n \geq 1}$ be a strictly
stationary process on a finite alphabet $\mathcal{X}$. Suppose $X$ is
exponentially $\beta$-mixing in the sense that there exist constants
$C > 0$ and $\rho \in (0, 1)$ such that for every $n$ and every
$W \geq 1$,}
\[
I\bigl(X_n ; X_{1:n-W-1} \,\big|\, X_{n-W:n-1}\bigr) \leq C \cdot \rho^W.
\tag{5.8}
\]
\emph{Then for any $N \geq 1$ and any $W \geq 1$,}
\[
0 \leq H_W(D_N) - H(D_N) \leq N \cdot C \cdot \rho^W.
\]
\emph{In particular, choosing
$W_N := \lceil \log_{1/\rho}(N \cdot C) \rceil$ gives
$H_{W_N}(D_N) - H(D_N) \leq 1$, and the per-position sequential
arithmetic-coding scheme of Lemma 5.1a achieves total expected
length}
\[
H(D_N) + O(\log N) + \Delta_{\mathrm{sync,total}}(W_N) + \varepsilon_N \cdot N,
\]
...
$O(\log L(R) + (K + k) \cdot \mathrm{cost}(M_{W_N}))$ per query,
where $\mathrm{cost}(M_{W_N})$ is the per-position cost of evaluating
the order-$W_N$ predictor (e.g., $O(1)$ for fixed-architecture neural
predictors regardless of $W$, or $O(\sigma^{W_N}) = N^{\log_2 \sigma /
\log_2(1/\rho)}$ for explicit lookup-table predictors).}

\emph{\textbf{Proof.}} For any $n \geq 1$ and $W \geq 1$,
\[
H(X_n \mid X_{n-W:n-1}) - H(X_n \mid X_{1:n-1})
= I(X_n; X_{1:n-W-1} \mid X_{n-W:n-1}) \leq C \cdot \rho^W
\]
by the mixing hypothesis (5.8). The first $W$ positions
($n \leq W$) contribute zero excess since the conditioning is the
full past in both cases (or empty if $n = 1$). Summing over
$n = 1, \ldots, N$:
\[
H_W(D_N) - H(D_N) = \sum_{n=1}^N \bigl[H(X_n \mid X_{n-W:n-1}) - H(X_n \mid X_{1:n-1})\bigr]
\leq N \cdot C \cdot \rho^W.
\]
Setting $W = W_N = \lceil \log_{1/\rho}(N \cdot C) \rceil$ gives
$\rho^{W_N} \leq 1/(NC)$, so $H_{W_N}(D_N) - H(D_N) \leq 1$. The
total-length bound follows from Lemma 5.1a with this $W_N$, and
the random-access cost from Theorem 10.3.
\hfill$\blacksquare$

\emph{Implications.} For natural-data sources with finite memory
(e.g., Markov chains of any order, hidden Markov models with
geometrically ergodic hidden chains, and more generally any
strictly stationary process satisfying (5.8)), Lemma 5.1b shows
that byte-granular random access is achievable with rate
$H(D_N) + O(\log N)$ excess (a logarithmic, not linear, rate gap),
provided the predictor architecture can be scaled to context
$W = O(\log N)$ at fixed per-position evaluation cost. This includes
fixed-architecture neural-network predictors with windowed
attention, fixed-size transformer caches, and recurrent networks
with bounded internal state. Lemma 5.1b's mixing hypothesis (5.8)
is verified for: irreducible aperiodic finite-state Markov chains
(geometric mixing, Doeblin 1937), order-$W'$ Markov chains for any
$W'$ (the conditional MI is $\leq C \rho^W$ for any $W \geq W'$ with
$\rho$ determined by the chain's spectral gap, Cover \& Thomas 2006
Thm 4.7.1), and hidden Markov models with geometrically ergodic
hidden chains and finite emission alphabets (Cappe, Moulines, Ryden
2005, \emph{Inference in Hidden Markov Models}, Theorem 4.3.16).
