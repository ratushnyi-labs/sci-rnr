...
\textbf{Lemma 6.8b (Bit-cost APX-hardness within Charikar normal form
on Berman-Karpinski-hard instances).} \emph{Restrict to Charikar
reduction instances $H = (V, E)$ at the Berman-Karpinski-hard extremum:
$|E| \leq (3/2)|V|$ and $k_{\mathrm{OPT}} \geq |V|/3$, where
Berman-Karpinski (1999) NP-hardness of approximating Vertex-Cover-3
within factor $145/144$ applies. Let $\mathcal{G}_{\mathrm{Char}}(H)$
denote the class of grammars produced by Charikar's reduction on
such $H$, every $G \in \mathcal{G}_{\mathrm{Char}}(H)$ encoding a
vertex cover $\sigma \subseteq V$ with}
\[
L_{\mathrm{sym}}(G) = 15|V| + 3|E| + |\sigma|, \quad
K(G) = K_0(H) + \alpha(H) \cdot |\sigma|,
\]
\emph{where $K_0(H) \geq 0$ and $\alpha(H) \geq 0$ are structural
constants of the reduction. Within this class on BK-hard instances,
the bit-cost APX-hardness ratio satisfies}
\[
\gamma^{\mathrm{bit}}_{\mathrm{Char}}(H)
\;\geq\; \gamma_{\mathrm{SGP}} \cdot \frac{\lceil \log_2(|\Sigma|+K_{\mathrm{NO}}) \rceil}{\lceil \log_2(|\Sigma|+K_{\mathrm{YES}}) \rceil}
\;\geq\; \gamma_{\mathrm{SGP}} \;=\; \frac{8569}{8568},
\]
\emph{since the second factor is $\geq 1$ (as $K_{\mathrm{NO}} \geq
K_{\mathrm{YES}}$ from $\alpha \geq 0$). Equivalently: bit-cost
APX-hardness within the Charikar class on BK-hard instances is at
least as strong as the symbol-count APX-hardness $\gamma_{\mathrm{SGP}}
= 8569/8568$ of Theorem 6.8.}

\emph{\textbf{Proof.}} For any two grammars $G_{\mathrm{NO}},
G_{\mathrm{YES}} \in \mathcal{G}_{\mathrm{Char}}(H)$ on a BK-hard
instance:
\[
...
\]
Theorem 6.8 (which adapts Charikar 2005 Theorem 1 to the BK-extremal
point $|E| = (3/2)|V|$, $k_{\mathrm{OPT}} = |V|/3$, c.f.\ the algebra
preceding it in §6) gives the symbol-count APX-hardness ratio
$L_{\mathrm{sym}}(G_{\mathrm{NO}}) / L_{\mathrm{sym}}(G_{\mathrm{YES}})
\geq \gamma_{\mathrm{SGP}}$ over $\mathcal{G}_{\mathrm{Char}}(H)$
for such $H$. Since $\alpha(H) \geq 0$ and $k_{\mathrm{NO}} \geq
k_{\mathrm{OPT}}$, $K(G_{\mathrm{NO}}) \geq K(G_{\mathrm{YES}})$, hence
the ceiled log-ratio is $\geq 1$. Multiplying: $\gamma^{\mathrm{bit}}_{\mathrm{Char}}
\geq \gamma_{\mathrm{SGP}}$.
\hfill$\blacksquare$

\emph{Scope and limits.} (i) The lemma requires \emph{BK-hard
instances} (where Theorem 6.8's $\gamma_{\mathrm{SGP}}$ inapproximability
applies). For non-BK instances (e.g., sparse graphs with $|E| \ll |V|$),
the symbol-count APX-hardness ratio is smaller, and the lower bound
becomes weaker. (ii) The lemma is \emph{class-restricted} to Charikar
normal-form grammars; non-Charikar-format escape grammars are
out-of-scope. (iii) The lemma claims only the \emph{lower bound}
$\gamma^{\mathrm{bit}} \geq \gamma_{\mathrm{SGP}}$, not convergence
of $\gamma^{\mathrm{bit}}$ to $\gamma_{\mathrm{SGP}}$ from above
(the actual $\gamma^{\mathrm{bit}}_{\mathrm{Char}}$ can spike near
ceiling boundaries of $\lceil \log_2 \rceil$, where $K_{\mathrm{NO}}$
and $K_{\mathrm{YES}}$ round differently; verified numerically at
$|V| = 422928$ where $\gamma^{\mathrm{bit}} \approx 1.0436$, far
above $\gamma_{\mathrm{SGP}} \approx 1.000117$). The unrestricted
bit-cost APX-hardness problem (over all valid grammars and instance
families, not just BK-hard Charikar-format ones) remains open.
