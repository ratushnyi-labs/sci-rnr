...
\item
  ...
  NP-hardness of joint dictionary-and-predictor learning under a
  normal-form symbol-count objective (Theorem 6.7), and APX-hardness
  transferred from the explicit $8569/8568$ inapproximability constant
  for the Smallest Grammar Problem (Charikar et al.\ 2005, Theorem
  6.8). We do not claim this is a complete complexity characterization
  of Type-III-C: e.g., the exact inapproximability constant under
  bit-cost (rather than symbol-cost) bookkeeping is open.
\item
...
\item
  ...
  $O(\log(N/K) + (K + k) \cdot \mathrm{cost}(M))$ --- polylogarithmic
  in archive size $N$ once sync-point spacing $K$, read window $k$,
  and predictor cost are held fixed --- via Theorem 10.3 with
  Definitions 10.2, 10.3 of $W$-bounded predictors and sync points.
...
\item
  ...
  archive size (Theorem 10.5), and the predictor can be required to
