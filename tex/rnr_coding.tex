...
\;\leq\;
2 \exp\!\Bigl(-\frac{t^2}{2 N \cdot \log_2^2(1/\eta)}\Bigr).
\]
\emph{Equivalently, for $t = c \cdot \sqrt{N} \cdot \log_2(1/\eta)$
...
\;\leq\;
\sqrt{2 N \log(2/\delta)} \cdot \log_2(1/\eta).
\]
...
The martingale differences $D_n := M_n - M_{n-1}$ are bounded by
$\log_2(1/\eta)$ in absolute value (each $\ell_n$ contribution to the
conditional expectation is bounded).
...
\;\pm\;
\underbrace{\sqrt{2 N \log(2/\delta)} \cdot p}_{\substack{\text{concentration} \\ O(\sqrt{N} \log(1/\delta) \cdot p)}}
\;+\;
...
Chinchilla-70B-class transformer giving
$H_{M'} \approx 1.04$ bits/byte (Del{\'e}tang et al.\ 2024 ICLR
arXiv:2309.10668), $\eta = 2^{-32}$ ($p = 32$), $K = \sqrt{N} \approx
31623$, $\delta_\infty \approx 1$ bit (English text empirical),
$\delta = 10^{-6}$:
\begin{itemize}
\item Predictor cross-entropy: $N \cdot H_{M'} = 10^9 \cdot 1.04$ bits
$\approx 124 \,\mathrm{MB}$.
...
(or $\sim 0.2\%$ of archive).
\item Concentration at $\delta = 10^{-6}$ ($\log(2/\delta) \approx 14.5$):
$\sqrt{2 \cdot 10^9 \cdot 14.5} \cdot 32 \approx 5.45 \cdot 10^6$ bits
$\approx 665 \,\mathrm{kB}$ ($\sim 0.5\%$ of archive).
\item Predicted total: $\approx 124 \pm 0.7 \,\mathrm{MB}$ at $\delta
= 10^{-6}$ ($\sim 7.68\times$ compression vs $1 \,\mathrm{GB}$ raw text).
...
$\sqrt{2 N \log(2/\delta)} \cdot p$. Combine all terms.
...
\emph{(c) [Insertion of $M$ bytes at position $i$]} \emph{Re-encode
the sub-block containing $i$ and shift sync-metadata offsets for
subsequent sub-blocks by $\Delta L$ (the archive-bit-length change).
Cost: $O((M+K) \cdot \mathrm{cost}(M'))$ for re-encoding the affected
sub-block (now containing the inserted bytes plus the original
content), plus $O((N - i)/K \cdot \log L(R))$ header offset updates.
The header offset shift is constant per sub-block, so cumulative
overhead is at most $m \cdot \log L(R) = \mathrm{polylog}(N)$ bits.}

\emph{(d) [Deletion of $M$ bytes at position $i$]} \emph{Symmetric to
insertion: re-encode affected sub-block, shift offsets of subsequent
sub-blocks. Cost: $O((M+K) \cdot \mathrm{cost}(M'))$ + header updates.}
...
payloads. Total merge: $O(m \cdot \log L(R)) = O(\mathrm{polylog}(N) \cdot
\mathrm{polylog}(N)) = \mathrm{polylog}(N)$ for $K = \mathrm{polylog}(N)$.

Hence
$T^{(P)} = N \cdot \mathrm{cost}(M') / P + \mathrm{polylog}(N)$,
...
\item \emph{(Zipf hot-key with skew $\alpha$)}: $\rho_C \approx 1 -
(C/m)^{-1+1/\alpha}$. For $\alpha > 1$ (heavy-tailed): $\rho_C \to 1$
as $C$ grows.
...
\emph{(b) Hot-key access patterns.} For Zipf-distributed access
($\alpha = 1.1$ typical for web caches), a cache of $C = 100$
sub-blocks captures $\sim 80\%$ of queries. Amortized cost reduced
$5\times$ vs cold cache.
...
\quad
\mathrm{TotalCost}(L^*, N) \;=\; N \cdot h(X) + \Theta\bigl(N^{\alpha/(1+\alpha)}\bigr),
\;
}
\]
\emph{i.e., the optimal predictor size grows as $L^* = \Theta(N^{1/(1+\alpha)})$,
strictly sub-linear in $N$, and the per-archive amortised predictor
overhead is $\Theta(N^{\alpha/(1+\alpha)})$ bits — also sub-linear.}

\emph{Concrete scaling values.}
\begin{itemize}
\item \emph{$\alpha = 1$ (heuristic):} $L^* = \Theta(\sqrt{N})$,
overhead $= \Theta(\sqrt{N})$. For $N = 1\,\mathrm{GB}$: $L^* \sim
30\,\mathrm{k}$ parameters, overhead $\sim 30\,\mathrm{kB}$.
\item \emph{$\alpha = 0.5$ (more conservative):} $L^* = \Theta(N^{2/3})$,
overhead $= \Theta(N^{1/3})$. For $N = 1\,\mathrm{GB}$: $L^* \sim
10^6$ params, overhead $\sim 1\,\mathrm{kB}$.
\item \emph{$\alpha = 0.34$ (Kaplan 2020 fit for text models):} $L^*
\sim N^{0.75}$, overhead $\sim N^{0.25}$. Balanced regime.
\end{itemize}
...
Substituting back:
$\mathrm{TotalCost}(L^*, N) = L^* + N \cdot h(X) + N \cdot C \cdot
(L^*)^{-\alpha} = L^* + N h(X) + (L^*)/\alpha = (1 + 1/\alpha) L^* +
N h(X) = \Theta(L^*) + N h(X)$.
With $L^* = \Theta(N^{1/(1+\alpha)})$, the overhead is $\Theta(N^{1/(1+\alpha)})$
which equals $\Theta(N^{\alpha/(1+\alpha)})$ when re-expressed in terms
of the source-cost decomposition (the $1/(1+\alpha)$ power on $L$
balances $\alpha/(1+\alpha)$ on $N$).
\hfill$\blacksquare$
...
(cryptographically secure under standard random-oracle / PRF
assumptions; Bellare--Canetti--Krawczyk 1996 \emph{Advances in
Cryptology --- CRYPTO '96}, Springer LNCS 1109 ``Keying hash
