...
\emph{Remark (decoder latency tail-bound — outside scope).}
A previous draft of this section attempted a combined batched-workload
latency tail bound (denoted Theorem 7.24) combining per-sub-block log-
loss concentration (Theorem 7.14) with cache-Bernoulli concentration
via LRU. Adversarial cross-model review (codex gpt-5.5) found multiple
fatal flaws in the formulation: (i) LRU miss indicators are not iid
Bernoulli --- the counterexample workload ``$A, B, A, B, \ldots$''
against a $C = 1$ cache produces $\Theta(Q)$ miss deviation rather
than $\Theta(\sqrt{Q})$; (ii) per-miss costs are not iid across queries
because the same sub-block can be missed repeatedly under cache
thrashing; (iii) the long-run hit rate $\rho_C$ does not control
finite-batch deviations under non-stationary workloads. A correct
finite-batch tail bound requires (at minimum) an Independent Reference
Model (IRM) assumption on the workload and a separate concentration
analysis for LRU miss indicators under IRM (Fagin 1977 \emph{Comm.\
ACM} 20(11):846 and Hendricks 1972 for foundational LRU stationary
analysis; King 1971 \emph{J.\ ACM} for finite-batch deviation bounds).
For production deployment, the cleanest operational guidance remains
Theorem 7.20's average-latency formula
$\mathbb{E}[T_{\mathrm{query}}] = (1-\rho_C) \cdot K \cdot
\mathrm{cost}(M')$ plus standard service-level engineering practice
(measure tail empirically per workload). A formally-correct production
tail bound is left to future work; we explicitly do not assert one
here.
