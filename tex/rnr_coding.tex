...
\textbf{Remark 4.3b (Handling asymmetric confusion matrices).} The
restriction to symmetric $F$ in Theorem 4.3 is not hiding an
asymmetric effect. For any nonnegative confusion matrix $F$,
\[
\sum_{i, j} F_{ij} \cdot d_H(E(i), E(j))
= \sum_{i, j} \frac{F_{ij} + F_{ji}}{2} \cdot d_H(E(i), E(j)),
\]
because $d_H$ is symmetric and diagonal terms vanish. Thus an
arbitrary asymmetric empirical confusion matrix induces the same
labeling objective as its symmetric part $(F + F^T)/2$, viewed as an
undirected pair-weight matrix. The hardness consequence is therefore
not specific to symmetric inputs.
...
\item
  ...
  APX-hard, transferring the explicit $8569/8568$ constant of
  Charikar et al. (2005), but the analogous constant for $E_{12}$ is
  open.
\item
