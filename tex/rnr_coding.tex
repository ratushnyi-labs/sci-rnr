...
(non-uniform spacing adds a binary-search factor
$\log(|X|/K)$ to the index-lookup term) --- polylogarithmic in
...
\item
  ...
  sync-point spacing (non-uniform spacing adds a binary-search
  factor $\log(|X|/K)$ to the lookup term)
...
$\log(|X|/K) \cdot \log L(R)$ binary-search cost under non-uniform
spacing, plus a $K+k$ decode window). For $K = 64$ KiB and $k = 1$ KiB, the cost is roughly
...
sync-point spacing (Theorem 10.3; non-uniform spacing adds a
$\log(|X|/K)$ binary-search factor) and
