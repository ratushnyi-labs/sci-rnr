...
established that SGP is APX-hard: there exists $\gamma_{\text{SGP}} > 1$
such that no polynomial-time $\alpha$-approximation exists for
$\alpha < \gamma_{\text{SGP}}$, unless $P = NP$. (Their construction
routes through Set Cover and yields an explicit constant
$\gamma_{\text{SGP}}$ derivable from the Set Cover inapproximability
ratio.) Combined with the constant-factor approximability transfer
above, this gives: no polynomial-time
$\alpha$-approximation exists for Type-III-C with $\alpha <
\gamma_{\text{SGP}} / C$, unless $P = NP$. Since $\gamma_{\text{SGP}}
> 1$ and $C$ is an absolute constant, $\gamma := \gamma_{\text{SGP}}
/ C > 1$ exists, completing the proof. \hfill$\blacksquare$
