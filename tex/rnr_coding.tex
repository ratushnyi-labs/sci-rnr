...
decoding with cost polylogarithmic in archive size for $W$-bounded
...
\item
  Complete characterization of Type-III-C: the greedy MDL admission
  criterion (Theorem 6.5), the bits-back integration with an exact
  KL-divergence accounting (Theorem 6.6), NP-hardness of joint
  dictionary-and-predictor learning (Theorem 6.7), and APX-hardness
  transferred from the super-constant inapproximability of the
...
\item
  ...
  $O(\log(N/K) + (K + k) \cdot \mathrm{cost}(M))$ --- only
  polylogarithmic in archive size $N$ --- via Theorem 10.3 with
  Definitions 10.2, 10.3 of $W$-bounded predictors and sync points, at
  a sync-point compression cost of approximately 1 percent for typical
  parameters.
...
\item
  ...
  OS-level driver can serve LBA-indexed read requests with cost
  polylogarithmic in archive size, with the predictor running
  bit-exactly on any conforming hardware target (CPU scalar, CPU SIMD including
...
Bits-back coding (Hinton and van Camp 1993; Frey 1997, 1998;
Townsend, Bird, Barber 2019) is a technique for losslessly
compressing data
...
stochastic latents; we discuss this connection in Section 6.5.
...
\emph{\textbf{Proof.}} Fix a parameter $c \in (0, 4)$ and set
$d := (4 - c) N$ (the message dimension). Let $G$ be a
$d \times 4N$ generator matrix over $\mathrm{GF}(2)$ of a linear
...
over $N$ nibble positions: $\leq 15 N \cdot 2^{-d}$. The combined
failure probability is $\leq 19 N \cdot 2^{-d} = 19 N \cdot 2^{-(4-c)N}$,
which is $o(1)$ as $N \to \infty$ for any fixed $c \in (0, 4)$
(and $O(2^{-\delta N})$ for any $\delta < 4 - c$).

Let $S_N \subset \{0, \ldots, 15\}^N$ be the image of all $2^d$
messages under $G$, with the $4N$ output bits parsed as $N$ nibbles.
Property \textbf{(P2)} in particular implies that $G$ has full row
rank $d$ (otherwise some non-trivial combination of the $d$ rows
would vanish on \emph{all} columns, contradicting linear independence
of any 4 of them), so the encoding map $m \mapsto mG$ is injective
and $|S_N| = 2^d = 2^{(4-c)N}$. Define $D_N$ as the uniform
distribution on $S_N$.
...
$\log_2 |S_N| + \varepsilon_N = d + \varepsilon_N = (4 - c)N +
\varepsilon_N$ bits. The savings over the factorized bound (ii) are
$4N - d = cN$ bits per block. Describing the generator matrix $G$
itself (one-time, amortized across the block) costs $L(G) = d \cdot
4N = 4(4-c)N^2$ bits, absorbed into the $L(M_{\text{pos}})$ term of
(3.1) and amortized across many blocks encoded with the same $G$;
per-block it does not enter the rate.
...
\]
where $L(\text{counts})$ is the cost of describing the count vector
$(k_0, \ldots, k_{15})$: under uniform $D_N$ on $S_N$, counts
concentrate within $O(\sqrt{N})$ of $N/16$ by Chernoff, so
$L(\text{counts}) = O(\log N)$ bits suffice. Combined,
$\Delta(N) = cN - \varepsilon_N - O(\log N) = \Theta(N)$ for any
fixed $c \in (0, 4)$. \hfill$\blacksquare$
...
$\mathrm{code\_cost}$ of other tokens. The global problem is
addressed by Theorems 6.7 and 6.8 below, where it is shown to be
NP-hard and APX-hard. For a corpus encoded across many archives,
...
dictionary class introduced in §6.5.}
...
§6.5 above, i.e., each token $\tau_i \in D$ is specified by a
...
exists a constant $\gamma > 1$ such that no polynomial-time
algorithm achieves approximation ratio better than $\gamma$ on all
sufficiently large instances, unless $P = NP$. The bound holds even
with $M$ restricted to the trivial constant predictor and $D$
restricted to the recursive dictionary class.}
...
\emph{Symbol-count formulation.} The Type-III-C objective in
Theorem 6.7 was specified in \emph{symbol count}: $L(D) = \sum_i
|\alpha_i| + K \cdot \lceil \log_2 K \rceil$ measures the total
length of all token right-hand sides in symbols, plus per-token
bookkeeping. We retain that convention here. (The bit-level objective
of \S6.5 differs from the symbol-count objective by a per-symbol
factor of $\lceil \log_2(|\Sigma| + K) \rceil$, which is bounded
above and below by constants on instances of bounded dictionary
size; the APX-hardness conclusion is invariant under this rescaling
because APX-hardness is preserved by linear transformations of the
objective with bounded multiplicative slack.)

\emph{Bijection preserves approximability.} Let $\mathrm{OPT}_{\text{SGP}}(s) = |G^\star|$
denote the size of the smallest grammar deriving $\{s\}$. From the
bijection in the proof of Theorem 6.7,
\[
L_{\text{III-C}}(D) = |G(D)| + K(D) \cdot \lceil \log_2 K(D) \rceil + O(\log N).
\]
Under the standard SGP normalization (each rule has right-hand-side
length $\geq 2$, eliminating unit productions), $K(D) \leq |G(D)|/2$,
so $K(D) \log K(D) = O(|G(D)| \log |G(D)|)$. Furthermore, any
instance with $|G^\star| = O(1)$ is solvable in polynomial time by
exhaustive enumeration over grammars of bounded size; the
interesting regime is $|G^\star| \to \infty$ with input length $N$,
and in that regime the bookkeeping satisfies $K \log K + \log N =
O(|G| \log |G|)$.

Therefore, an $\alpha$-approximation for $L_{\text{III-C}}$ yields a
grammar $G_A$ with
\[
|G_A| \leq L_{\text{III-C}}(D_A) \leq \alpha \cdot L_{\text{III-C}}(\text{OPT}_{\text{III-C}}) \leq \alpha \cdot c_{\text{enc}} \cdot |G^\star|,
\]
where $c_{\text{enc}} = O(\log |G^\star|)$ in worst case but is
bounded by a constant in any specific encoding-convention regime
(e.g., $c_{\text{enc}} \leq C$ for some constant $C$ when
$|G^\star| \leq |s|$ and $|s|$ ranges over a bounded family, which is
the standard APX-hardness setting). Hence an $\alpha$-approximation
for Type-III-C gives a $(C \cdot \alpha)$-approximation for SGP for
some absolute constant $C$.

\emph{Transfer of APX-hardness.} Charikar et al. (2005, Theorem 3.4)
established a $\Omega(\log n / \log \log n)$ inapproximability
threshold for SGP via a reduction from Set Cover and the Lund and
Yannakakis (1994) inapproximability of Set Cover. Their bound is
super-constant in the input size $n = |s|$; in particular, for any
constant $\gamma > 1$ there exists $n_0$ such that no polynomial-time
$\gamma$-approximation to SGP exists on instances of size $\geq n_0$,
unless $P = NP$. Combined with the constant-factor approximability
transfer above ($\alpha$-approximation for Type-III-C yields a
$(C \alpha)$-approximation for SGP), this gives: no polynomial-time
$\alpha$-approximation exists for Type-III-C with $\alpha \leq
\gamma / C$ for any constant $\gamma > 1$ and sufficiently large
instances, unless $P = NP$. Existence of such a constant $\gamma > 1$
suffices for APX-hardness of Type-III-C. \hfill$\blacksquare$

\textbf{Remark on the constant.} The explicit numerical value of
$\gamma$ depends on the dictionary-encoding convention (bit-cost
vs.\ symbol-cost, choice of bookkeeping format). We claim only that
\emph{some} constant $\gamma > 1$ exists; pinning down the optimal
constant for a specific encoding is left to follow-up work.
...
$O(\log(N/K) + (K + k) \cdot \mathrm{cost}(M))$. The second term
dominates whenever $(K+k) \cdot \mathrm{cost}(M) \gg \log(N/K)$, which
holds for any non-trivial predictor; in that regime the cost is
$O((K+k) \cdot \mathrm{cost}(M))$, with only a polylogarithmic
dependence on archive size $N$.}
...
$\log(N/K)$ seek-index lookup plus a $K+k$ decode window).
For $K = 64$ KiB and $k = 1$ KiB,
...
\item
  Random access (seek-and-read) into a compressed archive with cost
  polylogarithmic in archive size --- resolved by Definitions 10.2
  ($W$-bounded predictor) and 10.3 (sync point) and Theorem 10.3
  (random access in $O(\log(N/K) + (K + k) \cdot \mathrm{cost}(M))$
  time), with Theorem 10.4 covering non-$W$-bounded predictors via
  state snapshots. This is the
...
\item
  ...
  read requests with cost polylogarithmic in archive size) and Theorem
