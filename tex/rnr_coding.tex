...
$(k_0, \ldots, k_{15})$. Since each $k_v \in \{0, \ldots, N\}$, the
count vector lies in a set of size at most $(N+1)^{16}$, hence
$L(\text{counts}) \leq 16 \log_2(N+1) = O(\log N)$ bits suffice by
direct enumeration. (We do not invoke a Chernoff concentration
argument because positions in the linear-code construction are
correlated by membership in $S_N$, so per-position independence is
...
joint entropy $H(D_N) = (4 - c)N$ on the same $D_N$ and is
\emph{not} separated by Theorem 5.4; the relevant boundary is
...
$k = 2$, $s = 2$ verified in
\texttt{scripts/verify/prop\_5\_7\_counterexample.py}). Even when
$A_s(\alpha)$ is non-decreasing, the expected value of
$A_s(|M_v \setminus S_v|)$ depends on the full joint law of the
random set $M_v$, not only on the marginals $Q_v(i) = \Pr(i \in M_v)$.}
...
\item
  ...
  §13.2.
\item
  Optimal candidate-support threshold for Type-II --- open. The
  natural marginal-$Q$-prefix heuristic is not provably optimal in the
  semi-non-factorized regime (Remark 5.7, with a concrete
  counterexample at $N = 7, k = 2, s = 2$ in the verification
  scripts). The arithmetic-coding benchmark under the same $Q$ is a
  separate result and remains the comparison baseline.
\item
