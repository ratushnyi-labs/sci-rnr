...
\]
Using
$\log_2 \binom{N}{k} = N \cdot H_2(k/N) - \tfrac{1}{2} \log_2(2\pi N p(1-p)) + O(1/N)$,
the four binomials evaluate to approximately $203.6$, $172.3$,
$124.2$, and $0$ bits respectively, giving a total of
$13 \cdot 203.6 + 172.3 + 124.2 + 0 \approx 2943$ bits. Compared to
the multinomial baseline of $4021$ bits, the apparent gain is
$4021 - 2943 \approx 1078$ bits per block. Adding a model cost of,
say, $200$ bytes $= 1600$ bits amortized over $m = 8$ blocks gives
$200$ bits per block, leaving an effective gain of approximately
$878$ bits per block. This is a saving of roughly $110$ bytes per
$512$-byte equivalent block, a competitive figure on structured
positional data.
...
capability; decoding byte $p$ requires processing all $p$ previous
bytes sequentially, with a wall-clock cost proportional to $p$. This
is a deployment blocker for archive-style use cases where partial
reads are common. The RNR framework supports random access in time
independent of $|X|$, provided the predictor has bounded context. We
make this precise.
...
positions. Let $|X|$ denote the archive's uncompressed source
length. Random access to $k$ consecutive bytes starting at any
position $p \in [0, |X|)$ requires: (i)~$O(\log(|X|/K) \cdot \log L(R))$
bits read from the seek index, comprising $O(\log(|X|/K))$ index
entries probed during binary search, each entry containing a
$\lceil \log_2 L(R)\rceil$-bit repair-stream offset (or $O(\log
L(R))$ bits to read a single entry once the index is in memory);
...
$O(\log(|X|/K) \cdot \log L(R) + (K + k) \cdot \mathrm{cost}(M))$.
\emph{This is polylogarithmic in archive size $|X|$ only when $K$
and $k$ are held fixed or grow at most polylogarithmically in $|X|$
and $L(R) = \mathrm{poly}(|X|)$; for $K = \Theta(|X|)$ or $k =
\Theta(|X|)$ the cost is $\Theta(|X| \cdot \mathrm{cost}(M))$, no
better than full sequential decode}. When $(K+k) \cdot
\mathrm{cost}(M) \gg \log(|X|/K) \cdot \log L(R)$ the second term
dominates and the cost is $O((K+k) \cdot \mathrm{cost}(M))$,
independent of $|X|$ in that parameter regime.}

\emph{\textbf{Proof.}} Binary-search the seek index for the largest
sync point $P$ with $P \leq p$; the index has at most
$\lceil |X|/K \rceil$ entries, each of width
$\lceil \log_2 L(R)\rceil + O(1)$ bits, so the binary search reads
$O(\log(|X|/K) \cdot \log L(R))$ bits in total (or $O(\log L(R))$
bits per probed entry if the index is in memory). By
...
$O((|X|/K) \cdot (\log L(R) + L(\text{state})))$ bits. The random-access
cost per query (to $k$ bytes at position $p$) is
$O\bigl(\log(|X|/K) \cdot (\log L(R) + L(\text{state})) + (K + k) \cdot
\mathrm{cost}(M)\bigr)$, where the
$O(\log(|X|/K)) \cdot (\log L(R) + L(\text{state}))$ term covers
binary search through the augmented index plus loading and
deserializing the snapshot before decoding can resume.}
...
$[p, p+k)$, locate the latest sync point $P \leq p$ by binary search
on the augmented index (cost $O(\log(|X|/K) \cdot (\log L(R) +
L(\text{state})))$ since each probed entry is $\log L(R) +
L(\text{state})$ bits wide), read and deserialize the boundary
snapshot, restore the decoder's predictor, adaptive-parameter, and
entropy-coder boundary states, and decode
