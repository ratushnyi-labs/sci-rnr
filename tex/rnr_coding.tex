...
$W$-bounded predictors and uniform sync-point spacing
(Theorem 10.3), where $|X|$ is the uncompressed source length
(non-uniform spacing replaces the direct-lookup term with binary
search costing $O(\log(|X|/K) \cdot (\log |X| + \log L(R)))$ bits)
--- polylogarithmic in
...
\item
  ...
  sync-point spacing (non-uniform spacing replaces the direct
  lookup with binary search at cost
  $O(\log(|X|/K) \cdot (\log |X| + \log L(R)))$ bits, since each
  entry carries both a source-position key and a repair-stream
  offset)
...
The RNR framework supports random access in per-query time that is
...
seek index is $\Theta(|X|/K)$. We make this precise.
...
encoder would have used in a sync-point-free encoding. Under the
probability floor, both cold and warm per-byte costs lie in $[0,
\log_2(1/\eta)]$, so each summand
$\log_2(1/\eta) - (-\log_2 \Pr_M(X_i \mid C_i^{\mathrm{warm}}))$
lies in $[0, \log_2(1/\eta)]$ (non-negative because warm cost is
bounded by $\log_2(1/\eta)$ under the floor, eliminating the
``warm exceeds cold ceiling'' case that would otherwise allow
negative summands). The trivial upper bound
$W \cdot \log_2(1/\eta)$ is loose and does not
...
RNR using a \emph{per-position sequential} coding mode (Type-I, or
the per-byte sequential branches of Type-III-A; explicitly
excluding modes whose repair-stream encoding spans the entire block
as a single combinatorial object, such as Type-II's
multinomial/partition enumerative rank coding without
sub-block sync, see Remark 10.3a below) with a $W$-bounded
predictor $M$ satisfying Theorem 10.1 and with sync points placed
at uniformly spaced source positions $j K$ for $j = 0, 1, \dots$
(Definition 10.3). Assume the entropy coder operates under a
predictor probability floor $\eta > 0$ (Section 10.7), so each
position consumes at most $\log_2(1/\eta) = O(1)$ repair-stream
bits. Let $|X|$ denote the archive's uncompressed source length.
...
$O(\log(|X|/K) \cdot (\log |X| + \log L(R)))$ bits; the rest of
the analysis is unchanged.) \emph{This cost is polylogarithmic in archive size
...
each entry is $\lceil \log_2|X|\rceil + \lceil \log_2 L(R)\rceil + O(1)$
bits wide, and binary search over the index costs
$O(\log(|X|/K) \cdot (\log |X| + \log L(R)))$ bits in total; the
uniform case avoids both the $\log(|X|/K)$ factor and the
...
random access has \emph{sub-block granularity}: locating the
sub-block containing source position $p$ costs $O(\log L(R))$
under uniform spacing as in Theorem 10.3, but decoding it costs
$O(K \cdot \mathrm{unrank}(K))$ (the cost of unranking the
combinatorial object on $K$ positions) and produces all $K$ bytes
of the sub-block rather than $k$ bytes around $p$. Byte-granular
access within the sub-block requires decoding the whole sub-block,
not just the prefix up to $p+k$ as in Theorem 10.3's per-position
sequential decode. This is a strictly weaker random-access
property than the per-position sequential case.
...
$\log L(R)$ direct seek-index cost under uniform spacing, or a
$\log(|X|/K) \cdot (\log |X| + \log L(R))$ binary-search cost
under non-uniform spacing, plus a $K+k$ decode window). For $K = 64$ KiB and $k = 1$ KiB, the cost is roughly
...
$K$ positions: per-query random access in $O(\log L(R) + (K + k)
\cdot \mathrm{cost}(M))$ time for uniform spacing (or with an
extra $\log(|X|/K)$ factor for non-uniform), independent of $|X|$
in the predictor-cost term and polylog in $|X|$ in the
seek-index-lookup term, at a per-sync-point worst-case excess of
$W \cdot \log_2(1/\eta)$ bits (Section 10.7), giving a worst-case
total of $(|X|/K) \cdot W \cdot \log_2(1/\eta)$ bits over the
archive. For $K = 64$ KiB, $W = 4$ KiB,
$\eta = 2^{-12}$ (standard small uniform escape mixed with the
predictor's main distribution; $\eta = 2^{-8}$ would force a
uniform predictor and contradict any cold-vs-warm gap below), and
a 1 GiB source, the worst-case bound evaluates to
$16384 \cdot 4096 \cdot 12$ bits $= 96$ MiB, or roughly $9.4\%$ of
...
overhead --- on the order of a few percent for natural data at
typical $K$ and Type-I/II/III-A/III-B coders under the
probability-floor assumption (a heuristic estimate; not a theorem),
rising to a worst-case bound that depends on $K$ and $\max_\tau
|\tau|$ for Type-III-C as detailed above --- a capability that
...
sync-point spacing (Theorem 10.3; non-uniform spacing replaces the
direct lookup with binary search at
$O(\log(|X|/K) \cdot (\log |X| + \log L(R)))$ bits) and
...
\item
  ...
  $P \leq p$ in the seek index, decodes from $P$ to $p + k'$ using
  Theorem 10.3, and returns bytes $[p, p + k')$ (a short read when
  $p + k > |X|$, an empty read when $p \geq |X|$).
\item
...
\item
  ...
  SGP literature (Lehman--Shelat 2002; Charikar et al.\ 2005) to
  the same restricted case as the existence of some
  inapproximability constant $\gamma_{\mathrm{SGP}} > 1$. The
  \emph{specific numerical value} of $\gamma_{\mathrm{SGP}}$ is
  not pinned down here and is listed in §13.2; what Theorem 6.8
  establishes is only the existence of such a constant. Hardness
  of the \emph{unrestricted} joint problem is conjectured but not
  proved by the present reductions and is also listed in §13.2.
\item
...
\item
  ...
  under non-uniform spacing) for $W$-bounded predictors --- resolved
  by Definitions 10.2 ($W$-bounded predictor) and 10.3 (sync point,
  including entropy-coder flush) and Theorem 10.3. Non-$W$-bounded
  ...
  their published form (Theorem 10.3 in principle gives them
  random access if their encoders were modified to do so), while
  block-codec schemes (Bit-Swap, IDF++, HiLLoC) provide only
  block-granular access (not byte-granular) due to their
  block-coupled latent/ANS state.
\item
