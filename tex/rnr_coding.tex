...
$W$-Markov rate with byte-granular random access).} \emph{Let
$X \sim D_N$ be a block of $N$ symbols over a $\sigma$-symbol
alphabet, and let $M$ be a $W$-bounded predictor (Definition 10.2)
that at each position $i$ outputs the order-$W$ Markov chain-rule
conditional distribution
$P(x_i \mid x_{i-W}, \dots, x_{i-1}, C)$ (necessarily depending on
at most the last $W$ bytes, by Definition 10.2). Per-position
sequential arithmetic coding of $X$ under these conditionals
achieves expected total length}
\[
H_W(D_N) \;:=\; \sum_{i=1}^N H(X_i \mid X_{\max(1, i-W)}, \dots, X_{i-1}, C)
\;\;\leq\;\; N \log_2 \sigma,
\]
\emph{plus $\varepsilon_N \cdot N$ entropy-coder redundancy, where
$H_W(D_N)$ is the joint entropy under the $W$-Markov projection of
$D_N$. In general $H_W(D_N) \geq H(D_N)$ with equality iff $D_N$ is
order-$W$ Markov given $C$ (the gap is the multi-information beyond
order $W$). Combined with Definition 10.3 sync points, this coding
admits byte-granular random access via Theorem 10.3 at
$O(\log L(R) + (K+k) \cdot \mathrm{cost}(M))$ per query.}

\emph{\textbf{Proof.}} The order-$W$ Markov chain rule gives
...
beyond conditioning on the full past. By the $W$-bounded property,
the coding is per-position sequential in the sense of Theorem
10.3's scope; applying Theorem 10.3 directly gives byte-granular
random access. \hfill$\blacksquare$
...
\emph{\textbf{Proof.}} The permutation-like distribution is the
\emph{conditional Bernoulli design} of order $k_v$: it can be
realized as $N$ independent Bernoullis with parameters $q_i$
conditioned on $\sum_i \mathbf{1}[i \in M_v] = k_v$. Under this
distribution, $\Pr(i \in M_v)$ is monotone in $q_i$ (the higher
$q_i$, the higher the conditional inclusion probability), so the
ordering of positions by $q_i$ matches the ordering by $Q_v(i)$.
...
$M_v \cap \{i, i^\star\}$ has three possibilities
$\{\emptyset, \{i\}, \{i^\star\}, \{i, i^\star\}\}$ with weights
$1, q_i, q_{i^\star}, q_i q_{i^\star}$ (times conditional
normalization). Conditioning on the rest of $M_v$, the comparison
...
unbounded-alphabet variant of Type-III-C. For the fixed-byte
alphabet $|\Sigma| = 256$ of canonical RNR Type-III-C, encoding the
graph terminals as length-$\lceil \log_{256} |V| \rceil$ byte
strings adds a per-vertex multiplicative overhead in the grammar
encoding; the inherited inapproximability constant on the
fixed-byte case is at most $\gamma_{\mathrm{SGP}}$ (gap survives,
just with a different algebraic value), and pinning down the exact
byte-alphabet constant requires a fresh derivation that is left to
follow-up work. (ii)~\emph{Unrestricted joint problem caveat.} The
