...
\tag{6.2b.1}
\]
\emph{Moreover the per-tile-oracle target of Theorem 6.1, each tile
...
gives the floor. Writing $C_t$ for the richest context any selected mode
uses, the first inequality of (6.2b.2) is equality iff that mode is
expressive at $C_t$, and the second iff $C_t$ is causally sufficient
($H(X_t \mid C_t) = H(X_t \mid X_{<t}, Y)$). For the block-independent
specialization $C_t = Y$, the deficit (6.2b.3) becomes
$\sum_t H(X_t \mid Y) - H(X \mid Y) = \mathrm{TC}(X_1,\ldots,X_T \mid Y)$
by definition (Watanabe 1960). The optimality characterization follows
by comparing the Theorem 6.1 achievability (per-tile oracle $+ 2\sum_t
\varepsilon_t$) with the floor. \hfill$\blacksquare$
