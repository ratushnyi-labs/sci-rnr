...
stationary discrete-time random process on a finite alphabet
$\mathcal{X}$. Define the marginal entropy
...
\emph{Chaos} 13(1):25 for ``excess entropy'' in this sense), then
$\delta_N \to \delta_\infty < \infty$ as $N \to \infty$ and
\[
...
dependence (Lemma 5.6c). Specific applications:
text-byte-streams with English-language bigram structure realize
$\mathrm{TC}(D_N) \approx N \cdot (H_{\mathrm{marg}} - h_{\mathrm{en}})$
with $H_{\mathrm{marg}} - h_{\mathrm{en}} \approx 4.7$ bits/byte
(Shannon 1951 estimate); $1$ MiB English text accumulates
$\approx 4.7 \times 2^{20} \approx 4.9$ Mbits of joint structure
inaccessible to a per-position factorized coder. The verification
script
...
\item
  ...
  any mixing process with summable correlations; covers natural
  text-byte streams with $E \approx 4.7$ bits/byte by Shannon's
  1951 estimate).
