...
saturating near $1$ as $C \to \infty$ (using the
Euler--Maclaurin tail $\sum_{k>C} k^{-\alpha} \sim C^{1-\alpha}/(\alpha-1)$);
values near $\alpha = 1$ require the $\alpha = 1$-harmonic limit
...
$\alpha = 1.5$ (heavily skewed) and $m = 10^6$, $C = 100$:
$\rho_C \approx 1 - C^{1-\alpha}/((\alpha-1)\,\zeta(\alpha))
= 1 - 100^{-0.5}/(0.5 \cdot 2.612) \approx 1 - 0.077 \approx 0.92$
(92\% hit rate). For the typical Breslau measurement range
$\alpha \in [0.7, 0.85]$ (heavy-tailed web traces), $\rho_C \approx
(C/m)^{1-\alpha}$: at $C=100, m= 10^6, \alpha=0.8$ this gives
$\rho_C \approx (10^{-4})^{0.2} \approx 0.16$ (16\% hit rate, modest
but still better than uniform $C/m = 10^{-4}$). Amortized cost
reduced by factor $1/(1-\rho_C)$: $\sim 12\times$ for $\alpha=1.5$,
$\sim 1.2\times$ for the $\alpha=0.8$ heavy-tailed web-trace regime.
