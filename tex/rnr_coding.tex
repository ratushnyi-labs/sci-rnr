...
H(X \mid Y) = \mathrm{TC}(X_1,\ldots,X_T \mid Y)$. Consequently
Type-III-A attains the absolute floor $H(X\mid Y)$ (within the
$2\sum_t \varepsilon_t$ of Theorem 6.1) \textbf{iff, per tile and per
context value, the selected mode is expressive AND its context is
causally sufficient}; otherwise it overpays by the mode-inexpressiveness
...
causal-adaptive, not global-commit).} Getting the per-tile benchmark
...
\item[(ii)] the \emph{causal-adaptive} floor $R_{\mathrm{tile}} =
  ...
  (6.2b.2);
\item[(iii)] the \emph{global-commit} cost $\sum_t \min_m
  \mathbb{E}[L_m(t)]$, where a single mode is fixed for the whole source.
\end{itemize}
These obey $(\mathrm{i}) \le (\mathrm{ii}) \le (\mathrm{iii})$, and
(iii) is in general a \emph{strict over-estimate} of the operative cost:
fixing one global mode cannot exploit context the way the operative
selector does, so (iii) is \emph{not} a valid rate floor. Concretely,
...
global-commit cost is $1 + \min(2.1875, 2.1875) = 3.1875$, whereas the
operative causal-adaptive selector --- mode $A$ when $X_1{=}0$, mode $B$
when $X_1{=}1$, chosen flag-free from the already-decoded $X_1$ ---
attains $1 + 1.75 = 2.75 = H(X)$. Thus the binding floor is (ii):
