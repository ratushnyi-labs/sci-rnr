...
coder redundancy and the $N \cdot (h_W - h)$ term which vanishes for
order-$W'$ Markov). For general exponentially mixing non-Markov
sources the relationship is more subtle: the per-block lower-vs-upper
gap is $K (h_W - h) + \delta_W^{(h_W)} - \delta_K$, which can be
positive or negative depending on the source (concrete example:
$X_i = Y \oplus Z_i$ with $Y$ a hidden fair bit and $Z_i$ iid Bernoulli
gives $m \cdot \delta_K - \delta_N > (m-1) \cdot \delta_W$ for small
$W, K$). The achievability and lower bound coincide tightly only in
the order-$W'$ Markov sub-case; for general exp-mixing the two
characterize different orderings of the same per-block cost
$L_{\mathrm{sub}} = K h_W + \delta_W^{(h_W)}$ versus its
Shannon-minimum counterpart $H(D_K)$.}

\emph{(c) Open extensions.} \emph{(i) Non-sub-block schemes:
state-snapshot frameworks (Theorem 10.4) store predictor state at
sync points. Such state is \emph{source-dependent} (it is a function
of prior source bytes), violating Lemma 5.1e's source-independence
condition (iii); the lower bound (b) therefore does not apply to
state-snapshot schemes. Restricted to fixed-architecture predictor
states (e.g., transformer KV cache, RNN hidden state) with
serialized cost $L(\mathrm{state})$ accounted for explicitly, the
trade-off becomes excess $(m-1) \cdot \delta_W$ vs storage
$m \cdot L(\mathrm{state})$; without such restriction, ``state''
can smuggle global source information and defeat the lower bound
trivially.
