...
$64 / 64 = 100\%$ of source size as index overhead at $K = 64$ KiB
--- effectively doubling the on-disk archive size, which is
deployment-prohibitive without raising $K$. This motivates the $W$-bounded
...
A single forward scan over a region of size $\lesssim C$ without
re-access pays the cold cost on every byte (the cache helps only
prefetch, not the decoded-region hit rate). For
...
\item
  ...
  under non-uniform spacing) --- resolved by Definitions 10.2
  ($W$-bounded predictor) and 10.3 (sync point, including
  entropy-coder flush) and Theorem 10.3, with Theorem 10.4 covering
  non-$W$-bounded predictors via state snapshots. The cost is polylogarithmic in
