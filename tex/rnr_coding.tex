...
\;+\;
\underbrace{Q \cdot (1-\rho_C) \cdot \sigma_{\mathrm{sb}} \sqrt{2 \log(2/\delta)}}_{\text{per-sub-block AC concentration}}
\;
}
\]
...
exp-mixing). Their sum has Azuma--Hoeffding tail bound with deviation
$\sqrt{Q (1-\rho_C) \cdot 2 \log(2/\delta_2)} \cdot \sigma_{\mathrm{sb}}
\leq Q (1-\rho_C) \cdot \sigma_{\mathrm{sb}} \sqrt{2 \log(2/\delta_2)}$
when $Q(1-\rho_C) \geq 1$.
...
\end{itemize}
The crossover is at $Q^* \approx (\sigma_{\mathrm{sb}}/T_{\mathrm{miss}})^2
\rho_C / (1-\rho_C)$ queries; below $Q^*$ Bernoulli noise dominates,
above $Q^*$ source noise dominates.
...
+ 7 \cdot 10^5 \cdot 320\,\mu\mathrm{s} \cdot \sqrt{29} \approx 60\,\mathrm{s}$
($0.3\%$ tail beyond expectation). Useful for capacity planning.
