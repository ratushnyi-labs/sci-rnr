...
dictionary). For each subtype we give a concrete construction,
matching achievability and converse bounds, complexity results
(including NP-hardness and APX-hardness of the two natural search
problems), and a worked example. Two structural results connect the
framework to natural data: a $\Theta(N)$ separation theorem for
semi-non-factorized Type-II via linear codes, generalized to any
source by the multi-information identity $\mathrm{TC}(D_N)$; and a
tight $2\varepsilon$ approximate-oracle bound for Type-III-A mode
selection. The framework's deployment-grade feature is random-access
decoding in time independent of archive size for $W$-bounded
predictors, at a sync-point compression cost of approximately one
percent --- matching dictionary-based seekable formats while
preserving the neural-ratio advantage, a capability absent from
existing neural compressors. Bit-exact cross-platform reproduction is
reduced to a precision-and-determinism specification satisfied by
published integer-arithmetic methods. We do not claim universal
...
reading a byte at offset $N$ in constant time independent of archive
size. Dictionary-based seekable formats (bgzip, zstd seekable) achieve
this; existing neural compressors do not, because their autoregressive
predictors require sequential decoding of all bytes $[0, N)$ before
byte $N$ is available. The author's own engineering work on a
MySQL-compatible server in Rust (with 20 storage engines, including a
compressed-archive engine and a time-series engine for columnar
metric data) exposed this gap concretely: classical compressors leave
substantial entropy unexploited on structured columnar and binary
data, while neural compressors offer better ratios at the price of
$O(N)$ per-access latency, which is not viable for an archive engine.
This practical pressure --- better ratios on structured data
\emph{plus} random access --- motivated the present framework.
...
\item
  ...
  optimal $E_{12}$ is NP-hard via reduction from QAP (Theorem 4.3)
  and inapproximable within any constant factor unless $P = NP$
  (Theorem 4.4).
\item
  ...
  Type-II advantage.
\item
  A separation theorem (Theorem 5.4) for semi-non-factorized Type-II
  via a linear-code construction, generalized by Theorem 5.5
  (multi-information separation: the exact information-theoretic
  quantity governing factorized-versus-joint coding) and quantified
  for natural data by Remark 5.6 (manifold-hypothesis quantification
  with empirical intrinsic-dimension estimates).
\item
  An optimality characterization for Type-II support sets
  (Proposition 5.7): optimal $S_v$ is a prefix of positions sorted by
  $Q_v$, and support-set coding is at best equivalent to per-position
  arithmetic coding under the same $Q$.
\item
...
\item
  Complete characterization of Type-III-C: the MDL admission
  criterion (Theorem 6.5), the bits-back integration with an exact
  KL-divergence accounting (Theorem 6.6), NP-hardness of joint
  dictionary-and-predictor learning (Theorem 6.7), and
  inapproximability with an explicit constant transferred from the
  Smallest Grammar Problem (Theorem 6.8).
\item
...
\item
  ...
  conformance specification matched by published
  low-precision-training methods.
\item
  Random-access (seek-and-read) capability in $O(K + k)$ time
  independent of archive size (Theorem 10.3 with Definitions 10.2,
  10.3 of $W$-bounded predictors and sync points), at a sync-point
  compression cost of approximately 1 percent for typical parameters.
  This functionality is absent from existing neural compressors
  (NNCP, PixelCNN++, Bit-Swap) and matches the seek capability of
  dictionary-based seekable formats while preserving the neural ratio
  advantage. Theorem 10.4 covers non-$W$-bounded predictors via state
  snapshots.
\item
  Block-device and read-only filesystem deployment with
  hardware-target portability (Theorems 10.5 and 10.6 in §10.8): an
  OS-level driver can serve LBA-indexed read requests in time
  independent of archive size, with the predictor running bit-exactly
  on any conforming hardware target (CPU scalar, CPU SIMD including
  AVX-512, GPU integer Tensor Cores, integer NPUs). This enables the
  squashfs-style mounted-compressed-filesystem use case for neural
  compression, which existing neural compressors cannot support.
\item
...
which carries its own enumerative cost. The bound is tight: when
$\alpha_v = 0$ for all $v$, the second log-binomial vanishes and the
formula reduces to $\sum_v \log_2 \binom{s_v}{k_v}$, recovering the
...
code $C_N$ satisfying the following two properties:
\begin{itemize}
\item[\textbf{(P1)}]
for every codeword position $j \in \{1, \ldots, 4N\}$, the $j$-th
coordinate of $Gm$ is a non-trivial $\mathrm{GF}(2)$-linear function
of the $3cN$ message bits $m$;
\item[\textbf{(P2)}]
for every nibble position $i \in \{1, \ldots, N\}$, the four
$\mathrm{GF}(2)$-linear forms corresponding to codeword coordinates
$4(i-1)+1, \ldots, 4i$ are linearly independent.
\end{itemize}
Such matrices exist with high probability over the uniform
distribution on $3cN \times 4N$ matrices over $\mathrm{GF}(2)$. For
\textbf{(P1)}, each individual non-triviality fails with probability
$2^{-3cN}$, and a union bound over the $4N$ coordinates gives failure
probability $\leq 4N \cdot 2^{-3cN}$. For \textbf{(P2)}, the
probability that $4$ fixed $\mathrm{GF}(2)$-linear forms in $3cN$
variables are linearly \emph{dependent} equals the probability that
one of the $2^4 - 1 = 15$ non-zero linear combinations vanishes
identically, which is at most $15 \cdot 2^{-3cN}$ (each non-zero
combination is itself a non-trivial linear form, and a non-trivial
linear form on $\mathrm{GF}(2)^{3cN}$ vanishes identically with
probability $2^{-3cN}$ over the choice of the four defining vectors;
this is a Schwartz--Zippel-type estimate over $\mathrm{GF}(2)$). A
union bound over the $N$ nibble positions gives total failure
probability $\leq 4N \cdot 2^{-3cN} + 15N \cdot 2^{-3cN} =
O(N \cdot 2^{-3cN}) = O(2^{-cN})$ for sufficiently large $N$.

Let $S_N \subset \{0, \ldots, 15\}^N$ be the image of all
$2^{(4-c)N}$ messages under $G$, with the $4N$ output bits parsed as
$N$ nibbles. By construction $|S_N| = 2^{(4-c)N}$ (the map $G$ is
injective on messages because property \textbf{(P2)} in particular
implies $G$ has full row rank, hence its kernel is trivial). Define
$D_N$ as the uniform distribution on $S_N$.
...
message $m$, which by property \textbf{(P2)} are linearly
independent. The standard fact that $k$ linearly independent
$\mathrm{GF}(2)$-linear forms applied to a uniformly random vector in
$\mathrm{GF}(2)^{3cN}$ induce the uniform distribution on
$\mathrm{GF}(2)^k$ (the joint map is a surjective $\mathrm{GF}(2)$-linear
homomorphism with kernel of size $2^{3cN - k}$, so each image has the
same preimage size) then gives: each of the 16 bit patterns occurs at
position $i$ with probability exactly $1/16$. Therefore
$Q^{\text{marg}}_v(i) = 1/16$ for all $i, v$.
...
encodes membership in $S_N$ as the unique $cN$-bit message index per
block, followed by deterministic expansion via $G$, achieves length
$\log_2 |S_N| + \varepsilon_N = (4 - c)N + \varepsilon_N$ bits.
Describing the generator matrix $G$ itself (one-time, amortized across
the block) costs $L(G) = 12 c N^2$ bits, which is absorbed into the
$L(M_{\text{pos}})$ term of (3.1) and amortized across many blocks
encoded with the same $G$; per-block it does not enter the rate.

\textbf{(iv) Separation.} Subtracting the per-block upper bound (iii)
from the per-block lower bound (ii):
\[
\Delta(N) = 4N - \left[(4 - c)N + \varepsilon_N\right] - L(\text{counts}) = cN - \varepsilon_N - L(\text{counts}),
\]
where $L(\text{counts})$ is the cost of describing the (uniform on
$D_N$) count vector $(k_0, \ldots, k_{15})$: since counts concentrate
within $O(\sqrt{N})$ of $N/16$ by Chernoff, $L(\text{counts}) = O(\log N)$
suffices to specify the deviation from the typical count. Combined,
$\Delta(N) = cN - \varepsilon_N - O(\log N) = \Theta(N)$.
\hfill$\blacksquare$
...
\textbf{Remark 5.6 (Manifold-hypothesis quantification of TC).} The
...
hypothesis therefore predicts that Theorem 5.5's separation is
...
separation theorem itself is established by Theorem 5.5 plus the
...
\textbf{Proposition 5.7 (Optimal support is a prefix; arithmetic
...
Proposition 5.7 has a practical consequence: for predictors producing
...
\textbf{Theorem 6.5 (Type-III-C MDL token admission).} \emph{Let
$\tau$ be a candidate token of byte length $\ell$ and corpus
frequency $f$, with $\mathrm{code\_cost}(\tau)$ the average bit
length to encode an occurrence of $\tau$ under the predictor and
entropy coder, $\mathrm{dictionary\_cost}(\tau)$ the bit length to
describe $\tau$ in the dictionary header, and
$\mathrm{parse\_overhead}(\tau)$ the additional per-occurrence
boundary or interval bookkeeping introduced by including $\tau$.
The net change in total Type-III-C code length resulting from
admitting $\tau$ to the dictionary $D$ is}
\[
\Delta L(\tau) = \mathrm{dictionary\_cost}(\tau) + f \cdot \left[\mathrm{code\_cost}(\tau) + \mathrm{parse\_overhead}(\tau) - 8\ell\right].
\]
\emph{Consequently, an MDL-optimal admission policy admits $\tau$ if
and only if $\Delta L(\tau) < 0$, equivalently}
\[
...
constant predictor and $D$ is the recursive (straight-line grammar)
dictionary class introduced in §6.4.}

\emph{\textbf{Proof.}} We exhibit a polynomial-time reduction from
the Smallest Grammar Problem (SGP) of Charikar, Lehman, Liu,
Panigrahy, Prabhakaran, Sahai and Shelat (2005). An instance of SGP
is a string $s \in \Sigma^\star$ over an alphabet $\Sigma$ (we may
take $\Sigma = \{0,1\}^8$ for concreteness), and the task is to find
a straight-line grammar $G$ with $L(G) = \{s\}$ minimizing the
\emph{grammar size}
$|G| = \sum_{A \to \alpha \in P} |\alpha|$, the total length over all
right-hand sides $\alpha$ (each symbol --- terminal or non-terminal
reference --- counted as one). SGP is NP-hard (Charikar et al. 2005,
Theorem 3.4).

\emph{Reduction.} Given an SGP instance $s$, construct the
Type-III-C instance $\Phi(s)$ as follows:
\begin{itemize}
\item Corpus $X := s$;
\item Predictor $M$ := the constant predictor that, at every position
where a dictionary token is required, outputs the uniform
distribution over the current dictionary $D$; the predictor's
description thus consists only of the dictionary size $K = |D|$, so
$L(M) = \lceil \log_2 N \rceil$ bits (a single integer $\leq |s| = N$);
\item Dictionary representation: tokens are recursive in the sense of
§6.4 above, i.e., each token $\tau_i \in D$ is specified by a
right-hand side $\alpha_i \in (\Sigma \cup \{\tau_0, \ldots,
\tau_{K-1}\})^\star$ over the terminal alphabet $\Sigma$ and
references to other tokens, with the dependency graph required to
be acyclic;
\item Dictionary description cost:
$L(D) := \sum_{i=0}^{K-1} \left( |\alpha_i| + \lceil \log_2 K \rceil \right)$,
where the $|\alpha_i|$ term counts the symbols in $\alpha_i$ (each
symbol coded in $\lceil \log_2(|\Sigma| + K) \rceil$ bits) and the
$\lceil \log_2 K \rceil$ term is the bookkeeping (token id and
length prefix). For asymptotic analysis only the leading $|\alpha_i|$
term matters; lower-order $\log K$ overheads are absorbed into the
$o(1)$ analysis of Theorem 6.8;
\item Repair stream: under the constant predictor, the repair
stream $R \mid Y$ consists of a single pointer to the start token
$\tau_0$ (which expands deterministically into $s$), of length
$L(R \mid Y) = \lceil \log_2 K \rceil$.
\end{itemize}

\emph{Bijection between Type-III-C dictionaries on $\Phi(s)$ and
straight-line grammars deriving $\{s\}$.} Every straight-line grammar
$G = (V, \Sigma, P, S_0)$ deriving $\{s\}$ yields a Type-III-C
dictionary $D(G)$ with $|D(G)| = |V|$ tokens, one per non-terminal,
where the right-hand side $\alpha_A$ of the production $A \to \alpha_A$
in $G$ becomes the right-hand side of token $\tau_A$ in $D(G)$ with
non-terminal references replaced by token references; conversely,
every recursive dictionary $D$ whose start-token $\tau_0$ expands
deterministically to $s$ yields a straight-line grammar
$G(D)$ deriving $\{s\}$ with $|V(G(D))| = |D|$ non-terminals and
$|G(D)| = \sum_i |\alpha_i|$. The two maps are mutual inverses, and
under them
\[
L(D) = |G(D)| + |D(G)| \cdot \lceil \log_2 |D(G)| \rceil = |G| + O(K \log K).
\]
With $L(M) + L(R \mid Y) = O(\log N) + O(\log K) = O(\log N)$,
the total Type-III-C objective satisfies
\[
L_{\text{Type-III-C}}(D) = |G(D)| + O(K \log K) + O(\log N).
\]

\emph{Consequence.} A polynomial-time algorithm that minimizes
$L_{\text{Type-III-C}}$ on $\Phi(s)$ would, by the bijection,
minimize $|G|$ over straight-line grammars deriving $\{s\}$ (up to
the lower-order $O(K \log K) + O(\log N)$ term, which is sub-linear
in $|G^\star|$ whenever $|G^\star| = \omega(\log N)$ --- a regime that
holds for all SGP instances with $|s| \geq 2$ distinct symbols,
since $|G^\star| \geq |s| / \log |s|$ by a simple counting argument).
This would yield a polynomial-time solver for SGP, contradicting
Charikar et al. (2005, Theorem 3.4). The reduction $s \mapsto \Phi(s)$
is computable in linear time in $|s|$.

The general case (non-constant predictor $M$, or flat-only
dictionary) is at least as hard as the constant-$M$, recursive-$D$
case treated here, and therefore inherits NP-hardness.
\hfill$\blacksquare$
...
exists an explicit constant $\gamma > 1$ such that, for every
$\eta > 0$, no polynomial-time algorithm achieves approximation
ratio better than $\gamma - \eta$ on all sufficiently large
instances, unless $P = NP$. The bound holds even with $M$ restricted
to the trivial constant predictor and $D$ restricted to the recursive
dictionary class.}

\emph{\textbf{Proof.}} We verify that the reduction
$\Phi: s \mapsto \Phi(s)$ constructed in Theorem 6.7 is asymptotically
gap-preserving, then transfer the explicit SGP inapproximability
constant of Charikar et al. (2005).

\emph{Gap-preservation.} Let $\mathrm{OPT}_{\text{SGP}}(s) = |G^\star|$
denote the size of the smallest grammar deriving $\{s\}$, and let
$\mathrm{OPT}_{\text{III-C}}(\Phi(s))$ denote the optimal Type-III-C
code length on the constructed instance. From the bijection in the
proof of Theorem 6.7,
\[
\mathrm{OPT}_{\text{III-C}}(\Phi(s)) = \mathrm{OPT}_{\text{SGP}}(s) + r(s),
\quad \text{where } r(s) = O\!\left(K^\star \log K^\star\right) + O(\log N)
\]
and $K^\star \leq |G^\star|$ is the number of non-terminals in $G^\star$.
For instances with $|G^\star| \to \infty$, the additive term satisfies
$r(s) = o(|G^\star|)$. Therefore, for any approximation algorithm $A$
producing $A(\Phi(s)) \leq \alpha \cdot \mathrm{OPT}_{\text{III-C}}(\Phi(s))$,
the induced grammar $G_A$ has size
\[
|G_A| \leq A(\Phi(s)) \leq \alpha \cdot \left[\mathrm{OPT}_{\text{SGP}}(s) + r(s)\right]
= \left(\alpha + \alpha \cdot \tfrac{r(s)}{\mathrm{OPT}_{\text{SGP}}(s)}\right) \cdot \mathrm{OPT}_{\text{SGP}}(s).
\]
The slack factor $\alpha \cdot r(s) / \mathrm{OPT}_{\text{SGP}}(s) \to 0$
as $|s| \to \infty$. Hence an $\alpha$-approximation for the
Type-III-C dictionary problem yields an $(\alpha + o(1))$-approximation
for SGP.

\emph{Transfer of explicit constant.} Charikar et al. (2005, Theorem 3.4
and Corollary 3.5) established that there exists an explicit constant
$\gamma_{\text{SGP}} > 1$ such that no polynomial-time algorithm
$(\gamma_{\text{SGP}} - \eta)$-approximates SGP for any $\eta > 0$,
unless $P = NP$. (Their construction routes through an instance of
Set Cover with explicit inapproximability gap; the resulting
$\gamma_{\text{SGP}}$ is a constant strictly greater than 1, derivable
from the Set Cover inapproximability constant via their reduction
chain.) By the gap-preservation above, the same lower bound transfers
to the Type-III-C dictionary problem: for every $\eta' > 0$, no
polynomial-time algorithm $(\gamma_{\text{SGP}} - \eta')$-approximates
the Type-III-C objective on all sufficiently large instances, unless
$P = NP$. Set $\gamma := \gamma_{\text{SGP}}$ to complete the proof.
\hfill$\blacksquare$
...
support threshold $\tau_v$ in Proposition 5.7, the tile size in
...
\item
  Range-decode function. Given $(N, k)$, looks up the latest sync
  point $P \leq N$ in the seek index, decodes from $P$ to $N + k$
...
$I(H ; L \mid C)$ estimates of Section 6.5. On structured numeric
...
\item
  Computational complexity of optimal $E_{12}$ --- resolved by
  Theorem 4.3 (NP-hardness via QAP) and Theorem 4.4
  (inapproximability: no polynomial-time constant-factor
  approximation exists unless $P = NP$).
\item
  Optimal candidate-support threshold for Type-II --- resolved by
  Proposition 5.7: the optimal $S_v$ is a prefix of positions sorted
  by $Q_v$, and support-set coding is at best equivalent to
  per-position arithmetic coding under the same $Q$ (strictly more
  expensive unless $Q$ is binary).
\item
  Joint structure on natural data and the Type-II separation regime
  --- resolved by Theorem 5.5 (the multi-information
  $\mathrm{TC}(D_N)$ is the exact information-theoretic quantity
  governing factorized-versus-joint separation) and quantified by
  Remark 5.6 (the manifold hypothesis predicts
  $\mathrm{TC} = \Theta(N)$ for natural data classes of low intrinsic
  dimension, supported by empirical estimates of $d$ for images,
  text, and structured records).
\item
...
\item
  ...
  resolved by Theorem 6.6: the three-stream accounting yields rate
  $H(X \mid Y) + \mathrm{KL}(q \,\Vert\, P_{\text{posterior}})$ in
  expectation, matching the Slepian-Wolf bound when $q$ is exact.
\item
  Computational complexity of joint dictionary-and-predictor
  learning --- resolved by Theorem 6.7 (NP-hardness via SGP) and
  Theorem 6.8 (APX-hardness: an explicit inapproximability constant
  transferred from the Charikar et al. SGP bound).
\item
...
\item
  ...
  ($W$-bounded predictor) and 10.3 (sync point) and Theorem 10.3
  (random access in $O(K + k)$ time), with Theorem 10.4 covering
  non-$W$-bounded predictors via state snapshots. This is the
  framework's deployment-grade distinguishing feature relative to
  other neural compressors, which lack random-access support
  entirely.
\item
  Block-device and read-only filesystem deployment with
  hardware-target portability --- resolved by Theorem 10.5
  (block-device interface: an OS-level driver serves LBA-indexed
  read requests in time independent of archive size) and Theorem
  10.6 (hardware-target portability: bit-exact predictor execution
  on CPU scalar, CPU SIMD, GPU integer Tensor Cores, and integer
  NPUs follows from Theorem 10.1's conformance specification). The
  copy-on-write overlay pattern (§10.8) enables read-write semantics
  using the same architecture as existing compressed-filesystem
  deployments.
\end{itemize}
...
\item
  ...
  for low-dimensional natural-data manifolds. Constructing an
  efficient (polynomial-time) membership oracle and rank coder for
  the submanifold of a specific data class --- say, database pages
  under a given schema, or valid x86-64 instruction streams --- is
  open as a constructive question for each class. The existence of
  such oracles follows from the theorem (the submanifold is
  enumerable), but the polynomial-time construction is
  data-class-specific.
\item
  Tight inapproximability constants. Theorems 4.4 and 6.8 establish
  inapproximability via reductions from QAP and SGP respectively.
  The exact inapproximability constants --- the largest $\alpha$
  and $\gamma$ for which $\alpha$-approximation and
  $\gamma$-approximation are NP-hard --- inherit the corresponding
  constants from QAP and SGP, which are themselves only partially
  characterized. Improving the inapproximability constants for the
  source problems would automatically improve them for $E_{12}$ and
  dictionary learning.
\item
