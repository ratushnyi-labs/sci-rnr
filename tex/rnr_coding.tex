...
auxiliary bits. The expected net length is the \emph{negative
ELBO}: $\mathbb{E}_q[-\log P(x, z) + \log q(z \mid x)]
= \mathbb{E}[-\log P_X(x)] + \mathbb{E}\,\mathrm{KL}(q(z \mid x) \Vert P(z \mid x))$.
This equals the source entropy $H(X)$ only when the model marginal
