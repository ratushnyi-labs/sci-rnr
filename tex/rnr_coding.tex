...
\item
  ...
  We conjecture that for every fixed $\delta > 0$, no polynomial-time
  $(8569/8568 - \delta)$-approximation algorithm exists for
  unrestricted bit-cost SGP on sufficiently large
  Berman-Karpinski / Charikar-hard instances, unless
  $\mathrm{P} = \mathrm{NP}$. Motivation: Lemma 6.8b establishes
  this for the Charikar normal-form class; the unrestricted case
  differs only in admitting escape grammars with cover-independent
  nonterminal structure, but no such escape grammar is known to
  break the symbol-count APX-hardness bound, and the multiplicative
  $\log_2(|\Sigma| + K)$ bit-cost weight is slowly varying with $K$
  in the hardness regime. A polynomial-time
  $(\gamma_{\mathrm{SGP}} - \delta)$-approximation algorithm for any
  $\delta > 0$ would refute this conjecture.
\item
...
\item
  ...
  problem combinatorially non-trivial. $E_{12}$ therefore occupies
