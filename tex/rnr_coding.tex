...
does not transfer to a multiplicative gap on $L_{\mathrm{amort}}$:
both YES and NO instances admit the same family-storing
predictor $M^*$ with the same $O(\mathrm{poly}(n))$ amortised
cost, so the YES/NO amortised ratio is $1 + o(1)$. The MSGP
gap is absorbed because the family-storing predictor $M^*$
achieves $L_{\mathrm{amort}}$ much smaller than the
...
deterministic polynomial-time reduction $\mathrm{Red}_n:
\{0,1\}^{|d_n|} \times [N] \to \Sigma^*$ from a
$\mathrm{poly}(n)$-bit family description $d_n$, with $N(n) =
2^{\Theta(n)}$ and $|\mathrm{Red}_n| + |d_n| = \mathrm{poly}(n)$
bits. The amortised-family bit-cost objective
...
O(\mathrm{poly}(n))$ for the universal decoder $D^*$ and
trivial residuals $R_i^* := i$. Consequently, the optimum
amortised cost satisfies $\mathrm{OPT}_{\mathrm{amort}}(\mathcal{F}_n)
\leq O(\mathrm{poly}(n))$ on any family $\mathcal{F}_n$
produced by such a reduction, \emph{independent of} the
underlying $\Pi$-objective gap. Hence APX-hardness of $\Pi$
with any constant gap $\gamma_\Pi > 1$ on the $\Pi$-objective
does \emph{not} transfer to APX-hardness of $L_{\mathrm{amort}}$
on this family: both YES and NO instances admit the same
family-storing predictor with the same $O(\mathrm{poly}(n))$
amortised cost, so the amortised YES/NO ratio is $1 + o(1)$.}
...
$\mathrm{Red}_n$ to reproduce the family --- the bound holds
uniformly across YES and NO instances of any reduction
$\Pi$-YES vs.\ $\Pi$-NO based on the same family-description
format. Both admit the same $M^*$ construction (with the
appropriate $d_n$), achieving the same $O(\mathrm{poly}(n))$
amortised cost. The YES/NO amortised ratio is therefore
$O(1)$ but does \emph{not} preserve $\gamma_\Pi$ since
$\gamma_\Pi$ acted on a quantity ($|G^*_\Pi|$, $|\mathrm{VC}^*|$,
etc.) that does not appear in $L_{\mathrm{amort}}$. $\Box$
...
reduction algorithm in items~1--2, predictor bypass of the MSGP
solver via family reproduction in item~3, all unified by
Lemma~13.4.2a), no combinatorial-poly\-no\-mial-time route in
this list survives.
...
maps to Type-II through the affine $n_0(S)$ identity, and any
multiplicative source gap $g$ in the natural small-density regime
dilutes to $1+\Theta\!\bigl((D_{\mathrm{YES}}/m)\bigr)=1+\Theta(g^{-(r-1)})$
for $r$-uniform atoms.} Escaping OP4 hence requires a Type-II hardness
...
\textbf{{[}128{]}} Niyogi, P., Smale, S., and Weinberger, S. (2008).
...
\textbf{{[}129{]}} Muirhead, R. J. (1982). Aspects of Multivariate
Statistical Theory. Wiley.

\textbf{{[}130{]}} Diaconis, P., and Freedman, D. (1987).
