...
\emph{Engineering companion to the framework paper, v0.7}
...
\textbf{R-3.2 (Overflow avoidance).} The accumulator bit-width MUST be
at least ceil(log\_2(L * W\_max * I\_max)) + 1, where L is the maximum
layer depth, W\_max is an upper bound on the L1 norm of any weight row,
and I\_max is an upper bound on the magnitude of any layer input. The
bounds W\_max and I\_max MUST be declared in the model description; an
...
Sigmoid(x) = 1 / (1 + exp(-x)). The integer table is parameterized by an
input scale s\_in and output scale s\_out: table{[}i{]} =
round(sigmoid(i / s\_in) * s\_out). The model description specifies
s\_in (default 256) and s\_out (default 65536).

\emph{\textbf{6.2.2 Tanh}}

Tanh(x) = (exp(x) - exp(-x)) / (exp(x) + exp(-x)). Similar
parameterization to sigmoid; default s\_in = 128, s\_out = 32768.

\emph{\textbf{6.2.3 GELU}}

GELU(x) = x * Phi(x), where Phi is the standard normal CDF.
Parameterized by s\_in (default 256) and s\_out (default 65536).

\emph{\textbf{6.2.4 ReLU}}

ReLU(x) = max(0, x). MUST be implemented as max(0, x) directly, not as a
table; this is a special case for efficiency.

\emph{\textbf{6.2.5 Softmax}}

Softmax requires special handling because it involves division and is
not directly a table lookup. The decomposition is:

\begin{quote}
1. shift: x\textquotesingle{} = x - max(x) // numerical stability

2. exp\_lookup: y\_i = exp\_table{[}x\textquotesingle\_i{]} // integer
table

3. sum: S = sum(y\_i) // ordered reduction

4. division: z\_i = (y\_i * scale) / S // integer division
\end{quote}
...
\begin{quote}
1. mean: mu = sum(x) / n // ordered reduction + division

2. centered: c\_i = x\_i - mu

3. variance: v = sum(c\_i\^{}2) / n // ordered reduction + division

4. rsqrt\_lookup: r = rsqrt\_table{[}v + eps{]}

5. scale: z\_i = c\_i * r * gamma + beta
\end{quote}

The rsqrt table covers the variance range observed in training, with eps
added to avoid division by zero. eps and the rsqrt table parameters MUST
be in the model description.
...
\textbf{R-7.1.1 (Update rule).} The update is: param{[}i{]} -=
shift\_right(grad{[}i{]} * learning\_rate, lr\_scale).

\textbf{R-7.1.2 (Parameter ranges).} learning\_rate MUST be a
non-negative integer. lr\_scale MUST be a non-negative integer
specifying the right-shift after multiplication.

\textbf{R-7.1.3 (Saturation).} If the update produces a value outside
the parameter type range, the parameter MUST be clamped to the type
range. The clamping behavior MUST match the CPU reference target.

\textbf{7.2 SignSGD}

\textbf{R-7.2.1 (Update rule).} The update is: param{[}i{]} -=
learning\_rate * sign(grad{[}i{]}), where sign returns -1, 0, or +1 for
negative, zero, and positive gradient respectively.

\textbf{R-7.2.2 (Trivial conformance).} SignSGD is trivially compatible:
the update function is integer-to-integer with no real-valued
operations.

\textbf{7.3 Integer Adam}

\textbf{R-7.3.1 (State).} Adam state per parameter is (m, v) where m is
the first moment and v is the second moment, both integer-valued.

\textbf{R-7.3.2 (Update rule).} m\_new = (beta1 * m + (1 - beta1) *
grad) (in integer arithmetic with explicit scaling); v\_new = (beta2 * v
+ (1 - beta2) * grad\^{}2); param\_new = param - learning\_rate * m\_new
* rsqrt\_table{[}v\_new + eps{]}. The rsqrt\_table is part of the model
description.
...
\textbf{R-8.2.2 (Lookup).} Sync-point lookup for byte position N MUST
use binary search to find the largest byte\_offset \textless= N. The
lookup is O(log(index\_length)).
...
\begin{quote}
function range\_decode(archive, N, k):
...
P = binary\_search(archive.seek\_index, N) \# largest sync\_point
\textless= N

state = initialize\_predictor\_state(archive, P)

bit\_offset = archive.seek\_index{[}P{]}.repair\_bit\_offset

output = bytearray(k)

for i in {[}P.byte\_offset .. N + k):

Y\_i = predictor(state, context\_window)

repair\_bits = read\_bits(archive.repair\_stream, bit\_offset,
length\_for\_position\_i)

bit\_offset += length\_for\_position\_i

byte\_i = decode\_position(Y\_i, repair\_bits)

context\_window.append(byte\_i)

if i \textgreater= N and i \textless{} N + k:
...
\begin{quote}
function open(archive\_path) -\textgreater{} handle

function close(handle)

function read(handle, byte\_offset, length, buffer) -\textgreater{}
bytes\_read

function get\_size(handle) -\textgreater{} archive\_logical\_size
\end{quote}

Optional extensions for filesystem use:

\begin{quote}
function readv(handle, iovecs) -\textgreater{} bytes\_read \#
scatter-gather read

function readahead(handle, byte\_offset, length) \# cache warmup hint

function fstat(handle) -\textgreater{} archive\_metadata
\end{quote}

\textbf{9.2 Implementation pseudocode}

\begin{quote}
function read(handle, byte\_offset, length, buffer):

\# Check cache first

cached = cache\_lookup(byte\_offset, length)
...
chunk = range\_decode(handle.archive, byte\_offset, length)

cache\_insert(byte\_offset, chunk)
...
\begin{quote}
getattr(path) -\textgreater{} file metadata

readdir(path) -\textgreater{} directory entries \# if archive is a
filesystem image

open(path, flags) -\textgreater{} file handle

read(handle, offset, length, buffer) -\textgreater{} bytes\_read

release(handle) -\textgreater{} void

statfs(path) -\textgreater{} filesystem statistics
\end{quote}
...
mount -t overlay overlay \textbackslash{}
...
\textbf{R-11.1.1 (Hash per sync point).} Each sync
point\textquotesingle s seek-index entry MAY include a hash of the bytes
{[}P, P+K) of the source, computed with SHA-256 truncated to 64 bits.
The random-access reader verifies the hash after decoding the region.

\textbf{R-11.1.2 (Overhead).} Adds 8 bytes per sync point. For K = 64
KiB and N = 1 GiB, this is N/K * 8 = 128 KiB or about 0.01\% overhead.
...
\textbf{R-11.2.2 (Random access verification).} A reader that performs
random access at position N obtains the Merkle path from the leaf
containing N up to the root (a logarithmic number of hashes). The reader
verifies the path against the stored root.

\textbf{R-11.2.3 (Overhead).} Adds 32 bytes per sync point plus the tree
storage. For N = 1 GiB and K = 64 KiB, the tree has approximately 32 KiB
sync points and total tree storage is approximately 2 MiB, or 0.2\%
overhead.
...
runner: linux-x86\_64

- target: avx512

runner: linux-x86\_64-avx512

- target: neon

runner: linux-arm64

- target: nvidia-tensor-core

runner: linux-x86\_64-nvidia

- target: amd-mfma

runner: linux-x86\_64-amd-gpu

- target: apple-ane

runner: macos-arm64-m2

- target: edge-tpu

runner: linux-coral-dev-board

for each target in build\_matrix:
...
\textbf{R-13.2 (Forward compatibility).} An implementation supporting
format version X.Y MUST read archives with version X.Z for any Z
\textless= Y. It MAY refuse to read archives with version X.Z for Z
\textgreater{} Y, but if it does read them, MUST honor all features it
understands and reject features it does not.
...
predictor hash; readers MUST verify the hash before decoding. Mismatched
hashes MUST cause a hard failure with a clear error message.
...
representation. Migration tools MUST be lossless and reversible where
possible.
...
\textbf{Theorem 4.3, 4.4 (E\_12 NP-hard, inapproximable)} → Heuristic
E\_12 selection in encoder; spec does not require optimal E\_12.

\textbf{Lemma 5.1, Theorems 5.2--5.4, Proposition 5.5, Theorem 5.6 (Type-II)} → Type-II implementation
(separately specified in encoder spec).

\textbf{Theorems 6.1, 6.4 (Type-III-A mode selection, tight 2*eps
bound)} → Mode-selection logic in encoder (separately specified).

\textbf{Theorems 6.2, 6.5 (Type-III-B mutual-information and
decomposition invariance)} → Byte tensor coding (encoder spec).

\textbf{Theorems 6.3, 6.6, 6.7, 6.8 (Type-III-C MDL, bits-back, NP-hard,
